...
function fnBusStationShow(){
    var _BusNo = '<div class="Bus"><div class="Title">There have <font>{$No}</font> results searched of "<font>{$Key}</font>"</div>{$NoList}</div>';
    var r='';
...
function fnShowInfo(i){
    ...
    var _BusNoInfo = '<ul class="Info"><li id="Cost{$i}"><span>Price:</span>{$Cost}</li><li id="Card{$i}"><span>Available ID Card:</span>{$Card}</li><li id="OperatingTime{$i}"><span>service time:</span>{$OperatingTime}</li></ul><div class="NameHead">'
                    ...
                    +'<div id="Tf{$i}" class="UnActionRight" onclick="fnFan({$i},0);">U Turn</div></div>'
                    +'<div class="BusShow"><div id="Time{$i}" class="Time">First Arrive:{$Stime}&nbsp;&nbsp; Last Arrive:{$Etime}</div><ul id="lininfo{$i}" class="BusName"></ul></div>'
