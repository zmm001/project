function ClassMark(){};
ClassMark.prototype._MarkerView = '<div style="width:200px;"><div style="height:45px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\''+GlobalConfig._ImgPath+'ding.png\',sizingMethod=\'image\');"><!--[if !IE]><!--><img src="'+GlobalConfig._ImgPath+'ding.png" /><!--<![endif]--></div><div class="dialog">'
                                +'<div class="title"><div><img src="'+GlobalConfig._ImgPath+'newmap10.gif" alt="Mark" style="margin-top:3px;" /></div><div style="overflow:hidden; width:140px; height:20px;">{$MarkerTitle}</div><a class="HideMarkerInfo" style="cursor:pointer" onclick="parent._Mark.fnMarkEx(\'{$id}\',this);" ></a></div>'
                                ...
                                +'</div><div style="border-bottom:solid 1px #666; margin:5px 1px 10px 1px;"></div><input id="MarkAdd{$id}" value="{$Add}" style="width:195px; display:none;"  /><div class="textLine" style="background-color:#ffffff; height:25px;"><div class="DelMarker" onclick="parent._Mark.Del(\'{$id}\',\'{$Clear}\');">Delete</div><div class="CopyMarker" onclick="parent._Mark.fnCopyCode(MarkAdd{$id},\'Copy success!\')">Copy</div></div></div></div></div>';
ClassMark.prototype._MarkerAdd = '<div id="MenuMarkerPop" style="width:300px; position:absolute; z-index:200;"><div style="height:45px;filter:progid:DXImageTransform.Microsoft.AlphaImageLoader(src=\''+GlobalConfig._ImgPath+'ding.png\',sizingMethod=\'image\');"><!--[if !IE]><!--><img src="'+GlobalConfig._ImgPath+'ding.png" /><!--<![endif]--></div>'
                                +'<div class="dialog" style="height:195px;"><div class="title"><div><img src="'+GlobalConfig._ImgPath+'newmap10.gif" alt="Mark" style="margin-top:3px;" /></div><div style="margin-left:2px;">Mark</div><a style="cursor:pointer" onclick="parent._Mark.RemoveMarkPop(\'AddMarkerPop\');"></a></div>'
                                +'<div class="content"><div class="all"><div class="textLine"><div style="float:left;">Title: </div><input id="txtMarkerTitle" value="{$MarkerTitle}" style="width:220px; color:#333; border:solid 1px #666; margin-left:2px;" type="text" /><span class="redText">&nbsp;*</span>'
                                ...
                                +'<div class="textLine"><div style="float:left;">Content:</div><textarea id="txtMarkerContent" style="width:220px; height:70px; border:solid 1px #666;"></textarea>'
...
ClassMark.prototype.Load = function(){
    ...
    var aMark = c.getCookie('Mark')==null?'':c.getCookie('Mark').split('|');
	var r='';
	$('MarkLi').innerHTML=' <li onclick="if (confirm(\'Are you sure clear all marks?\')){_Mark.Clear()}">Clear</li>';
	for(i=0;i<aMark.length-1;i++){
        var aryTemp = aMark[i].split('^');
...
ClassMark.prototype.Save=function(x,y){
    if (vM.$('txtMarkerTitle').value.length < 1){
        vM.$('divMarkerError').innerHTML = 'you need to input the title';
        vM.$('divMarkerError').style.display = 'block';
        return;
    }
    if (vM.$('txtMarkerTitle').value.length > 20){
        vM.$('divMarkerError').innerHTML = 'the title can no longer than 20 characters';
        vM.$('divMarkerError').style.display = 'block';
        return;
    }
...
//    }
    if (vM.$('txtMarkerContent').value.length > 100){
        vM.$('divMarkerError').innerHTML = 'the content can no longer than 100 characters';
        vM.$('divMarkerError').style.display = 'block';
        return;
    }
