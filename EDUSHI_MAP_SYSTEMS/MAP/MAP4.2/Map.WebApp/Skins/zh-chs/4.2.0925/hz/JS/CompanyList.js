...
PageCard.prototype.Show=function(o){
    ...
    var t='<div class="Number"><span>收录{$count}家企业</span><span style="float:right;">[<a href="http://' + window.Config._AppDomain + '/LiveIn/Default.aspx?OwnerID=' + fnRequest('oid') + '&OwnerName=' + fnRequest('oname') + '" target="_blank">企业入住</a>]</span></div><ul class="ShowList">{$Record}</ul><div class="Fpage">{$PageStr}</div>';                 
    var strRecord='<li style="overflow:hidden;"><a href="' + window.Config._LocalUrl + 'Shop/{$CompanyID}.shtml" style="{$Style}" target="_blank" title="{$CompanyName}">{$CompanyName}</a></li>';
