...
function fnOnload() {
    ...
    x1 = fnRequest('x1'); y1 = fnRequest('y1'); x2 = fnRequest('x2');y2 = fnRequest('y2');
    var url1;
    url1 = window.Config.EDataCenterUrl + 'CommMap5.0/Bus.aspx?domain=' + window.Config.Domain + '&req=7&l=' + window.Config.Language + '&x=' + fnRequest('x1') + '&y=' + fnRequest('y1');
    var url2;
    url2 = window.Config.EDataCenterUrl + 'CommMap5.0/Bus.aspx?domain=' + window.Config.Domain + '&req=7&l=' + window.Config.Language + '&x=' + fnRequest('x2') + '&y=' + fnRequest('y2');
    var url2;
    LoadHTML = '';
    ENetwork.DownloadScript(url1, function() {
        if (typeof _Station == 'undefined' || _Station == null) {
            LoadHTML = "<div id=\"start\">起点：附近未找到公交站点</div>";
            flgData = false;
        }
        else {
        ...
        ENetwork.DownloadScript(url2, function() {
            if (typeof _Station == 'undefined' || _Station == null) {
                LoadHTML += "<div id=\"end\">终点：附近未找到公交站点</div>";
                flgData = false;
            }
            else {
                EndStation = _Station;
...
function inputclick(type, i) {
    if (!flgData)
        return;
    if (type == '1') {
        moveto(StartStation[i].PositionX, StartStation[i].PositionY, 1);
        $('startLi' + i).style.display = "none";
        $('startDiv').className = "bus_blue";
        $('startul').style.display = "none";
        $('startStationName').innerHTML = StartStation[i].StationName;
        _startIndex = i;
    ...
    if (type == '2') {
        moveto(EndStation[i].PositionX, EndStation[i].PositionY, 2);
        $('endLi' + i).style.display = "none";
        $('endDiv').className = "bus_blue";
        $('endul').style.display = "none";
        $('endStationName').innerHTML = EndStation[i].StationName;
        _endIndex = i;
        if (_startIndex != -1 && _endIndex != -1) {
            fnTransfer(_startIndex, _endIndex);
        }
...
function fnLoadStartList(type) {
    ...
    HtmlLi += "					<div style=\"float:left;\"><div class=\"pub_place\"><a style=\"cursor:pointer;\" onclick=\"moveto({$X},{$Y},1);\">{$stationName}</a></div></div>";
    ...
    if (StartStation.length==0) {
        ...
        if (StartStation.length > 0) {
            _RecordCount = StartStation.length;
            ...
            start = HtmlStartTitle.replace("{$StartStation}", '我的位置');
            var t = 1;
            for (var k = Begin; k < End; k++) {
                _temphtml = HtmlLi;
                item = StartStation[k];
                e += _temphtml.replace("{$stationName}", item.StationName).replaceAll("{$Li}", "startLi" + k).replace("{$i}", t).replaceAll('{$num}', k).replace('{$X}', item.PositionX).replace('{$Y}', item.PositionY);
                t++;
            }
            ...
            start = "<div id=\"start\">起点：附近未找到公交站点</div>";
        }
...
function fnLoadEndList(type) {
    ...
    HtmlLi += "				<li onmouseover=\"a({$type},{$num});\" onmouseout = \"b({$type},{$num});\">";
    HtmlLi += "					<div class=\"pub_nub_r\" >{$i}</div>";
    HtmlLi += "					<div style=\"float:left;\"><div class=\"pub_place\"><a style=\"cursor:pointer;\" onclick=\"moveto({$X},{$Y},2);\">{$stationName}</a></div></div>";
    ...
    if (EndStation.length == 0) {
        ...
        if (EndStation.length > 0) {
            _RecordCount = EndStation.length;
            if ((_RecordCount % _PageSize) == 0) {
                _PageCount = _RecordCount / (_PageSize);
            }
            else {
            ...
            e = '';
            end = HtmlEndTitle.replace("{$EndStation}", '我的位置');
            var t = 1;
            for (var j = Begin; j < End; j++) {
                _temphtml = HtmlLi;
                item = EndStation[j];
                e += _temphtml.replace("{$stationName}", item.StationName).replaceAll("{$Li}", "endLi" + j).replace("{$i}", t).replaceAll('{$num}', j).replace('{$X}', item.PositionX).replace('{$Y}', item.PositionY); ;
                t++;
            }
            ...
            end = "<div id=\"end\">终点：附近未找到公交站点</div>";
        }
        if (type == 1) {
...
function fnTransfer(sindex, eindex) {
    ...
    startName = sItem.StationName;
    endName = eItem.StationName;
    var stempType = 1;
    var etempType = 1;
    var url = window.Config.EDataCenterUrl + 'CommMap5.0/Transfer.aspx?domain=' + window.Config.Domain + '&l=' + window.Config.Language + '&StartId=' + sItem.StationID + '&StartName=' + escape(sItem.StationName) + '&StartType=' + stempType + '&StartX=' + sItem.PositionX + '&StartY=' + sItem.PositionY + '&EndId=' + eItem.StationID + '&EndName=' + escape(eItem.StationName) + '&EndType=' + etempType + '&EndX=' + eItem.PositionX + '&EndY=' + eItem.PositionY;
...
function fnShowTransferResult(result, index, s, e) {
    ...
    end += "			<li class=\"bus_way1_down\"><span style=\"height:16px;display:block;float:left;\"><img src=\"../images/pcde_170.png\" /></span><a style=\"height:16px;display:block;float:left;cursor:pointer;\" onclick=\"parent.ShowSendHtml(_phonedata[" + (index - 1) + "]);\"><b>免费发送到手机</b></a>>> 全程" + result.TransferLength / 1000 + "公里</li>";
...
function fnDisplayResult(id) {
    ...
    createIcon(parseInt(x1), parseInt(y1), 1);
    createIcon(parseInt(x2), parseInt(y2), 2);
    ...
    for (var i = 0; i < result.TransferNodeList.length; i++) {
        ...
        if (coords.length > 0) {
            ...
            if (i == 0) {
                CoordList[CoordList.length] = { Coords: parseInt(x1) + ',' + parseInt(y1) + ',' + result.StartStation.PositionX + ',' + result.StartStation.PositionY, Color: '#0099FF', Alpha: 0.7, Size: 3, PassStation: [] };
            }

            if (result.TransferNodeList.length == 1 || result.TransferNodeList.length == 3 && i == 2) {
                CoordList[CoordList.length] = { Coords: result.StopStation.PositionX + ',' + result.StopStation.PositionY + ',' + parseInt(x2) + ',' + parseInt(y2), Color: '#0099FF', Alpha: 0.7, Size: 3, PassStation: [] };
            }
        }
...
function fnExit() {
    ...
    parent.fnDrawingBusLine([]);
    if (parent.parent.vM.Body.MapState.Succeed) {
        parent.parent.vM.Body.MapSwitch.Switch3D();
    }
}
//初始化高度
function fnResize(h) {
