...
function fnOnload() {
    ...
    ENetwork.DownloadScript(url, function() {
        if (typeof _StartStation == 'undefined' || _StartStation == null) {
            LoadHTML = '<div class="gj_title_01"><div><span></span>起点不存在或尚未登记！请输入周边地名或试试该建筑物的名称！</div></div>';
            flgData = false;
        }
        else {
            fnLoadStartList(0);
        ...
        if (typeof _EndStation == 'undefined' || _EndStation == null) {
            LoadHTML += '<div class="gj_title"><div><span></span>终点不存在或尚未登记！请输入周边地名或试试该建筑物的名称！</div></div>';
            flgData = false;
        }
        else {
            fnLoadEndList(0);
...
function fnLoadStartList(type) {
    ...
    if (typeof _StartStation == 'undefined' || _StartStation == null) {
        LoadHTML = '<div class="gj_title"><div><span></span>起点不存在或尚未登记！请输入周边地名或试试该建筑物的名称！</div></div>';
    }
    else {
        var start = '', e = '', item;
        var _temphtml = "";
        ...
        if (_StartStation.length > 0) {
            ...
            start = '<div class="gj_title"><div><span></span>起点不存在或尚未登记！请输入周边地名或试试该建筑物的名称！</div></div>';
        }
...
function fnLoadEndList(type) {
    ...
    if (typeof _EndStation == 'undefined' || _EndStation == null) {
        ...
        if (_EndStation.length > 0) {
            ...
            end = '<div class="gj_title"><div><span></span>终点不存在或尚未登记！请输入周边地名或试试该建筑物的名称！</div></div>';
        }
        if (type == 1) {
...
function resultShow(data) {
    ...
    strHtml = '<div class="z3" id="bus_radio"><span><input type="radio" id="radio1" name="radio" onclick="selectBtn(1);" /><label>最快到达</label></span><span><input type="radio" name="radio" onclick="selectBtn(2);" id="radio2" /><label>最少换行</label></span><span><input id="radio3" type="radio" name="radio" onclick="selectBtn(3);"/><label>最小步行</label></span></div>';
