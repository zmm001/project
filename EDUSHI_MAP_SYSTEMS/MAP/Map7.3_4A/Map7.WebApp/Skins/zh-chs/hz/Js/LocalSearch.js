...
function fnShowData() {
    ...
    for (i = Begin; i < End; i++) {
        ...
        if (_Search[i].RecordType == 1 || _Search[i].RecordType == 2 || _Search[i].RecordType == 4) {
            sTmpHtml = LiHtml.replace('{$No}', i + 1).replace('{$Title}', _Search[i].OCName).replace('{$Title}', t.replace("'", "’")).replace('{$X}', _Search[i].X).replace('{$Y}', _Search[i].Y).replace('{$oid}', _Search[i].OwnerID).replace('{$cid}', _Search[i].CompanyID).replace('{$lstid}', _Search[i].LST_ID).replace('{$DataType}', _Search[i].RecordType).replace('{$Name}', _Search[i].OCName.replace("'", "’")).replace('{$Telephone}', _Search[i].Telephone).replace('{$Address}', sAddress).replace('{$Class}', 'SanWei').replace('{$num}', i);
        }
        else {
            sTmpHtml = LiHtml.replace('{$No}', i + 1).replace('{$Title}', _Search[i].OCName).replace('{$Title}', t.replace("'", "’")).replace('{$X}', _Search[i].Lx).replace('{$Y}', _Search[i].Ly).replace('{$oid}', _Search[i].OwnerID).replace('{$cid}', _Search[i].CompanyID).replace('{$lstid}', _Search[i].LST_ID).replace('{$DataType}', _Search[i].RecordType).replace('{$Name}', _Search[i].OCName.replace("'", "’")).replace('{$Telephone}', _Search[i].Telephone).replace('{$Address}', sAddress).replace('{$Class}', 'ErWei').replace('{$num}', i);
        }
...
function RecordClick(oid, cid, lstid, X, Y, DataType, Name,o, Telephone, num) {
    ...
    parent.fnShowSearchPop(oid, cid, lstid, X, Y, DataType, '', '', '' + Telephone + '');
}
