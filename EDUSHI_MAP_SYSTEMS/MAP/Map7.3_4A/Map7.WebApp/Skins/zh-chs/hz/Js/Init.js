...
function onSearchDataLoadComplete(data, begin, end) {
    ...
    for (var i = begin; i < end; i++) {
        ...
        if (data[i].RecordType == 1 || data[i].RecordType == 2 || data[i].RecordType == 4) {
            fnAppendIcon(data[i].OCName, x, y, i + 1, 'Images/VesicleBg.png', 'Images/AlterVesicle.png', 'if(parent.fnShowSearchPop){parent.fnShowSearchPop(' + data[i].OwnerID + ', ' + data[i].CompanyID + ',' + data[i].LST_ID + ',' + x + ', ' + y + ',' + data[i].RecordType + ',\'\',\'' + data[i].Address + '\',\'' + data[i].Telephone + '\');}', 41, 33, 7, 32, true);
        }
        else {
            var epoint = vM.ELatLng2EPoint(new AlaMap.LatLng(data[i].Lx, data[i].Ly));
            x = epoint.X;
            y = epoint.Y;
            fnAppendIcon(data[i].OCName, x, y, i + 1, 'Images/VesicleBg.png', 'Images/AlterVesicle.png', 'if(parent.fnShowSearchPop){parent.fnShowSearchPop(' + data[i].OwnerID + ', ' + data[i].CompanyID + ',' + data[i].LST_ID + ',' + data[i].Lx + ', ' + data[i].Ly + ',' + data[i].RecordType + ',\'\',\'' + data[i].Address + '\',\'' + data[i].Telephone + '\');}', 41, 33, 7, 32, true);
        }
    };
}
...
//显示公交泡泡
function fnShowWaterBusStation(id, name, x, y) {
    if (!_WaterBusStationControl) {
        ENetwork.DownloadScript('/Controls/WaterBusStationControl.js', function() {
            ...
            _WaterBusStationControl.onLoadComplete = function() {
                _WaterBusStationControl.ShowBusStation(id, name, x, y);
                MoveTo(x * 1 + vM.GradeWin2EGIS(66), y * 1 + vM.GradeWin2EGIS(-96), true);
            };
...
function fnLoadWaterBusStation() {
    ...
    var url = GlobalConfig.DataCetnerSearchDataUrl + 'SearchTankerByRangeStation/' + GlobalConfig.CityCode + '/' + GlobalConfig.Language + '/Format/Json/Search?x=' + vM.CenterX() + '&y=' + vM.CenterY() + '&range=2000';
    ENetwork.DownloadScript(url, function() {
...
function fnAppendWaterBusIcon(id,name,x, y) {
    var iconHTML = '';
    iconHTML = '<div style ="width:20px;height:44px; cursor:pointer;" onclick="parent.fnShowWaterBusStation(' + id + ',\'' + name + '\',' + x + ',' + y + ');"><img src="../images/ab.gif"/></div>';
    var p = vM.$C('div');
    p.innerHTML = iconHTML;
