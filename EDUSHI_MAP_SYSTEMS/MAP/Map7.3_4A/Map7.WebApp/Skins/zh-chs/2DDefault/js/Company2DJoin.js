...
function StartGetUnitCoords() {
    if (parent.fnStartGetUnitCoords()) {
        ...
        $('imgGetUnitCroods').src = $('imgGetUnitCroods').src.replace('ReMapAc.gif', 'ToMapRope.gif');
        $('hidCoords').value = '';
    }
}
function EndGetUnitCoords(coords) {
    $('strongUnitSelect').innerHTML = '已选择';
    $('imgGetUnitCroods').src = $('imgGetUnitCroods').src.replace('ToMapRope.gif', 'ReMapAc.gif');
...
function LoadCompanySmallType(sBigType) {
    if (sBigType != "-1" && sBigType != "") {
        cmXMLHttpReq(0, 'Company2DJoin.aspx?operatortype=0&companybigtypeid=' + sBigType + '&rd=' + Math.round(Math.random() * 10000), "", callBack_LoadSmallType);
    }
    else {
