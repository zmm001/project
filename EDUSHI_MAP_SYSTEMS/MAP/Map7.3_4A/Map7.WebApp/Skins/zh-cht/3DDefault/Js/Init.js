...
function fnMapEventBind() {
    //滑杆样式
    vM.$('ZoomBarControl').style.backgroundImage = "url('" + AlaMap.ApplictionRootUrl + "/images/ud.gif')";
    //绑定滑杆
    ...
    //测距结束
    vM.attachEvent(AlaMap.KeyWord.EventName.ScaleEnd, function(dis) {
    ...
    vM.attachEvent(AlaMap.KeyWord.EventName.Switch2D, function() {
        fnInitDelay();
        fnGoogleStat("切换2D图");
        vM.$('ZoomBarControl').style.backgroundImage = "url('" + AlaMap.ApplictionRootUrl + "/images/ZoomBarControl.gif')";
...
window.onresize = function() {
    ...
    if (_MapFullScreenState) {
        ...
        //        $('EdushiMap').style.width = (w - 324) + 'px';
        //        $('EdushiMap').style.height = (h - 122) + 'px';
...
function fnLoadHotKey() {
    //var url = GlobalConfig.EDataCenterUrl + 'CommMap5.0/ad.aspx?domain=' + GlobalConfig.Domain + '&l=' + GlobalConfig.Language + '&req=4';
    //新数据中心
        ...
        ENetwork.DownloadScript(sUrl, function() {
            ...
            if (typeof (_BdtList) != 'undefined' && _BdtList.length > 0) {
                ...
                */
                //向左滚动
                for (var i = 0; i < _BdtList.length; i++) {
                    arrayHtml.push('<a style=\"font-weight:bold;filter:alpha(opacity=100);opacity:1;\" href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=' + _BdtList[i].LM_ID + '" target="_blank">' + _BdtList[i].LM_Nickname + '</a>正在打听：<a style=\"font-weight:bold;filter:alpha(opacity=100);opacity:1;\" href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=' + _BdtList[i].LT_ID + '" target="_blank">' + _BdtList[i].LT_Title + '</a>&nbsp;&nbsp;');
                }
                $('divBdtList').innerHTML = arrayHtml.join('');
...
function fnLoadThemeMapsTypeList() {
    ...
    ENetwork.DownloadScript(url, function() {
        ...
        for (var i = themsCount - 1, j = 0; i >= 0; i--) {
            ...
            if (j < 4) {
                t += '<a onfocus="this.blur()" title="' + _ThemeMaps[i].BCT_TypeName + '" href="javascript:fnLoadThemeMapListByTypeId(' + _ThemeMaps[i].BCT_ID + ', \'' + _ThemeMaps[i].BCT_TypeName + '\');"><span><img src=' + sFocusIcon + ' /></span><span>' + _ThemeMaps[i].BCT_TypeName + '</span></a>';
            }
            else {
                r += '<a onfocus="this.blur()" title="' + _ThemeMaps[i].BCT_TypeName + '" href="javascript:fnLoadThemeMapListByTypeId(' + _ThemeMaps[i].BCT_ID + ', \'' + _ThemeMaps[i].BCT_TypeName + '\');"><span><img src=' + sFocusIcon + ' /></span><span>' + _ThemeMaps[i].BCT_TypeName + '</span></a>';
            }
...
function onSearchDataLoadComplete(data, begin, end) {
    ...
    for (var i = begin; i < end; i++) {
        ...
        if (data[i].RecordType == 1 || data[i].RecordType == 2 || data[i].RecordType == 4) {
            ...
            var epoint = vM.GLatLng2EPoint(new AlaMap.LatLng(data[i].Lx, data[i].Ly));
            x = epoint.X;
            y = epoint.Y;
...
function onGoogleSearchDataLoadComplete(data, begin, end, page) {
    _IconLayer.innerHTML = '';
    for (var i = begin; i < end; i++) {
        var epoint = vM.GLatLng2EPoint(new AlaMap.LatLng(data[i].lat, data[i].lng));
        var x = epoint.X;
        var y = epoint.Y;
        var phoneNumber = '';
...
function fnCheckUserLoginInit() {
    if ($('liLoginInfo').innerHTML == '') {
        ...
        if (cookieNickNameCookie != null) {
            $('liLoginInfo').innerHTML = '<span style="color:#ff6400;margin-right:5px;">欢迎您！ ' + cookieNickNameCookie + '</span><a href="' + GlobalConfig.UserCenterUrl + 'Index.aspx?city=' + GlobalConfig.CityCode + '" target="_blank">[我的E都市]</a><a href="javascript:fnLoginOut();">[退出]</a>';
        }
        else {
...
function fnLoginSuccess(memberInfo) {
    $('liLoginInfo').innerHTML = '<span style="color:#ff6400;margin-right:5px;">欢迎您！ ' + memberInfo.LM_NickName + '</span><a href="' + GlobalConfig.UserCenterUrl + 'Index.aspx?city=' + GlobalConfig.CityCode + '" target="_blank">[我的E都市]</a><a href="javascript:fnLoginOut();">[退出]</a>';
...
function fnShowEwPoiPop(x, y, name, address, telephone) {
    if (!_PoiPopControl) {
        ...
        if (vM.MapState().Map != 0) {
            var epoint = vM.GLatLng2EPoint(new AlaMap.LatLng(x, y));
