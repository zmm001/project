    ...
    visitedRegister: function (tab, type) {
        ...
        switch (type) {
            case 'add':
                ...
                break;
            case 'active':
                if (typeof tab.TabBody.contentWindow.ResultMode != 'undefined') {
                    if (fnGetResultModeByMapStatus() != tab.TabBody.contentWindow.ResultMode) {
                        if (tab.TabBody.src.indexOf("/Fundation/LocalSearch.aspx?") > -1 || tab.TabBody.src.indexOf("/Fundation/BusTransferSearch.html?") > -1) {
                            tab.TabBody.src = tab.Url;
                        }
                    }
                }
                if (typeof tab.TabBody.contentWindow.BusLineResultMode != 'undefined') {
                    if (fnGetResultModeByMapStatus() != tab.TabBody.contentWindow.BusLineResultMode && tab.TabBody.contentWindow.IsSearch) {
                        if (tab.TabBody.src.indexOf("/Fundation/BusNoSearch.html") > -1) {
                            tab.TabBody.contentWindow._BusLineSearch.BtnBusLineSearch();
                        }
                    }
                }
...
function Suggest(x, y, w, c) {
    ...
    (function() {
        ...
        this.SuggestPannel.style.cssText = 'width:' + w + 'px; height:auto;text-align:left;line-height:20px; border:solid 1px #cecece;overflow:hidden; position:absolute; z-index:99999; top:' + y + 'px; right:' + x + 'px; background-color:#fff;';
        ...
        if (this.WordList.length > 0) {
            for (var i = 0; i < this.WordList.length; i++) {
                if (i == this.SelectedIndex) {
                    this.SuggestPannel.WordList.childNodes[i].style.cssText = 'cursor:default; white-space:nowrap;overflow:hidden; background-color: #ddd; color:#000;';
                }
                else {
