...
Object.extend(FTPopControl.prototype, {
    ...
    ShowPop: function (index) {
        ...
        this.$('imgurl').href = 'http://taizhou.fangtoo.com/building/' + parent.FTHouseData[index].BID + '/';
        this.$('url_a').href = 'http://taizhou.fangtoo.com/building/' + parent.FTHouseData[index].BID + '/';
        this.$('address').innerHTML = parent.FTHouseData[index].A == undefined ? "" : parent.FTHouseData[index].A;
        var sNumHtml = '';
        if (parent.FTHouseData[index].TC && parseInt(parent.FTHouseData[index].TC) > 0) {
            sNumHtml += '二手房(<a target="_blank"  href="http://taizhou.fangtoo.com/es/ck' + encodeURI(parent.FTHouseData[index].Name) + '/">' + parent.FTHouseData[index].TC + '</a>套)&nbsp;&nbsp;';
        }

        if (parent.FTHouseData[index].LC && parseInt(parent.FTHouseData[index].LC) > 0) {
            sNumHtml += '出租(<a target="_blank" href="http://taizhou.fangtoo.com/cz/ck' + encodeURI(parent.FTHouseData[index].Name) + '/">' + parent.FTHouseData[index].LC + '</a>套)';
        }
