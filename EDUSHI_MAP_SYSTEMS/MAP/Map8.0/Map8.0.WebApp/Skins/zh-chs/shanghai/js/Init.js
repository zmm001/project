...
//公交线路搜索
function fnNewBusSearchByLine() {
    tab = new TabControl.Tab(document, 'buslinesearch', '线路查询', GlobalConfig.WebRootPath + 'Fundation/BusNewSearch.html', true, true, 80);
    fnAddTab(tab);
}

//公交站点搜索
function fnNewBusSearchByStation() {
    tab = new TabControl.Tab(document, 'busstationsearch', '站点查询', GlobalConfig.WebRootPath + 'Fundation/BusNewStationSearch.html', true, true, 80);
    fnAddTab(tab);
}
...
function fnShowBusStation(id, name, x, y) {
    if (!_BusStationControl) {
        ENetwork.DownloadScript('/Controls/BusStationControl.js', function () {
            ...
            _BusStationControl.ID = vM.AppendEntity(_BusStationControl.Body, _PopLayer, false, x, y, 354, 340, 64, 280, false);
        ...
        if (typeof SubWayIcon != 'undefined' && SubWayIcon.length > 0) {
            ...
            for (var i = 0; i < SubWayIcon.length; i++) {
                ...
                switch (listlen) {
                    case 1:
                        var _temp = popHTML;
                        r = string.Format('<span  style="background:{0};  color:#fff;  padding:0 11px; ">{1}</span>', oblist[0].Color, oblist[0].Asid);
                    ...
                    case 2:
                        ...
                        for (var k = 0; k < listlen; k++) {
                            r += string.Format('<span style="background:{0};  color:#fff;  padding:0 3px; margin-right:1px; ">{1}</span>', oblist[k].Color, oblist[k].Asid);
                        }
                    ...
                    case 3:
                        var _temp = popHTML;
                        for (var k = 0; k < listlen; k++) {
                            r += string.Format('<span style="background:{0};  color:#fff;  padding:0 1px; margin-right:1px; ">{1}</span>', oblist[k].Color, oblist[k].Asid);
                        }
...
/*--------地铁图层的加载begin-------------------*/
function ShowSubwayLayer() {

    var h = vM.MapHeight();
    var w = vM.MapWidth();

    if (!$('SubwaydivDialogBg')) {
        var div = $C('div');
        div.id = 'SubwaydivDialogBg';
    ...
    if (!$('SubwaydivDialog')) {
        var divBox = $C('div');
        divBox.id = 'SubwaydivDialog';
        divBox.style.left = '1px';
        divBox.style.top = '120px';

        divBox.style.position = 'absolute';
        divBox.style.zIndex = 1002;
        divBox.style.width = w + 'px';
        divBox.style.height = h + 'px';
        var boxHtml = '<a href="javascript:fnCloseSubway()" style="position:absolute;right:10px;top:10px; padding:2px 4px;background:#ccc;">关闭</a><iframe src="http://' + GlobalConfig.Domain + '/shanghaisubway.htm" id="iframesubway" width="' + w + '" height="' + h + '" scrolling="no" frameborder="0"></iframe>';
        document.body.appendChild(divBox);
        setTimeout(function () { divBox.innerHTML = boxHtml; }, 2);
    }
    setTimeout(function () {
        $('SubwaydivDialogBg').style.display = 'block';
        $('SubwaydivDialog').style.display = 'block';
    }, 10); //延时解决IE6下图片有时不能加载的BUG

    __IsResizeSubway = true;
}
function fnCloseSubway() {
    __IsResizeSubway = false;
    $('SubwaydivDialogBg').style.display = 'none';
    $('SubwaydivDialog').style.display = 'none';
}
function ResizeDialog() {
    ...
    if (!$('SubwaydivDialogBg')) {
        ...
        var divdiog = $('SubwaydivDialogBg');
        divdiog.style.width = w + 'px';
        divdiog.style.height = h + 'px';
    }
    if (!$('SubwaydivDialog')) {
        var divBox = $C('div');
        divBox.id = 'SubwaydivDialog';
        divBox.style.left = '2px';
        divBox.style.top = '120px';
        ...
        var newdivbox = $('SubwaydivDialog');
        var viframe = $('iframesubway');
        newdivbox.style.width = w + 'px';
        newdivbox.style.height = h + 'px';
        viframe.style.width = w + 'px';
        viframe.style.height = h + 'px';
    }
