    ...
    if (_Search.length > 0) {
        ...
        if (_SearchType == 3) {
            ...
            $('searchkeybdt').onclick = function () {
                window.open('http://sh.edushi.com/bdt/?content=' + encodeURIComponent($('searchkey').value));
            };
...
function fnResize(h) {
    if (!h) {
        this.$('TabContent').style.height = (fnGetWindowHeight() - 90) + 'px';
    }
    else {
        this.$('TabContent').style.height = (h - 90) + 'px';
    }
}
