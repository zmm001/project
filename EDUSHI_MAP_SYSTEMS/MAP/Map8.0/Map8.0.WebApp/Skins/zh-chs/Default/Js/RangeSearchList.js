...
function fnShowData() {
    ...
    if (_Range.length > 0) {
        ...
        for (i = Begin; i < End; i++) {
            ...
            LiHtml += string.Format('<li x="{12}" y="{13}" iN="{14}"><div class="MC"><div class="nb"><span class="icon">{0}</span></div><div class="location-r"><a href="javascript:;" onclick="parent.fnShowSearchPop({1},{2},{3},{4},{5},{6},\'{7}\',\'{8}\',\'\');" title="{9}">{10}</a>{15}<div class="DZ kk260">{11}</div></div><div class="clear"></div></div></li>', i + 1, OwnerID, CompanyID, LST_ID, X, Y, RecordType, sName, sAddress, sName, sName, sAddress, X, Y, i, sVipIcon);
        }
