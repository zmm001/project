...
Object.extend(EShopPopControl.prototype, {
    ...
    PopAD: [],
    Tab: null,
    dianTaoCan: ["", "金牌E店", "", "", "", "钻石E店", "至尊E店", "未认领E店", "已认领E店"],
    ...
    LoadPopData: function () {
        if (typeof _EntityInfo != 'undefined' && _EntityInfo.length > 0) {
            ...
            if (o.LST_ID * 1 > 0) {
                if (o.LET_ID * 1 > 0) {
                    Name_h3Html.push('<img src="/Images/dian' + o.LET_ID + '.gif" title="' + this.dianTaoCan[o.LET_ID * 1] + '" />');
                    if (o.LET_ID * 1 == 7) {
                        this.$("regShop").style.display = '';
                        this.$("regShop").href = "http://" + this.Config.CityCode + ".edushi.com/dian/join.html?storeid=" + o.CompanyID;
                    }
                    else {
                        this.$("regShop").style.display = 'none';
                    }
                }
                else {
                    Name_h3Html.push('<img src="/Images/dian1.gif" title="' + this.dianTaoCan[1] + '" />');
                }
    ...
    BtnRangSearch: function (key, x, y, oName) {
        switch (key) {
            case 'hotel':
                ...
                break;
            case 'food':
                parent.rangeSearch(x, y, '美食', oName, 'eshop', this.OID);
                break;
            case 'feature':
                parent.rangeSearch(x, y, '景点', oName, 'eshop', this.OID);
                break;
//            case 'bank':
