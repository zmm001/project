var WaterBusStationControl = Class.create();
Object.extend(WaterBusStationControl.prototype, ControlBase.prototype);
Object.extend(WaterBusStationControl.prototype, {
    Tab: null, //页卡
    CopyMsg: '复制好了，发给我的QQ/MSN好友分享吧！',
    _loadUI: function () {
        this.LoadUI('WaterBusStationControl');
    },
    //显示指定站点的公交信息，id：公交站点id
    ShowBusStation: function (id, sStationName, x, y, type, bid) {
        this.$('divStationName').innerHTML = sStationName;
        this.$('divBusList').innerHTML = 'loading...';
        this.Show();
        var url = this.Config.DataCetnerSearchDataUrl + 'SearchTankerStation/' + this.Config.CityCode + '/' + this.Config.Language + '/Format/Json/Search?q=' + id;
        ...
        var tempUrl = 'http://' + this.Config.Domain + '/?sid=' + id + '&x=' + x + '&y=' + y + '&sname=' + escape(sStationName) + '&ts=1&type=water';
        ...
        ENetwork.DownloadScript(url, function () {
            ...
            if (typeof _BusLineList == 'undefined' || _BusLineList.length < 1) {
                ...
                for (var k = 0; k < _BusLineList.length; k++) {
                    //sHtml += '<a href="javascript:;" onclick="window.WaterBusStationControl.onBusClick(' + _BusLineList[k].VehicleID + ',\'' + _BusLineList[k].VehicleName + '\',\'' + sStationName + '\')">' + _BusLineList[k].VehicleName + '</a>';
                    if (typeof type != 'undefined' && typeof bid != 'undefined') {
                        if (parseInt(type) == parseInt(_BusLineList[k].Direction) && parseInt(bid) == parseInt(_BusLineList[k].VehicleID)) {
                            sHtml = string.Format('<li><dl><dt class="waterbus-line-name"><em></em><a href="javascript:;" onclick="window.WaterBusStationControl.onBusClick({1},\'{2}\',\'{3}\')">{0}</a><span>（{5}班次信息）</span></dt><dd class="waterbus-line-time"><a href="javascript:;">{4}</a></dd></dl></li>', _BusLineList[k].VehicleName, _BusLineList[k].VehicleID, _BusLineList[k].VehicleName, sStationName, _BusLineList[k].VehicleFrequency, _BusLineList[k].Direction == 1 ? '上行' : '下行');
                            break;
                        }
                    } else {
                        sHtml += string.Format('<li><dl><dt class="waterbus-line-name"><em></em><a href="javascript:;" onclick="window.WaterBusStationControl.onBusClick({1},\'{2}\',\'{3}\')">{0}</a><span>（{5}班次信息）</span></dt><dd class="waterbus-line-time"><a href="javascript:;">{4}</a></dd></dl></li>', _BusLineList[k].VehicleName, _BusLineList[k].VehicleID, _BusLineList[k].VehicleName, sStationName, _BusLineList[k].VehicleFrequency, _BusLineList[k].Direction == 1 ? '上行' : '下行');
    ...
    _loadComplete: function () {
        this.Body.contentWindow.WaterBusStationControl = this;
