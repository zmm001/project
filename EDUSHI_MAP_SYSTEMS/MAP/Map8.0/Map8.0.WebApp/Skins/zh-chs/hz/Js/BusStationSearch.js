    ...
    this.fnBusStationShow = function(kw) {
        if (typeof _Station != 'undefined' && _Station.length > 0) {
            ...
            for (i = 0; i < l; i++) {
                ...
                s += string.Format("<div><div class=\"bus_zhan_mid\"><h2><div class=\"bus_zhan_title\" id=\"{5}\" onclick = \"javaccript:fnShowLi({0},{1},{2},'{3}',{4});\">{7}</div></h2><div id=\"{6}\" style=\"display:none;\"><ul class=\"bus_zhan_con\">{$liLine}</ul></div></div></div>", i, _Station[i].PositionX, _Station[i].PositionY, _Station[i].StationName, _Station[i].StationID, "img" + i, "stationdiv" + i, _tempName);
...
function fnShowLi(i,x,y,sName,sid) {
    if ($('stationdiv' + i).style.display == 'none') {
        ...
        $('img' + i).className = $('img' + i).className + ' bus_zhan_on';
    } else {
        $('stationdiv' + i).style.display = 'none';
        //$('img' + i).src = '../images/bus_ico3_1.gif';
        $('img' + i).className = "bus_zhan_title";
    }
