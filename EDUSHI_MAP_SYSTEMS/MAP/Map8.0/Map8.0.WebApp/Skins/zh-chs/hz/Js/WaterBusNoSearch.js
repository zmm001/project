...
function fnOnload() {
    ...
    if (action == 1) {
        ...
        ENetwork.DownloadScript(_url, function () { if (typeof _BusRoute != 'undefined') { _BusNumberData[0] = _BusRoute; _BusLineSearch.fnShowInfo(0, unescape(fnRequest('key'))); } });
    }
    else {
...
function BusLineSearch() {
    ...
    this.BusLineShow = function(kw) {
        ...
        if (typeof _Bus != 'undefined' && _Bus.length > 0) {
            ...
            var Td_Temp = '';
            for (i = 0; i < len; i++) {
                r += string.Format('<div class="gj_a1 gj_a2"><div class="title clearfix" onclick=\"javascript:_BusLineSearch.fnLoadData({4},{5});\"><div class="number2" id="number{6}">+</div><div class="T_left">{0}</div><div class="T_right"><a>{1}</a> <i>--</i> <a>{2}</a></div></div><div style="display: none;" id="{3}"></div></div>', _Bus[i].VehicleName, _Bus[i].StartStation, _Bus[i].EndStation, "LineInfo" + i, i, _Bus[i].VehicleID, i);
    ...
    this.fnLoadData = function (i, BusID) {
        if ($('LineInfo' + i).style.display == 'block') {
            ...
            if ($('LineInfo' + i).innerHTML == '') {
                ...
                ENetwork.DownloadScript(url, function () { _BusNumberData[i] = _BusRoute; _BusLineSearch.fnShowInfo(i,BusID); });
            }
            else {
                _CurrentIndex = i;
                parent.fnShowBusStationIco(_BusNumberData[i], _IsUpOrDown, 'water');
            }
    ...
    this.fnShowInfo = function(i,bid) {
        _CurrentIndex = i;
        if (action == 1) {
            $('textKeyWords').value = _BusNumberData[i].LineInfo.VehicleName;
            $('TabContent').innerHTML = string.Format('<div class="gj_a1 gj_a2"><div class="title clearfix" onclick=\"javascript:_BusLineSearch.fnLoadData({4},{5});\"><div class="number2" id="number{6}">--</div><div class="T_left">{0}</div><div class="T_right"><a>{1}</a> <i>--</i> <a>{2}</a></div></div><div style="display: none;" id="{3}"></div></div>', _BusNumberData[i].LineInfo.VehicleName, _BusNumberData[i].LineInfo.StartStation, _BusNumberData[i].LineInfo.EndStation, "LineInfo" + i, i, _BusNumberData[i].LineInfo.VehicleID, i);
            $('LineInfo' + i).style.display = 'block';
            $('resultDiv').innerHTML = '<span></span>共有<i>' + _BusNumberData.length + '项</i>符合<a>' + _BusNumberData[i].LineInfo.VehicleName + '</a>的搜索结果！';
            fnResize();
        }
        //var sBusNoInfo = string.Format('<div class="xlxx"><div class="time_bus"><div class="title_S">线路票价</div><div class="text_S" id="Price{0}">{16} 元</div><div class="clear"></div></div><div class="time_bus"><div class="title_S">可用IC卡</div><div class="text_S" id="Card{1}">{7}</div><div class="clear"></div></div><div class="time_bus"><div class="title_S">服务时间</div><div class="text_S" id="ServiceTime{2}">{8}</div><div class="clear"></div></div><div class="time_bus time_bus"><div class="title_S">班次信息</div><div class="text_S" id="WaterBusInfo{3}">{9}</div><div class="clear"></div></div><div class="time_bus2"><a href="javascript:;" class="a1" onclick="parent.fnCopyToClipboard(\'{4}\',\'复制好了，发给我的QQ/MSN好友分享吧！\');"></a><a href="javascript:;" class="a2" onclick="_BusLineSearch.fnReport(\'{12}\',\'{13}\')"></a><a href="javascript:_BusLineSearch.ViewCompleteBusLine({5});" class="a3"></a><div class="clear"></div></div><div class="clear"></div></div><div class="xl"><div class="title_xl"><div class="left current" id="Up1" onclick="_BusLineSearch.fnReverse({10},1,{14});">上行线路</div><div class="right" id="down2"  onclick="_BusLineSearch.fnReverse({11},0,{15});">下行线路</div></div><div class="title_xl" style="padding-top:6px;text-indent:5px;" id="OperatingTime{0}"></div><ul id="Tz{6}"></ul><div class="clear"></div></div>', i, i, i, i, 'http://' + window.Config.Domain + '/?bname=' + escape(_BusNumberData[i].LineInfo.VehicleName) + '&bid=' + _BusNumberData[i].LineInfo.VehicleID + '&type=water', i, i, _BusNumberData[i].LineInfo.BusCards, _BusNumberData[i].LineInfo.OperatingTime, _BusNumberData[i].LineInfo.BusFrequency, i, i, _BusNumberData[i].LineInfo.VehicleID, _BusNumberData[i].LineInfo.VehicleName, bid, bid, _BusNumberData[i].LineInfo.BusPrice);
        var sBusNoInfo = string.Format('<div class="xlxx"><div class="time_bus clearfix"><div class="title_S">线路票价</div><div class="text_S" id="Price{0}">{16} 元</div></div><div class="time_bus clearfix"><div class="title_S">可用IC卡</div><div class="text_S" id="Card{1}">{7}</div></div><div class="time_bus clearfix"><div class="title_S">服务时间</div><div class="text_S" id="ServiceTime{2}">{8}</div></div><div class="time_bus clearfix"><div class="title_S">班次信息</div><div class="text_S" id="WaterBusInfo{3}">{9}</div></div></div><div class="time_bus2 clearfix"><a href="javascript:;" onclick="parent.fnCopyToClipboard(\'{4}\',\'复制好了，发给我的QQ/MSN好友分享吧！\');"><span class="a1"></span>分享线路</a><a href="javascript:;" onclick="_BusLineSearch.fnReport(\'{12}\',\'{13}\')"><span class="a2"></span>我要纠错</a><!--<a href="javascript:_BusLineSearch.ViewCompleteBusLine({5});"><span class="a3"></span>查看整条线路</a>--></div><div class="xl"><div class="title_xl clearfix"><div class="left current" id="Up1" onclick="_BusLineSearch.fnReverse({10},1,{14});">上行线路</div><div class="right" id="down2"  onclick="_BusLineSearch.fnReverse({11},0,{15});">下行线路</div></div><div class="title_xl" style="padding:5px 0;text-indent:5px;" id="OperatingTime{0}"></div><ul id="Tz{6}" class="clearfix"></ul></div>', i, i, i, i, 'http://' + window.Config.Domain + '/?bname=' + escape(_BusNumberData[i].LineInfo.VehicleName) + '&bid=' + _BusNumberData[i].LineInfo.VehicleID + '&type=water', i, i, _BusNumberData[i].LineInfo.BusCards, _BusNumberData[i].LineInfo.OperatingTime, _BusNumberData[i].LineInfo.BusFrequency, i, i, _BusNumberData[i].LineInfo.VehicleID, _BusNumberData[i].LineInfo.VehicleName, bid, bid, _BusNumberData[i].LineInfo.BusPrice);
        $('LineInfo' + i).innerHTML = sBusNoInfo;

        _BusLineSearch.fnReverse(i, 1, bid);
    }

    this.fnReverse = function (no, k,bid) {
        _IsUpOrDown = k;
        var info = '';
        ...
        if (k == 1) {
            ...
            for (i = 0; i < _BusNumberData[no].BusUp.length; i++) {
                ...
                if (oBusUp.PositionX == 0 && oBusUp.PositionY == 0) {
                    ...
                    if (_CurrentBusStation == oBusUp.StationName) {
                        info += ' <li><a style=\"cursor:pointer;\" title=\"点击定位公交站\" onclick="parent.fnShowWaterBusStation(' + oBusUp.StationID + ',\'' + oBusUp.StationName + '\',' + oBusUp.PositionX + ',' + oBusUp.PositionY + ',1,' + bid + ');">' + (i + 1) + '.' + oBusUp.StationName + '</a></li>';
                    }
                    else {
                        info += ' <li><a style=\"cursor:pointer;\" title=\"点击定位公交站\" onclick="parent.fnShowWaterBusStation(' + oBusUp.StationID + ',\'' + oBusUp.StationName + '\',' + oBusUp.PositionX + ',' + oBusUp.PositionY + ',1,' + bid + ');">' + (i + 1) + '.' + oBusUp.StationName + '</a></li>';
                    }
            ...
            for (i = 0; i < _BusNumberData[no].BusDown.length; i++) {
                ...
                if (oBusDown.PositionX == 0 && oBusDown.PositionY == 0) {
                    ...
                    if (_CurrentBusStation == oBusDown.StationName) {
                        info += ' <li><a style=\"cursor:pointer;\" title=\"点击定位公交站\" onclick="parent.fnShowWaterBusStation(' + oBusDown.StationID + ',\'' + oBusDown.StationName + '\',' + oBusDown.PositionX + ',' + oBusDown.PositionY + ',2,' + bid + ');">' + (i + 1) + '.' + oBusDown.StationName + '</a></li>';
                    }
                    else {
                        info += ' <li><a style=\"cursor:pointer;\" title=\"点击定位公交站\" onclick="parent.fnShowWaterBusStation(' + oBusDown.StationID + ',\'' + oBusDown.StationName + '\',' + oBusDown.PositionX + ',' + oBusDown.PositionY + ',2,' + bid + ');">' + (i + 1) + '.' + oBusDown.StationName + '</a></li>';
                    }
        ...
        $('OperatingTime' + no).innerHTML = '首末班时间：首班：' + stime + '  末班：' + etime;

        info = null;
        parent.__IsDrawBusLineState = true;
        parent.fnShowBusStationIco(_BusNumberData[no], k, 'water');
    }
...
function fnActive() {
    if (typeof _BusNumberData[_CurrentIndex] != 'undefined' && _BusNumberData[_CurrentIndex].LineInfo.length > 0) {
        ...
        parent.__IsDrawBusLineState = true;
        parent.fnShowBusStationIco(_BusNumberData[_CurrentIndex], _IsUpOrDown, 'water');
    }
    else {
