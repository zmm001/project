...
//水上公交
var _WaterBusStationLayer;
var _BusShowType = "default";
var _BusTansShowType = "default";

var _FTLayer;

var IsHavePanoramaControl = false; //是否有全景控件
var pano = null; //全景实例
...
function LoadMap() {
    ...
    //水上巴士
    _WaterBusStationLayer = vM.NewMapLayer('WaterBusStation', 258, true);
    //fangtoo
    _FTLayer = vM.NewMapLayer('FTLayer', 257, true);
    ...
    //加载水上巴士
    fnLoadWaterBusStation();
    //加载弹窗广告
    //adscript();
    //setTimeout("fnShowPicture()", 2000);  //视窗广告
//    setTimeout(function () { //关闭视窗广告 10秒
//        jQuery("#picad").hide();
    //    }, 12000);
    if (vM.Zoom() < 2) {
//        fnLoadFTData();
    }
    AddSearchInfoWindowCssLink();
}
//百度搜索结果点击后在地图上显示的CSS
function AddSearchInfoWindowCssLink() {
    var __css1 = vM.$C('link');
    __css1.href = 'http://api.map.baidu.com/library/SearchInfoWindow/1.5/src/SearchInfoWindow_min.css';
    __css1.rel = 'stylesheet';
    __css1.type = 'text/css';
    vM.ContentWindow.document.getElementsByTagName('head')[0].appendChild(__css1);

    var __css2 = vM.$C('link');
    __css2.href = '/Css/SearchInfoWindow.css';
    __css2.rel = 'stylesheet';
    __css2.type = 'text/css';
    vM.ContentWindow.document.getElementsByTagName('head')[0].appendChild(__css2);
}
//百度搜索结果点击后在地图上显示的JS
function AddSearchInfoWindowJsLink() {
    var __bs1 = vM.$C('script');
    __bs1.charset = 'utf-8';
    __bs1.defer = true;
    __bs1.src = '/js/SearchInfoWindow.js';
    vM.ContentWindow.document.getElementsByTagName('head')[0].appendChild(__bs1);
}

//统计BDT发帖
function getBdtCount() {
    ENetwork.DownloadScript("Data/getData.aspx?action=IndexBdtCount&rnd=" + $Rnd(), function () {
        if (typeof vcount != 'undefined') {
            $('signcount').innerHTML = vcount;
        }
    });
}

function adscript() {
    var _script = vM.$C('script');
    _script.charset = 'utf-8';
    _script.src = GlobalConfig.DataCetnerAdDataUrl + 'Ad2.0/ads.aspx?citycode=' + GlobalConfig.CityCode + '&l=' + GlobalConfig.Language + '&key=tanchuangguanggao&domid=adpichtml';
    document.getElementsByTagName('head')[0].appendChild(_script);
    fnShowPicture();
}

function fnShowPicture() {
    var b = $("picad").style.bottom.replace('px', '');
    b = parseInt(b);
    if (b < 0) {
        b += 10;
        if (b > 0) {
        ...
        $("picad").style.bottom = b + 'px';
        setTimeout("fnShowPicture()", 100);
    }
}
...
window.onresize = function () {
    ...
    if (_MapFullScreenState) {
        ...
        jQuery("#Content").css("height", h + "px");
        jQuery("#EdushiMap").css("height", (h - 32) + "px");
        ...
        jQuery("#Content").css("height", (h - 112) + "px");
        if ((w - 650) < 500) {
            //jQuery("#divBdtList").css("width", "500px");
            //jQuery("#divGonggao").css("width", "500px");
        }
        else {
            //jQuery("#divBdtList").css("width", (w - 650) + "px");
            //jQuery("#divGonggao").css("width", (w - 650) + "px");
        }
        jQuery("#BodyContainer").css("height", (h - 144) + 'px');
        jQuery("#Navigation").css("height", (h - 112) + 'px');
        jQuery("#EdushiMap").css("height", (h - 144) + 'px');
...
function fnLoadPlug() {
    ...
    if (vM.Property.flgShowBaiduMap) {
        vM.LoadPlug('BaiduMap');
    }
    //添加鹰眼视图框
...
function ToggleRefrencePoint() {
    if (!refrenceSystem) {
        setTimeout(InitRefrencePoint, 100);
...
function fnMapEventBind() {
    ...
    vM.attachEvent(AlaMap.KeyWord.EventName.MapZoomChange, function (z) {
        ...
        if (__IsDrawBusLineState) {
            fnShowBusStationIco(_BusLineData, _BusLineType, _BusShowType);
        }
        else {
            fnDrawingBusLine(_BusLineCoordList, _BusTansShowType);
        }
        ...
        if (z > 2) {
            ...
            if (vM.MapState.Map == 0) {
                _WaterBusStationLayer.style.display = 'none';
            }
            return;
        }
        else {
            ...
            if (vM.MapState.Map == 0) {
                if (_WaterBusStationLayer.style.display == 'none') {
                    _WaterBusStationLayer.style.display = 'block';
                }
            }
        }
        ...
        if (vM.MapState.Map == 0) {
            if (vM.Zoom() > 1) {
                _FTLayer.style.display = 'none';
            }
            else {
                if (_FTLayer.style.display == 'none') {
                    _FTLayer.style.display = 'block';
                }
            }
        }
...
//        fnLoadFTData();
    ...
    vM.attachEvent(AlaMap.KeyWord.EventName.Switch3D, function () {
        ...
        if (__IsDrawBusLineState) {
            setTimeout(function () { fnShowBusStationIco(_BusLineData, _BusLineType, _BusShowType); }, 100);
        }
        else {
            setTimeout(function () { fnDrawingBusLine(_BusLineCoordList, _BusTansShowType); }, 100);
        }
        fnDestroyAllDriveTab(); //切换到3D模式时，把右边所有驾车的Tab销毁
        fnChangeTabBodySrc(0);
        if ($('DriveSearch').className == "current") {
            fnSearchChange('MapSearch');
        }
        if (_FTLayer) {
            if (_FTLayer.style.display == 'none') {
                _FTLayer.style.display = 'block';
        ...
        if (_BusTransferLineLayer) {
            if (_BusTransferLineLayer.style.display == 'none') {
                _BusTransferLineLayer.style.display = 'block';
            }
        }
        if (_BusStationLayer) {
            if (_BusStationLayer.style.display == 'none') {
                _BusStationLayer.style.display = 'block';
            }
        }
        if (IsHavePanoramaControl) {//jQuery("div[title='进入全景']", vM.ContentWindow.document)
            if (jQuery("div[title='进入全景']", vM.ContentWindow.document).css("display") == "block") {
                jQuery("div[title='进入全景']", vM.ContentWindow.document).css("display", "none");
            }
        }
        //系统对象
        if (vM.$("AlaMap$Layer$Sign").style.display == 'none') {
            vM.$("AlaMap$Layer$Sign").style.display = 'block';
        }
        if (vM.$("AlaMap$Layer$SpotLabel").style.display == 'none') {
            vM.$("AlaMap$Layer$SpotLabel").style.display = 'block';
        }
        if (vM.$("AlaMap$Layer$Plot").style.display == 'none') {
            vM.$("AlaMap$Layer$Plot").style.display = 'block';
        }
        if (_PopLayer) {
            if (_PopLayer.style.display == 'none') {
                _PopLayer.style.display = 'block';
            }
        }
        if (_seIconLayer) {
            if (_seIconLayer.style.display == 'none') {
                _seIconLayer.style.display = 'block';
            }
        }
        if (_IconLayer) {
            if (_IconLayer.style.display == 'none') {
                _IconLayer.style.display = 'block';
            }
        }
        if (_EyeIconLayer) {
            if (_EyeIconLayer.style.display == 'none') {
                _EyeIconLayer.style.display = 'block';
            }
        }
    });
    ...
    vM.attachEvent(AlaMap.KeyWord.EventName.Switch2D, function () {
        ...
        fnDestroyAllWaterBusTab(); //切换到2D模式时，把右边所有水上巴士的Tab销毁
        fnDestroyAllStationSearchTab(); //切换到2D模式时，把右边所有站点查询的Tab销毁
        fnChangeTabBodySrc(1);
        if ($('WaterBusSearch').className == "current") {
            fnSearchChange('MapSearch');
        }
        if (_FTLayer) {
            if (_FTLayer.style.display == 'block' || _FTLayer.style.display == '') {
                _FTLayer.style.display = 'none';
            }
        }
        if (_WaterBusStationLayer) {
            if (_WaterBusStationLayer.style.display == 'block' || _WaterBusStationLayer.style.display == '') {
                _WaterBusStationLayer.style.display = 'none';
            }
        }
        if (_SubWayIconLayer) {
            if (_SubWayIconLayer.style.display == 'block' || _SubWayIconLayer.style.display == '') {
                _SubWayIconLayer.style.display = 'none';
            }
        }
        if (_BusTransferLineLayer) {
            if (_BusTransferLineLayer.style.display == 'block' || _BusTransferLineLayer.style.display == '') {
                _BusTransferLineLayer.style.display = 'none';
            }
        }
        if (_BusStationLayer) {
            if (_BusStationLayer.style.display == 'block' || _BusStationLayer.style.display == '') {
                _BusStationLayer.style.display = 'none';
            }
        }
        if (!IsHavePanoramaControl) {//jQuery("div[title='进入全景']", vM.ContentWindow.document)
            addPanorama();
            ...
            if (jQuery("div[title='进入全景']", vM.ContentWindow.document).css("display") == "none") {
                jQuery("div[title='进入全景']", vM.ContentWindow.document).css("display", "block");
            }
        }
        //系统对象
        if (vM.$("AlaMap$Layer$Sign").style.display == 'block' || vM.$("AlaMap$Layer$Sign").style.display == '') {
            vM.$("AlaMap$Layer$Sign").style.display = 'none';
        }
        if (vM.$("AlaMap$Layer$SpotLabel").style.display == 'block' || vM.$("AlaMap$Layer$SpotLabel").style.display == '') {
            vM.$("AlaMap$Layer$SpotLabel").style.display = 'none';
        }
        if (vM.$("AlaMap$Layer$Plot").style.display == 'block' || vM.$("AlaMap$Layer$Plot").style.display == '') {
            vM.$("AlaMap$Layer$Plot").style.display = 'none';
        }
        if (_PopLayer) {
            if (_PopLayer.style.display == 'block' || _PopLayer.style.display == '') {
                _PopLayer.style.display = 'none';
            }
        }
        if (_seIconLayer) {
            if (_seIconLayer.style.display == 'block' || _seIconLayer.style.display == '') {
                _seIconLayer.style.display = 'none';
            }
        }
        if (_IconLayer) {
            if (_IconLayer.style.display == 'block' || _IconLayer.style.display == '') {
                _IconLayer.style.display = 'none';
            }
        }
        if (_EyeIconLayer) {
            if (_EyeIconLayer.style.display == 'block' || _EyeIconLayer.style.display == '') {
                _EyeIconLayer.style.display = 'none';
            }
        }
    });
    ...
    vM.attachEvent(AlaMap.KeyWord.EventName.SwitchWX, function () {
        ...
        fnGoogleStat("切换卫星图");
        SwitchMapMethod(false);
        fnDestroyAllWaterBusTab(); //切换到卫星模式时，把右边所有水上巴士的Tab销毁
        fnDestroyAllStationSearchTab(); //切换到2D模式时，把右边所有站点查询的Tab销毁
        fnChangeTabBodySrc(2);
        if ($('WaterBusSearch').className == "current") {
            fnSearchChange('MapSearch');
        }
        if (_FTLayer) {
    ...
    function addPanorama() {
        if (vM["BaiduMap"].BMap == null) {
            setTimeout(function () { addPanorama(); }, 200);
            return;
        }
        else {
            var stCtrl = new vM.ContentWindow.BMap.PanoramaControl(); //构造全景控件
            var mysize = new vM.ContentWindow.BMap.Size(37, 195);
            stCtrl.setOffset(mysize); //设置全景控件的初始位置
            vM["BaiduMap"].BMap.addControl(stCtrl); //添加全景控件
            IsHavePanoramaControl = true;

            pano = vM["BaiduMap"].BMap.getPanorama();
            pano.addEventListener('visible_changed', function () {
                var IsVisible = pano.getVisible();
                if (IsVisible) {
                    if (vM.$("ZoomBarControl").style.display == 'block' || vM.$("ZoomBarControl").style.display == '') {
                        vM.$("ZoomBarControl").style.display = 'none';
                    }
                    if (vM.$("MapSwitch2D").parentNode.parentNode.style.display == 'block' || vM.$("MapSwitch2D").parentNode.parentNode.style.display == '') {
                        vM.$("MapSwitch2D").parentNode.parentNode.style.display = 'none';
                    }
                }
                else {
                    if (vM.$("ZoomBarControl").style.display == 'none') {
                        vM.$("ZoomBarControl").style.display = 'block';
                    }
                    if (vM.$("MapSwitch2D").parentNode.parentNode.style.display == 'none') {
                        vM.$("MapSwitch2D").parentNode.parentNode.style.display = 'block';
                    }
                }
            });
        }
    ...
    vM.attachEvent(AlaMap.KeyWord.EventName.SignClick, function (sign) {
        switch (sign.Group) {
            case 'bus':
            case 'bus3d':
                fnShowBusStation(sign.ID, sign.Name, sign.Cx, sign.Cy);
                break;
            case 'vir':
                if (sign.Url.indexOf('http://') > -1) {
...
function fnLoadHotKey() {
    ...
    ENetwork.DownloadScript(url, function () {
        ...
        if (arrHotkey.length > 0) {
            ...
            //jQuery("#tcjy").html("<a href=\"http://hz.edushi.com/jing/\" target=\"_blank\"><img src=\"images/1b1j.png\" /></a>");
            //<a style=\"padding-top:10px;\" href=\"http://www.quanquanme.com/flash/game.jsp\" target=\"_blank\"><img src=\"images/tcjy.gif\" /></a>
        }
    });
};
...
function fnGoogleStat(sTrackName) {
    try {
        pageTracker._trackEvent('Map7.6', 'Mater_UI', sTrackName);
    } catch (e) {
    }
}
//添加页卡
...
function fnCommonGoToClick(QuickLink) {
    ...
    if (QuickLink.Flag == 0) {
        MoveTo(QuickLink.x, QuickLink.y);
    }
...
function fnSearchChange(type) {
    ...
    if (type == 'DriveSearch') {
        if (vM.MapState.Map == 0) {
//            if (window.confirm("三维模式下暂时不支持驾车功能，如需使用驾车功能，请点击“确定”按钮，地图会自动切换到二维模式")) {
                if (vM.MapState.Succeed) {
                    vM.MapSwitch.Switch2D();
                    //vM.SwitchMap(1);
                }
//            }
//            else {
...
//            }
    ...
    //水上巴士   20111020
    if (type == 'WaterBusSearch') {
        if (vM.MapState.Map != 0) {
//            if (window.confirm("二维模式下暂时不支持水上巴士功能，如需使用水上巴士功能，请点击“确定”按钮，地图会自动切换到三维模式")) {
                if (vM.MapState.Succeed) {
                    vM.MapSwitch.Switch3D();
                    //vM.SwitchMap(0);
                }
//            }
//            else {
...
//            }
        ...
        if (flag_waterbus) {
            ...
            $('BusSearch').className = '';
            $('DriveSearch').className = '';
            $('WaterBusSearch').className = 'current';
            $('divMapSearch').style.display = 'none';
            $('divBusSearch').style.display = 'none';
            $('divDriveSearch').style.display = 'none';
            $('divWaterBusSearch').style.display = 'block';
            fnWaterBus();
        }
    }
...
//站点互换---水上公交
function KeyWordsChange_Water() {
    if ($('txtWaterBusStart').value.trim() != '' && $('txtWaterBusEnd').value.trim() != '' && $('txtWaterBusStart').value.trim() != '起点' && $('txtWaterBusEnd').value.trim() != '终点') {
        var txtstart = $('txtWaterBusStart').value.trim();
        var txtend = $('txtWaterBusEnd').value.trim();

        $('txtWaterBusStart').value = txtend;
        $('txtWaterBusEnd').value = txtstart;
    }
}
//站点互换---驾车
...
function fnLoadBdtList(IsTrue) {
    if (GlobalConfig.IsBDT == '1' && IsTrue) {
        //新数据中心
        var sUrl = 'Data/getData.aspx?action=MapIndexMarquu';
        ENetwork.DownloadScript(sUrl, function () {
            var arrayHtml = [];
            if (typeof (_rightTabData) != 'undefined' && _rightTabData.bdt.length > 0) {
                //向上滚动
                for (var i = 0; i < _rightTabData.bdt.length; i++) {
                    arrayHtml.push('<p style=\"height:20px;white-space:nowrap;\"><a style=\"font-weight:bold;filter:alpha(opacity=100);opacity:1;\" href="' + _rightTabData.bdt[i].TitleUrl + '" target="_blank">' + decodeURI(_rightTabData.bdt[i].Title) + '----' + _rightTabData.bdt[i].Time + '</a></p>');

                }
                jQuery('#divBdtList').append(arrayHtml.join(''));
                var marquee = new Marquee('divBdtList', 0, 1, null, null, 20, 5000, 100);
...
//加载赞助商广告
function fnAddADTab() {
    var tab = new TabControl.Tab(document, 'ads', '推荐阅读', '/Fundation/News.aspx?rnd=' + $Rnd(), true, true, 80);
    fnAddTab(tab);
};
...
function fnOpenMergeSearch(key, lst_id, domain, cid, vid, tid, card, vip, type, tags, adtype) {
    var tab = new TabControl.Tab(document, 'MergeSearch', key, '/Fundation/MergeSearch.aspx', true, true, 80);
    fnAddTab(tab);
    return tab;
}
...
function fnEShopAndCompanyJoin(oid, oname, x, y) {
    ...
    var url = 'http://' + GlobalConfig.Domain + '/dian/join.html';
...
function fnBusSearchByStation() {
    if (vM.MapState.Map != 0) {
        //if (window.confirm("二维模式下暂时不支持站点查询功能，如需使用站点查询功能，请点击“确定”按钮，地图会自动切换到三维模式")) {
        if (vM.MapState.Succeed) {
            vM.MapSwitch.Switch3D();
            //vM.SwitchMap(0);
        }
...
function fnShowSubway() {
    var tab = new TabControl.Tab(document, 'Subway', '地铁列表', '/Fundation/subWayStationList.html', true, true, 80);
    fnAddTab(tab);
}
...
function fnKeyInputChanged(input, keyCode, x, y, w, type, maptype) {
    if (input.value.length > 0 && input.value != __EdushiSuggest.LastKeyword && keyCode != 38 && keyCode != 40 && keyCode != 13) {
        if (vM.MapState.Map == 0) {
            ...
            else if (type == 'waterbus') {
                var url = GlobalConfig.DataCetnerSearchDataUrl + "SearchTankerStationSuggest/" + GlobalConfig.CityCode + "/" + GlobalConfig.Language + "/Format/Json/Search?q=" + encodeURIComponent(input.value);
            ...
            BMap_Map = vM["BaiduMap"].BMap;
            if (!ac) {
                ac = new vM.ContentWindow.BMap.Autocomplete(    //建立一个自动完成的对象
                    {
                    "input": vM.ContentWindow.document.getElementById("hidKey"), //jQuery("#hidKey", window.frames[0].document),
                    "location": BMap_Map,//GlobalConfig.CityName,
                    onSearchComplete: function (results) {
                        SearchCompleteFun(results);
                    }
                });
            }
            if (type == 'drive') {//驾车情况
                if ($('navReturnIcon').style.display == "none") {//有途径站点时
                    if (input.id == "txtDriveStart") {//起点输入框
                        tempX = x;
                        tempY = y;
                        tempW = w;
                    }
                    if (input.id.indexOf("txt_via_") > -1) {
                        var num = parseInt(input.id.replace("txt_via_", ""));
                        switch (num) {
                            case 1:
                                tempX = x + 150 + 20; //加上起点输入框缩短后的长度（150）及互换按钮的宽度（20）
                                break;
                            case 2:
                            case 3:
                            case 4:
                            case 5:
                                tempX = x + 150 + 20 + 14; //加上起点输入框缩短后的长度（150）及互换按钮的宽度（20）和序号的宽度
                                break;
                        }
                        tempY = y + (num - 1) * 39;
                        tempW = w;
                    }
                    if (input.id == "txtDriveEnd") {//终点输入框
                        tempX = x + 65; //加上终点输入框所缩短的宽度（缩短前为215，缩短后150）
                        tempY = y;
                        tempW = w;
                    }
                }
                else {//无途径站点时（即正常两个输入框）
                    tempX = x;
                    tempY = y;
                    tempW = w;
                }
...
function SearchCompleteFun(results) {
    var total = results.getNumPois();
    if (total > 0) {
        var Data = "";
        Data += "[";
        jQuery(".tangram-suggestion-main", vM.ContentWindow.document).css("display", "none");
        for (var i = 0; i < total; i++) {
            if (i != 0) {
                Data += ",";
            }
            var poi = results.getPoi(i);
            Data += "{\"Title\":\"" + poi.business + "\",\"Value\":\"" + poi.business + "\",\"Type\":\"1\"}";
            //alert(poi.province + "\n" + poi.city + "\n" + poi.business);
        }
        Data += "]";
        __EdushiSuggest.Show(eval(Data), tempX, tempY, tempW);
    }
    else {
        __EdushiSuggest.Hide();
    }
...
function fnCheckUserLoginInit() {
    if ($('liLoginInfo').innerHTML == '') {
        ...
        if (cookieNickNameCookie != null) {
            $('liLoginInfo').innerHTML = '<i></i><a href="' + GlobalConfig.UserCenterUrl + 'Index.aspx?city=' + GlobalConfig.CityCode + '" target="_blank" class="login">' + unescape(cookieNickNameCookie) + '</a><a href="javascript:fnLoginOut();">退出</a>';
        }
        else {
            $('liLoginInfo').innerHTML = '<i></i><a style="cursor:pointer;" onclick="top.location.href=\'http://my.edushi.com/Login.aspx?BackUrl=\'+encodeURIComponent(window.location.href);" class="login">登录</a><a href="' + GlobalConfig.UserCenterUrl + 'Register.aspx" target="_blank" title="注册">注册</a>';
        }
    }
}
...
function fnDrawingBusLine(busLineCoordList, t) {
    _BusLineCoordList = busLineCoordList;
    _BusTransferLineLayer.innerHTML = '';
    _BusStationLayer.innerHTML = '';
    var _tempfn = 'fnShowBusStation';
    if (t == 'water') {
        _BusTansShowType = t;
        _tempfn = 'fnShowWaterBusStation';
    }
...
//加载完线路显示公交站
function fnShowBusStationIco(arrBusData, type, t) {
    _BusLineType = type;
    ...
    if (_BusLineData != null) {
        if (_BusLineType == 1) {
            ...
            for (i = 0; i < l; i++) {
                ...
                if (t == 'water') {
                    _BusShowType = t;
                    nd.innerHTML = string.Format("<span style=\"cursor:pointer; height:26px; float:left; font-size:12px; color:#000;white-space:nowrap;{0}\"><span style=\"cursor:pointer; width:24px; height:24px; float:left; background:url(/Images/gongjiao.gif) no-repeat; text-align:center; padding-top:2px; color:white;\">{1}</span><span style=\"cursor:pointer; height:19px;line-height:19px; background:#fff; padding-right:4px; float:left;{2};\" onclick=\"parent.fnShowWaterBusStation({3},'{4}',{5},{6})\">{7}</span></span>", 'width:' + (vlen + 40) + 'px', (i + 1), 'width:' + (vlen + 10) + 'px', oBusDown.StationID, oBusDown.StationName, oBusDown.PositionX, oBusDown.PositionY, oBusDown.StationName);
                }
                else {
                    _BusShowType = "default";
...
//销毁所有站点查询的Tab
function fnDestroyAllStationSearchTab() {
    if (_TabControl) {
        var tabs = [];
        for (var i = 0; i < _TabControl.TabList.length; i++) {
            if (_TabControl.TabList[i].TabBody.src.indexOf("/Fundation/BusStationSearch.html") > -1) {
                tabs.push(_TabControl.TabList[i]);
            }
        }
        for (var i = 0; i < tabs.length; i++) {
            _TabControl.DestroyTab(tabs[i]);
        }
    }
}
...
//销毁所有驾车的Tab
function fnDestroyAllDriveTab() {
    if (_TabControl) {
        var tabs = [];
        for (var i = 0; i < _TabControl.TabList.length; i++) {
            if (_TabControl.TabList[i].TabBody.src.indexOf("/Fundation/Drive.html") > -1) {
                tabs.push(_TabControl.TabList[i]);
            }
        }
        for (var i = 0; i < tabs.length; i++) {
...
function fnDriveSearch() {
    __EdushiSuggest.Hide();
    fnGoogleStat('驾车搜索');
    var sStartStation = $('txtDriveStart').value.trim();
    if (sStartStation.length < 1) {
        $('txtDriveStart').focus();
        return;
    }
    ...
    if ($('navReturnIcon').style.display == "none") {
        var inputs = jQuery(".myclass");
        if (inputs.length > 0) {
            //alert("dddd");
            for (var i = 0; i < inputs.length; i++) {
                if ($('txt_via_' + (i + 1)).value.trim() != "") {
                    sVia += $('txt_via_' + (i + 1)).value.trim() + ",";
                }
            }
            if (sVia != "") {
                sVia = sVia.substr(0, sVia.length - 1);
            }
        }
    }

    var sEndStation = $('txtDriveEnd').value.trim();
    if (sEndStation.length < 1) {
        $('txtDriveEnd').focus();
        return;
    }
    ...
    fnAddDriveTransferSearchTab(sStartStation, sEndStation, sVia);
}

function fnAddDriveTransferSearchTab(sStartStation, sEndStation, sVia) {
    fnDestroyAllDriveTab(); //添加驾车Tab时，先把右边所有驾车的Tab销毁

    var tab = new TabControl.Tab(document, 'Drive', sStartStation + '→' + sEndStation, '/Fundation/Drive.html?s=' + escape(sStartStation) + '&e=' + escape(sEndStation) + '&via=' + escape(sVia), true, true, 80);
    fnAddTab(tab);
}
    ...
    if (id > 0) {
        if (!_EntityPopControl) {
            ENetwork.DownloadScript('/Controls/EntityPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _EntityPopControl.ID = vM.AppendEntity(_EntityPopControl.Body, _PopLayer, false, 0, 0, 374, 330, 72, 330, false);
                } else {
                    _EntityPopControl.ID = vM.AppendEntity(_EntityPopControl.Body, _PopLayer, false, x, y, 374, 330, 72, 330, false);
                }
                ...
                _EntityPopControl.onLoadComplete = function () {
                    if (x == 0 && y == 0) {
                        ...
                        MoveTo(x * 1 + vM.GradeWin2EGIS(150), y * 1 + vM.GradeWin2EGIS(-96), true);
                    }
                };
    ...
    if (id > 0) {
        if (!_CompanyPopControl) {
            ENetwork.DownloadScript('/Controls/CompanyPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _CompanyPopControl.ID = vM.AppendEntity(_CompanyPopControl.Body, _PopLayer, false, 0, 0, 374, 330, 102, 313, false);
                }
                else {
                    _CompanyPopControl.ID = vM.AppendEntity(_CompanyPopControl.Body, _PopLayer, false, x, y, 374, 330, 102, 313, false);
                }
    ...
    if (id > 0) {
        if (!_EShopPopControl) {
            ENetwork.DownloadScript('/Controls/EShopPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _EShopPopControl.ID = vM.AppendEntity(_EShopPopControl.Body, _PopLayer, false, 0, 0, 374, 400, 62, 362, false);
                }
                else {
                    _EShopPopControl.ID = vM.AppendEntity(_EShopPopControl.Body, _PopLayer, false, x, y, 374, 400, 62, 362, false);
                }
...
function fnShowBusStation(id, name, x, y) {
    if (!_BusStationControl) {
        ENetwork.DownloadScript('/Controls/BusStationControl.js', function () {
            ...
            _BusStationControl.ID = vM.AppendEntity(_BusStationControl.Body, _PopLayer, false, x, y, 354, 340, 64, 328, false);
            _BusStationControl.Width = 354;
...
function fnShowBusLinePop(index, x, y, type) {
    if (!_BusLinePopControl) {
        ENetwork.DownloadScript('/Controls/BusLinePopControl.js', function () {
            ...
            _BusLinePopControl.ID = vM.AppendEntity(_BusLinePopControl.Body, _PopLayer, false, x, y, 354, 291, 0, 300, false);
            _BusLinePopControl.Width = 354;
            _BusLinePopControl.Height = 291;
...
function fnGetPositionBySID() {
    var sid = fnRequest('sid');
    if (parseInt(fnRequest('ts')) == 1) {
        var sname = unescape(fnRequest('sname'));
    }
    else {
    ...
    if (x != '' && y != '' && sname != '' && sid != '') {
        MoveTo(x * 1, y * 1);
        if (fnRequest('type') == 'water') {
            fnShowWaterBusStation(sid, sname, x * 1, y * 1);
        }
        else {
...
function fnUrlParse() {
    ...
    if (fnRequest('type') == 'water' && iBusId != '') {
        fnOnWaterBusClick(iBusId, unescape(fnRequest('bname')), '');
    }
    else {
        if (iBusId != '') {
...
function fnShowMark() {
    ...
    if (x != '' && y != '' && title != '') {
        fnShowMarkPop(x, y, unescape(title), unescape(content));
    }
}
...
function fnShowScaleResult(_dis, id) {
    if (_dis > 1000) {
        _dis = _dis / 1000; _dis = _dis * 100; _dis = Math.round(_dis); _dis = _dis / 100; _dis = _dis + '千米';
    }
    else {
        _dis = _dis * 100; _dis = Math.round(_dis); _dis = _dis / 100; _dis = _dis + '米';
    }
...
function fnPrint() {
    ...
    window.open('/Print.aspx?x=' + iMapX + '&y=' + iMapY + '&z=' + iZoom + '&w=' + iWidth + '&h=' + iHeight, 'print');
}
...
function sendTransferMsg(content) {
    ...
    var url = "/SendMsg.aspx?n=" + _phone + "&t=" + escape(_content) + "&v=" + _VerifyCode;
    ENetwork.DownloadScript(url, function () {
        //返回参数
        if (typeof _returnstatus != 'undefined') {
            if (_returnstatus == '1') {
                ...
                $('divResultSendMsg').style.display = 'block';
                $('divResultSendMsg').innerHTML = "验证码不正确!";
...
jQuery(function ($) {
    $("#moreid").hover(function () {
        $(".xlmore").slideDown(300);
    }, function () {
        $(".xlmore").slideUp(300);
    });

    $("#markdiv").click(function () {
        if ($("#PromptInfo").css("display") == 'none') {
            $("#PromptInfo").fadeIn(200, function () {
                $("#PromptInfo").fadeOut(2000);
            });
            $("#PromptInfo").html("您已开启标注功能！");
    ...
    $("#labeldiv").click(function () {
        if ($("#PromptInfo").css("display") == 'none') {
            ...
            if (vM.Property.VisibleSpotLabels['hotarea']) {
                $("#PromptInfo").html("您已显示全部标签！");
                $("#labeldiv").html("关闭标签");
            }
            else {
                $("#PromptInfo").html("您已关闭全部标签！");
                $("#labeldiv").html("显示标签");
            }
            $("#PromptInfo").fadeIn(200, function () {
                $("#PromptInfo").fadeOut(2000);
    ...
    $("#aFullScrenn").click(function () {
        if ($("#PromptInfo").css("display") == 'none') {
            fnFullScreen(this);
            if (_MapFullScreenState) {
                $("#PromptInfo").html("您已开启全屏功能！");
            }
            else {
                $("#PromptInfo").html("您已关闭全屏功能！");
            }
            $("#PromptInfo").fadeIn(200, function () {
                $("#PromptInfo").fadeOut(2000);
            });
    ...
    $("#AdHide").click(function () {
        if ($("#PromptInfo").css("display") == 'none') {
            ...
            if (vM.Property.VisiblePlots['ad']) {
                $("#PromptInfo").html("您已选择开启广告！");
                $("#AdHide").html("屏蔽广告");
            }
            else {
                $("#PromptInfo").html("您已选择屏蔽广告！");
                $("#AdHide").html("显示广告");
            }
            $("#PromptInfo").fadeIn(200, function () {
                $("#PromptInfo").fadeOut(2000);
    ...
    $("#PoiSettled").click(function () {
        if ($("#PromptInfo").css("display") == 'none') {
            $("#PromptInfo").fadeIn(200, function () {
                $("#PromptInfo").fadeOut(2000);
            });
            $("#PromptInfo").html("您已开启入驻功能！");
        }
        fnSelectWikiPoint();
    });

    $("#ToolsList").hover(function () {
        $("#ToolsListddl").show();
    }, function () {
        $("#ToolsListddl").hide();
    });

    $("#CommPositionDiv").hover(function () {
        //常用位置
...
/*===================================水上巴士====================================*/
//销毁所有水上巴士的Tab
function fnDestroyAllWaterBusTab() {
    if (_TabControl) {
        var tabs = [];
        for (var i = 0; i < _TabControl.TabList.length; i++) {
            var src = _TabControl.TabList[i].TabBody.src;
            if (src.indexOf("/Fundation/WaterBus.html") > -1 || src.indexOf("/Fundation/WaterBusNoSearch.html") > -1 || src.indexOf("/Fundation/WaterBusStationSearch.html") > -1 || src.indexOf("/Fundation/WaterBusTransferSearch.html") > -1) {
                tabs.push(_TabControl.TabList[i]);
            }
        }
        //alert(tabs.length + "sss");
        for (var i = 0; i < tabs.length; i++) {
            //alert(tabs[i].TabBody.src);
            _TabControl.DestroyTab(tabs[i]);
        }
    }
}

function fnWaterBusSearchByLine() {
    tab = new TabControl.Tab(document, 'waterbuslinesearch', '水上巴士线路搜索', GlobalConfig.WebRootPath + 'Fundation/WaterBusNoSearch.html', true, true, 80);
    fnAddTab(tab);
}

function fnWaterBusSearchByStation() {
    tab = new TabControl.Tab(document, 'waterbusstationsearch', '水上巴士站点搜索', GlobalConfig.WebRootPath + 'Fundation/WaterBusStationSearch.html', true, true, 80);
    fnAddTab(tab);
}

function fnWaterBus() {
    tab = new TabControl.Tab(document, 'waterbus', '水上巴士大全', GlobalConfig.WebRootPath + 'Fundation/WaterBus.html', true, true, 80);
    fnAddTab(tab);
}

//水上巴士
function fnOnWaterBusClick(busid, busname, stationName) {
    tab = new TabControl.Tab(document, 'search', busname, '/Fundation/WaterBusNoSearch.html?stationName=' + escape(stationName) + '&action=1&key=' + busid, true, true, 80);
    fnAddTab(tab);
}

//公交搜索
function fnWaterBusSearch() {
    __EdushiSuggest.Hide();

    fnGoogleStat('水上巴士搜索');
    var sStartStation = $('txtWaterBusStart').value.trim();
    if (sStartStation.length < 1) {
        $('txtWaterBusStart').focus();
        return;
    }
    var sEndStation = $('txtWaterBusEnd').value.trim();
    if (sEndStation.length < 1) {
        $('txtWaterBusEnd').focus();
        return;
    }
    if (sStartStation == '起点' || sEndStation == '终点') {
        $('txtWaterBusStart').focus();
    ...
    fnWaterBusTransferSearchTab(sStartStation, sEndStation);
}

function fnWaterBusTransferSearchTab(sStartStation, sEndStation) {
    var tab = new TabControl.Tab(document, 'busTransfer', sStartStation + '→' + sEndStation, '/Fundation/WaterBusTransferSearch.html?s=' + escape(sStartStation) + '&e=' + escape(sEndStation) + '&action=0', true, true, 80);
    fnAddTab(tab);
}

var _WaterBusStationControl;
//显示公交泡泡
function fnShowWaterBusStation(id, name, x, y, type, bid) {
    if (!_WaterBusStationControl) {
        ENetwork.DownloadScript('/Controls/WaterBusStationControl.js', function () {
            _WaterBusStationControl = new WaterBusStationControl(vM.Body.document);
            _WaterBusStationControl.ID = vM.AppendEntity(_WaterBusStationControl.Body, _PopLayer, false, x, y, 354, 275, 64, 288, false);
            _WaterBusStationControl.Width = 354;
            _WaterBusStationControl.Height = 275;

            _WaterBusStationControl.ResumeLayout();
            _WaterBusStationControl.onLoadComplete = function () {
                _WaterBusStationControl.ShowBusStation(id, name, x, y, type, bid);
                MoveTo(x * 1 + vM.GradeWin2EGIS(66), y * 1 + vM.GradeWin2EGIS(-96), true);
            };
            _WaterBusStationControl.onBusClick = function (busid, busname, stationName) {
                fnOnWaterBusClick(busid, busname, stationName);
            };
            _WaterBusStationControl.onMergeSearch = function (key) {

            };
            _WaterBusStationControl.onCavil = function (id, name, x, y) {
                fnReport(x, y, 3, id, name);
            };
        });
    }
    else {
        vM.MoveEntity(_WaterBusStationControl.ID, x, y);
        MoveTo(x * 1 + vM.GradeWin2EGIS(66), y * 1 + vM.GradeWin2EGIS(-96), true);
        _WaterBusStationControl.ShowBusStation(id, name, x, y, type, bid);
    }
}

function fnLoadWaterBusStation() {
    var url = GlobalConfig.DataCetnerSearchDataUrl + 'SearchTankerByRangeStation/' + GlobalConfig.CityCode + '/' + GlobalConfig.Language + '/Format/Json/Search?x=&y=&range=';
    ENetwork.DownloadScript(url, function () {
        if (typeof _Station != 'undefined' && _Station.length > 0) {
            _WaterBusStationLayer.innerHTML = '';
            for (var i = 0; i < _Station.length; i++) {
                fnAppendWaterBusIcon(_Station[i].StationID, _Station[i].StationName, _Station[i].PositionX, _Station[i].PositionY);
            }
        }
    });
}

function fnAppendWaterBusIcon(id, name, x, y) {
    var iconHTML = '';
    iconHTML = '<div style ="width:20px;height:44px; cursor:pointer;" title="' + name + '" onclick="parent.fnShowWaterBusStation(' + id + ',\'' + name + '\',' + x + ',' + y + ');"><img src="../images/ab.gif"/></div>';
    var p = vM.$C('div');
    p.innerHTML = iconHTML;
    vM.AppendEntity(p, _WaterBusStationLayer, false, x, y, 20, 44, 10, 44, false);
}
/*===============================================================================*/

/*====地铁相关===========================================================================*/
//显示地铁泡泡
function fnShowSubWayPop(index) {
    if (typeof _SubwayData[index] == 'undefined') {
        return;
    }

    var x = _SubwayData[index].X;
    var y = _SubwayData[index].Y;
    var key = _SubwayData[index].Name;
    if (!_SubWayControl) {
        ENetwork.DownloadScript('/Controls/SubWayControl.js', function () {
            _SubWayControl = new SubWayControl(vM.Body.document);
            _SubWayControl.ID = vM.AppendEntity(_SubWayControl.Body, _PopLayer, false, x, y, 354, 220, 0, 0, false);
            _SubWayControl.Width = 354;
            _SubWayControl.Height = 220;

            _SubWayControl.ResumeLayout();
            _SubWayControl.onLoadComplete = function () {
                _SubWayControl.ShowBusStation(_SubwayData[index]);
            };
            _SubWayControl.onMergeSearch = function (key) {
                if (_SubWayControl.Tab != null) {
                    _SubWayControl.Tab.destroy();
                }
                _SubWayControl.Tab = fnOpenMergeSearch(key, 0, '', 0, 0, 0, 0, 0, '', '', 1);
                _SubWayControl.Tab.onDestroy = function () {
                    _SubWayControl.Tab = null;
                };
            };
            _SubWayControl.onBusClick = function (busid, busname, stationName) {
                fnOnBusClick(busid, busname, stationName);
            };
        });
    }
    else {
        vM.MoveEntity(_SubWayControl.ID, x, y);
        _SubWayControl.ShowBusStation(_SubwayData[index]);
    }
}
var _SubwayData = '';
function ShowSubwayExport() {
    var popHTML = ' <div style="float:left ;cursor:pointer;" title="{$title}" onclick="parent.fnShowSubWayPop({$data});"><div style="text-align:center; margin-left:1px; border:solid 1px #000; padding:2px 0;background:#fff;"><img src="../images/xh_1.jpg" /></div><div style="text-align:center; margin-left:1px;border:solid 1px #000; border-top:none; font-family:Arial, Helvetica, sans-serif; font-size:10px; font-family:Arial, Helvetica, sans-serif; padding:1px;background:#fff;">{$list}</div><div style="text-align:center; margin-left:1px; margin-top:3px;"><img src="../images/xh_3.png" /></div></div>';
    var arrCoords = vM.GetMapPosCurrentRegion();
    var ExportUrl = GlobalConfig.DataCetnerMapDataUrl + "Map7.0/SubWayInfo.aspx?citycode=" + GlobalConfig.CityCode + "&l=" + GlobalConfig.Language + "&req=2&x1=" + parseInt(arrCoords[0].X) + "&y1=" + parseInt(arrCoords[0].Y) + "&x2=" + parseInt(arrCoords[2].X) + "&y2=" + parseInt(arrCoords[2].Y) + "&z=" + vM.Zoom();
    ENetwork.DownloadScript(ExportUrl, function () {
        if (typeof SubWayIcon != 'undefined' && SubWayIcon.length > 0) {
            _SubwayData = '';
            _SubWayIconLayer.innerHTML = '';
            _SubwayData = SubWayIcon;
            for (var i = 0; i < SubWayIcon.length; i++) {
                var ob = SubWayIcon[i];
                var oblist = SubWayIcon[i].List;
                var listlen = SubWayIcon[i].List.length;
                var r = '';
                var p = vM.$C('div');
                p.zIndex = 10000;
                p.id = "subwayicon" + $Rnd();
                switch (listlen) {
                    case 1:
                        var _temp = popHTML;
                        r = string.Format('<span  style="background:{0};  color:#fff;  padding:0 11px; ">{1}</span>', oblist[0].Color.toString() == '' ? '#cc0000' : oblist[0].Color, oblist[0].Asid);
                        var temptitle = ob.Name + "，经过" + oblist[0].Asid + "号线";
                        _temp = _temp.replace("{$list}", r).replace("{$title}", temptitle).replace("{$data}", i);
                        p.innerHTML = _temp;
                        vM.AppendEntity(p, _SubWayIconLayer, false, ob.X, ob.Y, 35, 50, 15, 50, false);
                        break;
                    case 2:
                        var _temp = popHTML;
                        for (var k = 0; k < listlen; k++) {
                            r += string.Format('<span style="background:{0};  color:#fff;  padding:0 3px; margin-right:1px; ">{1}</span>', oblist[k].Color.toString() == '' ? '#cc0000' : oblist[k].Color, oblist[k].Asid);
                        }
                        var temptitle = ob.Name + "，经过" + oblist[0].Asid + "、" + oblist[1].Asid + "号线";
                        _temp = _temp.replace("{$list}", r).replace("{$title}", temptitle).replace("{$data}", i);
                        p.innerHTML = _temp;
                        vM.AppendEntity(p, _SubWayIconLayer, false, ob.X, ob.Y, 35, 50, 15, 50, false);
                        break;
                    case 3:
                        var _temp = popHTML;
                        for (var k = 0; k < listlen; k++) {
                            r += string.Format('<span style="background:{0};  color:#fff;  padding:0 1px; margin-right:1px; ">{1}</span>', oblist[k].Color.toString() == '' ? '#cc0000' : oblist[k].Color, oblist[k].Asid);
                        }
                        var temptitle = ob.Name + "，经过" + oblist[0].Asid + "、" + oblist[1].Asid + "、" + oblist[2].Asid + "号线";
                        _temp = _temp.replace("{$list}", r).replace("{$title}", temptitle).replace("{$data}", i);
                        p.innerHTML = _temp;
                        vM.AppendEntity(p, _SubWayIconLayer, false, ob.X, ob.Y, 35, 50, 16, 50, false);
                        break;
                    case 4:
                        var _temp = popHTML;
                        for (var k = 0; k < listlen; k++) {
                        ...
                        var temptitle = ob.Name + "，经过" + oblist[0].Asid + "、" + oblist[1].Asid + "、" + oblist[2].Asid + "、" + oblist[3].Asid + "号线";
                        _temp = _temp.replace("{$list}", r).replace("{$title}", temptitle).replace("{$data}", i);
                        p.innerHTML = _temp;
                        vM.AppendEntity(p, _SubWayIconLayer, false, ob.X, ob.Y, 35, 50, 25, 50, false);
                        break;
                }
            }
        }
...
var FTHouseData = [];
function fnLoadFTData() {
    if (vM.MapState.Map == 0) {
        if (vM.Zoom() > 1) {
            return;
        }
        var XY = vM.GetMapPosCurrentRegion();
        jQuery.getJSON("http://aip.fangtoo.com/edushiMapData.aspx?dt=xml&way=flags&citydomain=hangzhou&xt=" + XY[0].X + "&yt=" + XY[0].Y + "&xb=" + XY[2].X + "&yb=" + XY[2].Y + "&lv=1&hids=0&cache=true&egisjsondataid=7&jsonclassname=EGIS.JSONHttpRequest&callback=?", function (json) {
            if (json.length > 0) {
                FTHouseData = json;
                _FTLayer.innerHTML = "";
                for (var i = 0; i < json.length; i++) {
                    if (json[i].Group == "FT_217" && parseInt(json[i].SSP) > 0) {
//                        fnAppendFTData(i);
                    }
                }
            }
...
function fnAppendFTData(index) {
    var p = vM.$C('div');
    p.innerHTML = string.Format('<div id="FT_SpotLabel_{0}" onclick="parent.fnShowFTPop({3});" style="cursor:pointer;width:300px; height:24px;white-space: nowrap;"><div style="font-size: 12px; color:  White;line-height:19px;border: 1px solid white; padding:0px 3px; background-color: #FF6400;float:left">{1}<span style="padding-left:5px;font-size:12px;font-weight:bold;height:12px;color:#FFFF33;">挂￥{2}</span></div><div style="position: absolute; width: 5px; height: 5px; overflow: hidden; background: url(../Images/FcMapIco2.gif) no-repeat left top;top: 20px; left: 0;"></div></div>', FTHouseData[index].ID, FTHouseData[index].Name, FTHouseData[index].SSP, index);
    vM.AppendEntity(p, _FTLayer, false, FTHouseData[index].Cx, FTHouseData[index].Cy, 0, 0, 0, 0, false);   
}

var _FTPopControl;
function fnShowFTPop(index) {
    if (!_FTPopControl) {
        ENetwork.DownloadScript('/Controls/FTPopControl.js', function () {
            _FTPopControl = new FTPopControl(vM.Body.document);
            _FTPopControl.ID = vM.AppendEntity(_FTPopControl.Body, _PopLayer, false, FTHouseData[index].Cx, FTHouseData[index].Cy, 374, 206, 98, 206, false);
            _FTPopControl.onLoadComplete = function () {
                _FTPopControl.ShowPop(index);
                //MoveTo(FTHouseData[index].Cx * 1 + vM.GradeWin2EGIS(150), FTHouseData[index].Cy * 1 + vM.GradeWin2EGIS(-96), true);
            };
        });
    }
    else {
        vM.MoveEntity(_FTPopControl.ID, FTHouseData[index].Cx * 1, FTHouseData[index].Cy * 1);
        //MoveTo(FTHouseData[index].Cx * 1 + vM.GradeWin2EGIS(150), FTHouseData[index].Cy * 1 + vM.GradeWin2EGIS(-96), true);
        _FTPopControl.ShowPop(index);
    }
}
