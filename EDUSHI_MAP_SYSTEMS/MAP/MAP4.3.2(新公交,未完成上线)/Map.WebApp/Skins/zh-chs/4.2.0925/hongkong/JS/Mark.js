...
ClassMark.prototype.Load = function(){
	...
	$('divMyMark').innerHTML='<a class="line" href="javascript:;" onclick="if(confirm(\'确认清除所有标记\')){_Mark.Clear()}">清除所有标记</a> ';
	for(i=0;i<aMark.length-1;i++){
        var aryTemp = aMark[i].split('^');
        r+='<a href="javascript:;" onclick="_Mark.Show('+i+','+aryTemp[0]+','+aryTemp[1]+',\''+aryTemp[3]+'\',\''+aryTemp[2]+'\');" >'+aryTemp[3]+'</a>';
	}
	$('divMyMark').innerHTML += r;
}
//保存标记
...
//    }
    ...
    $('divMyMark').innerHTML = '<a href="javascript:;" onclick="_Mark.Show('+i+','+x+','+y+',\''+vM.$('txtMarkerTitle').value+'\',\''+vM.$('txtMarkerContent').value+'\');">' + vM.$('txtMarkerTitle').value + '</a> ' + $('divMyMark').innerHTML;
    
    this.RemoveMarkPop('AddMarkerPop');
    
    //闪烁
    fnFlashFont('txtMyMark', 'white', 'red');
}
