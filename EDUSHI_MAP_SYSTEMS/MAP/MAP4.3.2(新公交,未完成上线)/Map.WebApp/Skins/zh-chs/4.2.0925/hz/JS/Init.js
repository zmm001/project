...
function fnLoadInit(){
    ...
    _BusLineLayer = vM.NewMapLayer('BusLine', 260);
    ...
    vM.onMapZoomChange = function(z){
      ...
      cSearchResultControl.onDrawingBusLine(_BusLineCoordList);
    };
        ...
        cEntityPopControl.onLoadComplete  = function()
        {
            ...
            vM.onContextMenuClick = function(key,wx,wy,mx,my,spot){
                switch (key){
                    ...
                    case 'errors':
                        if (spot){
                            fnShowDebugControl(vM.PointerX(),vM.PointerY(), spot.ID, spot.Title, 2);
                        }
                        else{
...
function fnShowSearchControl(){
    if (!cSearchControl){
        ...
        cSearchControl.onYellowPageSearch = function(sKeyword){
            window.open(GlobalConfig._LocalUrl + 'LocalSearch/?' + escape(sKeyword), '_blank', '');
        };
        
        cSearchControl.onTabIndexChanged = function(id){
            __EdushiSuggest.Hide();
        };
        cSearchControl.onInputChanged = function(input, keyCode, x, y, w, type){
            if (input.value.length > 0 && input.value != __EdushiSuggest.LastKeyword && keyCode != 38 && keyCode != 40 && keyCode != 13)
            {
                if (type == 'area')
                {
                    var url=GlobalConfig._EDataCenterUrl + 'BusTransfer/StationSuggest.aspx?node='+GlobalConfig._Node+'&l='+GlobalConfig._L+'&Keyword=' + escape(input.value);
                    ENetwork.DownloadScript(url,
                        function(){
                            __EdushiSuggest.Show(__SuggestData, x, y, w);
                        }
                     );
                 }
            }
...
//                }
        ...
        cSearchResultControl.onDrawingBusLine = function(busLineCoordList){
            _BusLineCoordList = busLineCoordList;
            _BusLineLayer.innerHTML = '';
            for (var i=0; i<_BusLineCoordList.length; i++)
            {
                var coord = _BusLineCoordList[i];
                vM.DrawPolyLine(_BusLineLayer, vM.ChangeCoords(coord.Coords, vM.Property.ZoomPer[vM.Zoom()]), coord.Size, coord.Color, coord.Alpha);
                var PopHtml='<span style="cursor:pointer; height:26px; float:left; font-size:12px; color:#000;white-space:nowrap;{$topwidth}"><span style="cursor:pointer; width:24px; height:24px; float:left; background:url(' + GlobalConfig._ImgPath + 'gongjiao.gif) no-repeat; text-align:center; padding-top:2px;">{$No}</span><span style="cursor:pointer; height:19px;line-height:19px; background:#fff; padding-right:4px; float:left;{$width};">{$BusStationName}</span></span>';
                for (var j=0; j<coord.PassStation.length; j++)
                {
                    var vlen = coord.PassStation[j].StationName.length*12.1;
                    var nd = vM.$C('div');
                    nd.innerHTML=PopHtml.replace('{$No}',(j+1)).replace('{$BusStationName}',coord.PassStation[j].StationName).replace('{$topwidth}','width:'+(vlen+40)+'px').replace('{$width}','width:'+(vlen+10)+'px');
                    vM.appendEntity(nd, _BusLineLayer, false, coord.PassStation[j].PositionX, coord.PassStation[j].PositionY,vlen+35,26,12,28, false);
                }
            }
        };
    }
...
function fnLoadPro(){
    ...
    ENetwork.DownloadScript(url,function(){
        ...
        var t='';
        for(i=ThemeMapsList.ID.length-1;i>=0;i--){      //杭州主题地图按时间倒序排
