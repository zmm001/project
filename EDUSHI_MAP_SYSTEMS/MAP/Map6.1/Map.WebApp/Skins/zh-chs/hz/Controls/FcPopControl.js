...
Object.extend(FcPopControl.prototype, {
    ...
    ShowPopData: function(type, item) {
        if (type == 6) {
            ...
            this.$('url_a').href = 'http://hangzhou.fangtoo.com/building/' + item.BID + '/?f=edushi';
            var sNumHtml = '';
            if (item.TC && parseInt(item.TC) > 0) {
                sNumHtml += '二手房(<a target="_blank"  href="http://hangzhou.fangtoo.com/house/trade/?k=' + encodeURI(item.Name) + '&f=edushi">' + item.TC + '</a>套)&nbsp;&nbsp;';
            }

            if (item.LC && parseInt(item.LC) > 0) {
                sNumHtml += '出租(<a target="_blank" href="http://hangzhou.fangtoo.com/house/lease/?k=' + encodeURI(item.Name) + '&f=edushi">' + item.LC + '</a>套)';
            }
            ...
            if (item.Exp) {
                ...
                for (var i = 0; i < item.Exp.length; i++) {
                    sDes += '<a target="_blank" href="http://hangzhou.fangtoo.com/broker/' + item.Exp[i].ID + '/?f=edushi">' + item.Exp[i].Name + '</a>';
            ...
            switch (type) {
                case 1:
                    this.$('Name_h3').innerHTML = '<span class="Til">' + item.Name + '</span>';
                    this.$('Name_h3').title = item.Name;
                    this.$('Num_p').innerHTML = '二手房(<a target="_blank" href="http://hangzhou.fangtoo.com/house/trade/?k=' + encodeURI(item.Name) + '&f=edushi">' + item.SecondNum + '</a>套)&nbsp;&nbsp;出租(<a target="_blank" href="http://hangzhou.fangtoo.com/house/lease/?k=' + encodeURI(item.Name) + '">' + item.LeaseNum + '</a>套)';
                ...
                case 5:
                    this.$('Name_h3').innerHTML = '<span class="Til">' + item.Name + '</span>';
                    this.$('Name_h3').title = item.Title;
                    this.$('Num_p').innerHTML = '二手房(<a target="_blank" href="http://hangzhou.fangtoo.com/house/trade/?k=' + encodeURI(item.Name) + '&f=edushi">' + item.SecondNum + '</a>套)&nbsp;&nbsp;出租(<a target="_blank" href="http://hangzhou.fangtoo.com/house/lease/?k=' + encodeURI(item.Name) + '&f=edushi">' + item.LeaseNum + '</a>套)';
                    this.$('Num_p').style.visibility = 'visible';
                    var hangPrice = 0;
                    if (item.AvgPriceType != 0) {
                        if (item.AvgPriceType == 3) {
