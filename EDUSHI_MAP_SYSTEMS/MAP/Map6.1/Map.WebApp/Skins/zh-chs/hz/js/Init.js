...
//是否有以下泡泡开启
var __IsEntityPopOn = false;
var __IsCompanyPopOn = false;
var __IsEShopPopOn = false;
var __IsThemePopOn = false;
function fnLoadInit() {
    ...
    _RoadLineLayer = vM.NewMapLayer('RoadLineLayer', 149, true); //道路
    ...
    vM.attachEvent(EGIS.KeyWord.EventName.AppendSpotLabel, function(spot) {
        if (spot.Group == 'build') {
            var obj = vM.$('divclick' + spot.ID + 'BuildFloor');
            if (obj) {
                obj.onclick = function(e) { BuildFloorSelectShow(spot.ID, spot, e) };
            }
        }
        else if (spot.Group == 'FT_217') {
            var obj = vM.$('FT_SpotLabel_' + spot.ID);
            if (obj) {
                if (spot.SSP && parseInt(spot.SSP) > 0) {
                    obj.innerHTML += '<span  style="font-size:12px;font-weight:bold;height:12px;color:#FF0000;filter:glow(color=#FFFF33,strength=3);">挂￥' + spot.SSP + '</span>';
                }
                else if (spot.SEP && parseInt(spot.SEP) > 0) {
                    obj.innerHTML += '<span  style="font-size:12px;font-weight:bold;height:12px;color:#FF0000;filter:glow(color=#FFFF33,strength=3);">成￥' + spot.SEP + '</span>';
                }
            }
        }
    });
    ...
    vM.attachEvent(EGIS.KeyWord.EventName.Switch3D, function() {
        ...
        if (_MapType == 1) {
            vM.ViewSystemDataLayer(true, 'FTDataLayer');
        }
    ...
    vM.attachEvent(EGIS.KeyWord.EventName.Switch2D, function() {
        ...
        vM.ViewSystemDataLayer(false, 'FTDataLayer');
...
//房价地图热门关键字
function fnLoadHotHousePriceMap() {
    var keywords = ['公寓', '小区', '滨江', '九堡', '闲林', '下沙'];
    var html = [];
    for (var i = 0; i < keywords.length; i++) {
        html.push('<a href="javascript:fnFangtooMapSearch(\'' + keywords[i] + '\')">' + keywords[i] + '</a>&nbsp;');
    }
    $('spanHotHousePriceMap').innerHTML = html.join('');
    
    keywords = ['西湖区', '上城区', '下城区', '江干区', '拱墅区'];
    html.length = 0;
    for (var i = 0; i < keywords.length; i++) {
        html.push('<a href="javascript:fnFangtooCommunityHouseSearch(\'' + keywords[i] + '\')">' + keywords[i] + '</a>&nbsp;');
    }
    $('spanAreaHousePrice').innerHTML = html.join('');
}
var _bdtmarquee = null;
function fnLoadBdtList() {    
    ...
    var arrayHtml = [];
      arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=163910" target="_blank" class="Us">博觉广告策划</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=67586" target="_blank">请问杭州哪个影视制作公司相对来说比较专业？价格相对公道</a></span>');
    //arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=174870" target="_blank" class="Us">xiaoZ</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=67377" target="_blank">过年给侄女买鞋，介绍下品牌或者童鞋店吧</a></span>');
    //arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=174688" target="_blank" class="Us">CC</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=67368" target="_blank">听说怡莱酒店非常时尚、服务也很好，有住过的吗？来杭州玩住哪家店好呢？</a></span>');
    //arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=160085" target="_blank" class="Us">姒波妖莲</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=67332" target="_blank">XDJMs帮帮忙，求找杭城个性婚纱影楼or工作室</a></span>');
        //arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=144364" target="_blank" class="Us">朱寰</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=66947" target="_blank">杭城最高档的婚纱会馆在哪里？路线怎么走？</a></span>');//12-09撤换
        arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=155250" target="_blank" class="Us">品味人生</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=65310" target="_blank">杭州王长根肝胆肾结石专科中医诊所怎么走？</a></span>');
        arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=159788" target="_blank" class="Us">joy</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=68144" target="_blank">杭州群童湾婴童生活馆在哪里？</a></span>');
        if (typeof (_BdtList) != 'undefined' && _BdtList.length > 0) {
            ...
            _bdtmarquee = new Marquee('divBdtList', 2);
            _bdtmarquee.Start();
        }
    });    
}
...
function fnSwichCity() {
    ...
    //<iframe frameborder="0" scrolling="no" allowTransparency="true"  style="height: 358px; width:640px;position:absolute; display:none; top:40px; left:60px; z-index:999;" id="iframeCityList" src="{$WebRootPath}CityList.html"></iframe>
}
function fnClosePanel(id) {
    $(id).style.display = 'none'
}
...
function fnGoogleStat(sTrackName) {
    try {
        pageTracker._trackEvent('Map6.1', 'Mater_UI', sTrackName);
    } catch (e) {
    }
}
function fnEntityGoogleStat(sTrackName) {
    try {
        pageTracker._trackEvent('Map6.1', '实体点击', sTrackName);
    } catch (e) {
    }
}
function fnCompanyGoogleStat(sTrackName) {
    try {
        pageTracker._trackEvent('Map6.1', '企业点击', sTrackName);
    } catch (e) {
    }
}
function fnBusPopGoogleStat(sTrackName) {
    try {
        pageTracker._trackEvent('Map6.1', '公交点击', sTrackName);
    } catch (e) {
    }
}
//初始化Pop
function fnInitPopControl() {
            ...
            vM.attachEvent(EGIS.KeyWord.EventName.SpotClick, function(spot) {
                if (!_IsBeginSelectMark) {
                    if (_IsBeginSelectBuild) {
                        fnEndGetBuild();
                        if (!spot.CurrentFloor) {
                            spot.CurrentFloor = 1;
                        }
                        if (spot.Group == 'build') {
                            _EShopAndCompanyJoinTab.TabBody.contentWindow.EndGetBuild(spot.RelationID, spot.Name, spot.CurrentFloor, spot.ID, spot.Key);
                        }
                        else {
                            _EShopAndCompanyJoinTab.TabBody.contentWindow.EndGetBuild(spot.ID, spot.Name, spot.CurrentFloor, 0, 0);
                        }
                        _CurrentSpot = spot;
                        fnDrawFloorBuildUnitToPoly();
                    }
                    else {
                        if (spot.Group == 'build') {
                            if (spot.RelationID != 0) {
                                fnShowEntityPop(spot.RelationID, spot.Cx * 1, spot.Cy * 1);
                            }
                        }
                        else if (spot.Group == 'hotarea') {
    ...
    if (!_EShopPopControl) {
        ...
        _EShopPopControl.ID = vM.AppendEntity(_EShopPopControl.Body, _PopLayer, false, 0, 0, 305, 316, 36, 314, false);
    ...
    if (!_FcPopCpntrol) {
        _FcPopCpntrol = new FcPopControl(vM.Body.document);
        _FcPopCpntrol.ID = vM.AppendEntity(_FcPopCpntrol.Body, _PopLayer, false, 0, 0, 305, 276, 36, 274, false);
    }
}
function fnOpenTab() {
...
//显示FcPop
function fnShowFcPop(type, item) {
    //vM.MoveEntity(_CommendPopControl.ID, x * 1, y * 1);
    //MoveTo(x * 1 + vM.GetMapPos(66), y * 1 + vM.GetMapPos(-96), true);
    _FcPopCpntrol.ShowPopData(type,item);
}

//显示主题Pop  &*&
function fnShowThemePop(id, x, y, cid) {    
...
//显示推荐Pop根据主题ID
function ShowCommendPopById(id, x, y, cid) {
    vM.MoveEntity(_CommendPopControl.ID, x * 1, y * 1);
    MoveTo(x * 1 + vM.GetMapPos(66), y * 1 + vM.GetMapPos(-96), true);
...
///////////////////////////////////
//function fnAddDigTab() {
//    var Digtab = new TabControl.Tab(document,'dig','领奖公告','/digcowry/TrophyToDay4PageCard.aspx',true,false,80);
//    fnAddTab(Digtab);
//}
//////////////////////////////////
...
window.onresize = function() {
    ...
    if (_MapFullScreenState) {
        ...
        if (_MapExpandingState) {
            ...
            $('BdtScrollBoxBg').style.width = (w - 692) + 'px';
            $('divBdtList').style.width = (w - 702) + 'px';
        }
        else {
...
function fnSearchChange(oThis) {
    ...
    switch (oThis.id) {
        ...
        case 'liEShopSearch':
            ...
            $('liBusSearch').className = '';
            $('liEAddress').className = '';
            var tab = new TabControl.Tab(document, 'EShopSearch', '找店助手', '/Fundation/FindEstore.html', true, true, 80);
            fnAddTab(tab);
            break;
        case 'liEAddress':
            $('liEShopSearch').className = '';
...
function fnKeyInputChanged(input, keyCode, x, y, w, type, maptype){
    if (input.value.length > 0 && input.value != __EdushiSuggest.LastKeyword && keyCode != 38 && keyCode != 40 && keyCode != 13)
    {
        ...
        else if (type == "fangtoo") {
            var url = '';
            if (maptype == 1) {
                url = 'http://aip.fangtoo.com/suggest.aspx?way=josnsearchbuilding&encode=utf-8&citydomain=hangzhou&sg=hp&t=all&k=' + escape(input.value);
            }
            else {
                url = 'http://aip.fangtoo.com/suggest.aspx?way=josnsearchbuilding&encode=utf-8&citydomain=hangzhou&sg=hp&t=new&k=' + escape(input.value);
            }

            ENetwork.DownloadScript(url, function() {
                if (typeof _FTSuggestData != 'undefined') {
                    _SuggestData = _FTSuggestData;

                    //                for (var key in buildingsResult.Items) {
                    //                    var item = buildingsResult.Items[key];
                    //                    if (item != '') {
                    //                        _SuggestData[_SuggestData.length] = {};
                    //                        _SuggestData[_SuggestData.length - 1].Title = item.BuildingName.replace(input.value, '<b><font color="red">' + input.value + '</font></b>') + '[' + item.Addr + ']';
                    //                        _SuggestData[_SuggestData.length - 1].Value = item.BuildingName;
                    //                        _SuggestData[_SuggestData.length - 1].Type = '2';
                    //                    }
                    //                }
                    if (_SuggestData.length > 0) {
                        __EdushiSuggest.Show(_SuggestData, x, y, w);
                    }
                    else {
                        __EdushiSuggest.Hide();
...
function onSearchEStoreLoadComplete(data, begin, end) {
    var lblHtml = '<div style="position:relative; height:24px; width:auto;"><span style="float:left;height:19px; width:6px; overflow:hidden; background-image:url(../images/EstoreLabelBg.gif); background-repeat:no-repeat;background-position:left top;"></span><span style="float:left;height:17px; line-height:17px; font-size:12px; font-weight:bold; color:#FFC; background-color:#C30; border-top:1px solid #e3e3e3; border-bottom:1px solid #e3e3e3;cursor:pointer" onclick="parent.fnShowEShopPop({$Id},{$X},{$Y})">{$Title}</span><span style="float:left;height:19px; width:6px; overflow:hidden; background-image:url(../images/EstoreLabelBg.gif); background-repeat:no-repeat;background-position:right top;"></span><div style="position:absolute; top:18px; left:15px; width:9px; height:6px; overflow:hidden; background:url(../images/EstoreLabelBg.gif) no-repeat -6px -18px;"></div></div>';
    ...
    if (data != null) {
        ...
        for (var i = 0; i < data.length; i++) {
            if (data[i].BCC_CompanyID * 1 == 0) {
                ...
                fnAppendIcon(data[i].BCC_CompanyName, data[i].X, data[i].Y, i + 1, sThemeImage, sThemeOverImage, 'if(parent.fnShowThemePop){parent.fnShowThemePop(' + data[i].BCC_ID + ', ' + data[i].X + ', ' + data[i].Y + ','+data[i].BCC_CompanyID+');}', 41, 33, 7, 32, true);
            }
        }
    }
}
...
//显示隐藏热区标签
function fnLabel(obj) {
    if (__IsShowSpotLabel) {
        vM.ViewSpotLabels(false, 'hotarea');
        vM.ViewSpotLabels(false, 'FT_217', true);
        obj.innerHTML = '显示标签';
        __IsShowSpotLabel = false;
    }
    else {
        if (_MapType == 0) {
            vM.ViewSpotLabels(true, 'hotarea');
            vM.ViewSpotLabels(false, 'FT_217', true);
        }
        else {
            vM.ViewSpotLabels(true, 'FT_217');
            vM.ViewSpotLabels(false, 'hotarea', true);
        }
        obj.innerHTML = '隐藏标签';
        __IsShowSpotLabel = true;
    }
}
...
function fnUrlParse() {
    ...
    if (x != '' && y != '' && title != '') {
        if (title.indexOf('%e8%bf%99%e9%87%8c') < 0) {
            _Mark.Show(x, y, unescape(title), unescape(content));
        }
        else {
            _Mark.Show(x, y, '这里出错了', unescape(content));
        }
    }
}
/******************end:各种定位********************/
...
/*************楼层选择功能开始**************/
var __ShowFloorCount = 5;
var _CurrentSpot = null;
function BuildFloorSelectShow(id, spot, e) {
    if (spot.Floor.length > 0 && vM.$('ul' + id + 'BuildFloor').childNodes.length == 0) {
        var ULinnerHTML = '<li id="li' + id + 'UpBuildFloor" onclick="parent.BuildFloorDownUpClick(' + id + ',0,\'Up\',CurrentSpot,event)" style="cursor:pointer;"><img src="http://api3.edushi.com/Images/FloorUp.gif" alt="向上滚动" /></li>';
        for (var i = 0; i < spot.Floor.length; i++) {
            if (i < __ShowFloorCount) {
                ULinnerHTML += '<li onclick="parent.BuildFloorSelectClick(' + id + ',this,CurrentSpot,event)" style="cursor:pointer;">' + spot.Floor[i] + '</li>';
            }
            else {
                ULinnerHTML += '<li onclick="parent.BuildFloorSelectClick(' + id + ',this,CurrentSpot,event)" style="cursor:pointer;display:none">' + spot.Floor[i] + '</li>';
            }
        }
        ULinnerHTML += '<li id="li' + id + 'DownBuildFloor" onclick="parent.BuildFloorDownUpClick(' + id + ',0,\'Down\',CurrentSpot,event)" style="cursor:pointer;"><img src="http://api3.edushi.com/Images/FloorDown.gif" alt="向下滚动"  /></li>';
        vM.$('ul' + id + 'BuildFloor').innerHTML = ULinnerHTML;
    }
    vM.$('div' + id + 'BuildFloor').style.display = 'block';
...
function BuildFloorSelectClick(id, obj,spot,e) {
    var liList = vM.$('ul' + id + 'BuildFloor').childNodes;
    for (var i = 0; i < spot.Floor.length; i++) {
        liList[i + 1].style.background = '';
        liList[i + 1].style.color = '#999';
    }
    obj.style.background = 'url(http://api3.edushi.com/Images/FloorShow.gif) no-repeat left -7px';
    obj.style.color = '#933';
    vM.$('div' + id + 'BuildFloor').style.display = 'none';
    vM.$('span' + id + 'BuildFloor').innerHTML = obj.innerHTML;
    ...
    spot.CurrentFloor = parseInt(obj.innerHTML);
    _CurrentSpot = spot;
    fnDrawFloorBuildUnitToPoly();
}

function fnDrawFloorBuildUnitToPoly() {
    if (!_CurrentSpot) {
        return;
    }
    var spot = _CurrentSpot;

    _FloorBuildUnitCoordsLayer.innerHTML = '';
    
    if (vM.$('span' + spot.ID + 'BuildFloor')) {
        vM.$('span' + spot.ID + 'BuildFloor').innerHTML = spot.CurrentFloor;
    }
    if (spot.CurrentFloor == 1) {
        return false;
    }
    else  {
        vM.DrawPoly(_FloorBuildUnitCoordsLayer, vM.GetLayerCoords(spot.Coords.toString()), 2, 'yellow', 'black', 0);
        var url = 'http://pic6.edushi.com/GetData.aspx?action=getfloor&gbrid='+spot.ID+'&floor=' + spot.CurrentFloor;
        ENetwork.DownloadScript(url, function() {
            if (typeof __FloorData != undefined) {
                for (var i = 0; i < __FloorData.length; i++) {
                    for (var j = 0; j < __FloorData[i].Coords.length; j++) {
                        vM.DrawPolyLine(_FloorBuildUnitCoordsLayer, vM.GetLayerCoords(__FloorData[i].Coords[j].toString()), 2, 'blue', 0);
                        var poi = vM.$C('div');
                        poi.innerHTML = '<div onclick="parent.fnShowCompanyPop(' + __FloorData[i].RelationID + ',' + __FloorData[i].Cx + ',' + __FloorData[i].Cy + ')" style="font-size:12px;height:16px;white-space: nowrap; padding: 2px;cursor:pointer;">' + __FloorData[i].Name + '</div>';
                        vM.AppendEntity(poi, _FloorBuildUnitCoordsLayer, false, __FloorData[i].Cx, __FloorData[i].Cy, false, false, 2, 8, false);
                    }
                }

            }
        }, 'FloorBuildUnitCoordsLayerScript');
        vM.detachEvent(EGIS.KeyWord.EventName.MapZoomChange, fnDrawFloorBuildUnitToPoly);
        vM.attachEvent(EGIS.KeyWord.EventName.MapZoomChange, fnDrawFloorBuildUnitToPoly);
    }
    

}
function BuildFloorDownUpClick(id, index, updown,spot,e) {
    var liList = vM.$('ul' + id + 'BuildFloor').childNodes;
    if (updown == 'Up') {
        if (index > 0) {
            liList[index].style.display = '';
            liList[index + __ShowFloorCount].style.display = 'none';
            index -= 1;
        }
    }
    else {
        if (index < spot.Floor.length - __ShowFloorCount) {
            index += 1;
            liList[index].style.display = 'none';
            liList[index + __ShowFloorCount].style.display = '';
            
        }
    }
    vM.$('li' + id + 'UpBuildFloor').onclick = function(e) { BuildFloorDownUpClick(id, index, 'Up', spot,e) };
    vM.$('li' + id + 'DownBuildFloor').onclick = function(e) { BuildFloorDownUpClick(id, index, 'Down', spot, e) };
    if (!e) {
        e = vM.Body.event;
    }
    //停止事件冒泡
...
function fnEShopAndCompanyJoin(oid,oname) {
    ...
    var url = '/Fundation/EShopAndCompanyJoin.aspx?oid='+oid+'&oname='+oname;
    if (!_TabControl.ExistTab(url)) {
        _EShopAndCompanyJoinTab = new TabControl.Tab(document, 'EShopAndCompanyJoin', '我要入驻', url, true, true, 80);
...
function fnStartGetUnitCoords() {
    ...
    vM.StartGetCoords('请描点连成您要认领的户型，左键取点，右键撤销，双击结束。');
    return true;
}
...
function SwitchMapType(obj) {
    switch (obj.id) {
        case 'liEdushiMap':
            if (_MapType != 0) {
                fnGoogleStat('切换到三维地图');
                vM.Body.ChangeShowSpotShape(false);
                $('liEdushiMap').className = 'Edushi Current';
                $('liFangtooMap').className = 'Fangtoo';
                $('FcSearchNav').style.display = 'none';
                $('SearchNav').style.display = '';

                _MapType = 0;
                vM.ViewSpotAreas(true, 'hotarea');
                vM.ViewSpotAreas(false, 'FT_217');
                __IsShowSpotLabel = true;
                vM.ViewSystemDataLayer(false, 'FTDataLayer');
                fnLabel($('ShowSpotLabel_a'));
            }
            else {
                return;
            }
        ...
        case 'liFangtooMap':
            if (_MapType == 0) {
                fnGoogleStat('切换到地图找房');
                vM.Body.ChangeShowSpotShape(true);
                $('liEdushiMap').className = 'Fangtoo';
                $('liFangtooMap').className = 'Edushi';
                $('FcSearchNav').style.display = '';
                $('SearchNav').style.display = 'none';

                _MapType = 1;
                vM.ViewSpotAreas(false, 'hotarea');
                vM.ViewSpotAreas(true, 'FT_217');
...
//房图搜索选项切换
function fnFcSearchChange(obj) {
    __EdushiSuggest.Hide();
    switch (obj.id) {
        case 'liFcPrMap':
            $('liFcNewsHouse').className = '';
            $('liFcSecHouse').className = '';
            $('liFcHackHouse').className = '';
            $('liFcArHouse').className = '';
            $('divFcPrMap').style.display = 'block';
            $('divFcNewsHouse').style.display = 'none';
            $('divFcSecHouse').style.display = 'none';
            $('divFcHackHouse').style.display = 'none';
            $('divFcArHouse').style.display = 'none';
            break;
        case 'liFcNewsHouse':
            $('liFcPrMap').className = '';
            $('liFcSecHouse').className = '';
            $('liFcHackHouse').className = '';
            $('liFcArHouse').className = '';
            $('divFcNewsHouse').style.display = 'block';
            $('divFcPrMap').style.display = 'none';
            $('divFcSecHouse').style.display = 'none';
            $('divFcHackHouse').style.display = 'none';
            $('divFcArHouse').style.display = 'none';
            break;
        case 'liFcSecHouse':
            ...
            $('liFcHackHouse').className = '';
            $('liFcArHouse').className = '';
            $('divFcSecHouse').style.display = 'block';
            $('divFcPrMap').style.display = 'none';
            $('divFcNewsHouse').style.display = 'none';
            $('divFcHackHouse').style.display = 'none';
            $('divFcArHouse').style.display = 'none';
        ...
        case 'liFcHackHouse':
            ...
            $('liFcSecHouse').className = '';
            $('liFcArHouse').className = '';
            $('divFcHackHouse').style.display = 'block';
            $('divFcPrMap').style.display = 'none';
            $('divFcNewsHouse').style.display = 'none';
            $('divFcSecHouse').style.display = 'none';
            $('divFcArHouse').style.display = 'none';
        ...
        case 'liFcArHouse':
            ...
            $('liFcSecHouse').className = '';
            $('liFcHackHouse').className = '';
            $('divFcArHouse').style.display = 'block';
            $('divFcPrMap').style.display = 'none';
            $('divFcNewsHouse').style.display = 'none';
            $('divFcSecHouse').style.display = 'none';
            $('divFcHackHouse').style.display = 'none';
...
function fnFangtooMapGoogleStat(sFunc, sKey) {
    try {
        pageTracker._trackEvent('Map6.1', sFunc, sTrackName);
    } catch (e) {
    }
}
function fnFangtooMapSearch(key) {
    ...
    if (key) {
        ...
        keyword = $('txtFcPrMapKeyword').value;
    }
    if (keyword == '输入小区名、地址或关键字') {
        keyword = '';
    } 
    fnFangtooMapGoogleStat('房价地图',keyword);
    var tab = new TabControl.Tab(document, 'FcLocalSearch', keyword==''?'房价地图':keyword, '/Fundation/FcLocalSearch.aspx?type=1&keyword=' + escape(keyword), true, true, 80);
    fnAddTab(tab);
}
function fnFangtooNewHouseSearch() {
    var keyword = $('txtFcNewsHouseKeyword').value;
    if (keyword == '输入小区名、地址') {
        keyword = ''
    }
    fnFangtooMapGoogleStat('新房', keyword);
    var region = $('FcNewsHouseRegion').value;
    var type = $('FcNewsHouseType').value;
    var price = $('FcNewsHousePrice').value.split('-');
    if (price.length != 2) {
        price[0] = '';
        price[1] = '';
    }
    var tab = new TabControl.Tab(document, 'FcLocalSearch', keyword==''?'新房':keyword, '/Fundation/FcLocalSearch.aspx?type=2&regionCode=' + region + '&housekind=' + type + '&pl=' + price[0] + '&ph='+ price[1] +'&keyword=' + escape(keyword), true, true, 80);
    fnAddTab(tab);
}

function fnFangtooSecondHouseSearch() {
    var keyword = $('txtFcSecondHouseKeyword').value;
    if (keyword == '输入小区名、地址') {
        keyword = '';
    }
    fnFangtooMapGoogleStat('二手房', keyword);
    var region = $('FcSecHouseRegion').value;
    var type = $('FcSecHouseType').value;
    var price = $('FcSecHousePrice').value.split('-');
    if (price.length != 2) {
        price[0] = '';
        price[1] = '';
    }
    var tab = new TabControl.Tab(document, 'FcLocalSearch', keyword==''?'二手房':keyword, '/Fundation/FcLocalSearch.aspx?type=3&regionCode=' + region + '&housekind=' + type + '&pl=' + price[0] + '&ph=' + price[1] + '&keyword=' + escape(keyword), true, true, 80);
    fnAddTab(tab);
}

function fnFangtooLeaseHouseSearch() {
    var keyword = $('txtFcLeaseHouseKeyword').value;
    if (keyword == '输入小区名、地址') {
        keyword = '';
    }
    fnFangtooMapGoogleStat('租房', keyword);
    var region = $('FcHackHouseRegion').value;
    var type = $('FcHackHouseType').value;
    var price = $('FcHackHousePrice').value.split('-');
    if (price.length != 2) {
        price[0] = '';
        price[1] = '';
    }
    var tab = new TabControl.Tab(document, 'FcLocalSearch', keyword==''?'租房':keyword, '/Fundation/FcLocalSearch.aspx?type=4&regionCode=' + region + '&housekind=' + type + '&pl=' + price[0] + '&ph=' + price[1] + '&keyword=' + escape(keyword), true, true, 80);
    fnAddTab(tab);
}

function fnFangtooCommunityHouseSearch(key) {
    var keyword = '';
    if (key) {
        keyword = key;
    }
    else {
        keyword = $('txtFcCommunityHouseKeyword').value;        
    }
    if (keyword == '输入小区名、地址或关键字') {
        keyword = '';
    }
    fnFangtooMapGoogleStat('小区房价', keyword);
    var tab = new TabControl.Tab(document, 'FcLocalSearch', keyword==''?'小区房价':keyword, '/Fundation/FcLocalSearch.aspx?type=5&keyword=' + escape(keyword), true, true, 80);
    fnAddTab(tab);
}
...
{      
    ...
    else 
    {
      if ( $('NewHotelImg'))
      {
          $('NewHotelImg').style.display='none';
      }
