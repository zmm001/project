...
////   var adurl =window.Config.EDataCenterUrl + 'ad/ads.aspx?citycode=' + window.Config.CityCode + '&l=' + window.Config.Language + '&key=gongjiaoliebiao&domid=div_ad&rnd=' + ENetwork.GetExecutionID();
//  
//   ENetwork.DownloadScript(adurl);
   //
...
function fnBusStationShow(){
    var sBusNo = '共有<strong>{$No}</strong>项符合<strong>{$Key}路</strong>的查询结果';
    var r='';
    ...
    { 
        ...
        $('TabBusCount').innerHTML = '没有相关数据';
        $('TabContent').innerHTML ='';
       
       return false;
    }
    $('TabBusCount').innerHTML = sBusNo.replace('{$No}', _Bus.length).replace('{$Key}', unescape(fnRequest('key'))); 
