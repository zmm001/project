...
Object.extend(CompanyPopControl.prototype, {
    ...
    LoadPopData: function() {
        if (typeof _EntityInfo != 'undefined' && _EntityInfo.CompanyInfoDetail[0]) {
            ...
            this.$('Img_img').src = 'http://' + this.Config.Domain + '/yp/CompanyDetail.aspx?id=' + o.CompanyID; 
            ...
            if (this.Config.IsBDT == 1) {
             ...
             var existurl = "http://"+this.Config.Domain+"/ExistHotel.aspx?type=1&HotelList="+escape(o.OCName)+"&CityCode="+this.Config.CityCode;
           
              ENetwork.DownloadScript(existurl, this.existName.bind(this));
 ...
 existName: function ()
   {
       if (typeof ExistName != 'undefined' && typeof this.Entity != 'undefined')
          {
            if (ExistName !="0")            
             {
          
                this.$('sp_wydz').style.display ='block';

                var links = this.$('view_a').href+'&DiscountState=1';

                this.$('a_wydz').href =links;
                this.$('view_a').href = links;
             }
             else 
             {
                 this.$('sp_wydz').style.display ='none';
             }
