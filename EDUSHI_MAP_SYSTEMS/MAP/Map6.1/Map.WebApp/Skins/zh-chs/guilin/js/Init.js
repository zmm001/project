...
var _EShopPopControl;           //E店Pop tmm 20100401
...
function fnShowSearchPop(oid, cid, lstid, x, y, datatype, name, address, telephone) {
    if (datatype == 1 || datatype == 2) {
        ...
        //        if (cid * 1 > 0) {
        //            fnShowCompanyPop(cid, x, y); &*& tmm修改
        //        }
        if (cid * 1 > 0) {
            if (lstid > 0) {
...
function fnKeyInputChanged(input, keyCode, x, y, w, type, maptype) {
    if (input.value.length > 0 && input.value != __EdushiSuggest.LastKeyword && keyCode != 38 && keyCode != 40 && keyCode != 13) {
        if (type == 'eshop') {
            ...
            var url = '';
            if (maptype == 1) {
                url = 'http://aip.fangtoo.com/suggest.aspx?way=josnsearchbuilding&encode=utf-8&citydomain=guilin&sg=hp&t=all&k=' + escape(input.value);
            }
            else {
                url = 'http://aip.fangtoo.com/suggest.aspx?way=josnsearchbuilding&encode=utf-8&citydomain=guilin&sg=hp&t=new&k=' + escape(input.value);
            }
