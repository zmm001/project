...
function fnCopyMarkEAddress() {
    if ($('hx').value != '' && $('hy').value != '') {
        var sDomainUrl = '/Fundation/EAddressHandle.aspx?operatortype=5&rnd=' + $Rnd(8);
        var ajax = new Ajax();
        ajax.post($('form1')
            , function(xmlObj) {
                if (xmlObj != null) {
                    ...
                    if (sReturn.indexOf("Fail:") > -1) {
                        ...
                        parent.fnCopyToClipboard('http://edizhi.edushi.com/' + sReturn, '复制好了，发给我的QQ/MSN好友分享吧！');
...
function GetRndName() {
    ...
    var sDomainUrl = '/Fundation/EAddressHandle.aspx?operatortype=4&rnd=' + $Rnd(8);
...
function CheckEAddress(id, name) {
    ...
    var sVerifyUrl = '/Fundation/EAddressHandle.aspx?operatortype=2&lmeid=' + id + '&eaddress=' + name;
...
function RedirectCity(type, citycode) {
    var sDomainUrl = '/Fundation/EAddressHandle.aspx?operatortype=3&citycode=' + citycode + '&rd=' + Math.round(Math.random() * 10000);
    ENetwork.DownloadScript(sDomainUrl, function() {
