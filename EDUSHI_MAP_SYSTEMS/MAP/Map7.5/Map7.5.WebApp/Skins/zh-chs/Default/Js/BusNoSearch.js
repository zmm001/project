...
function BusLineSearch() {
    ...
    this.fnShowInfo = function(i) {
        ...
        if (action == 1) {
            ...
            $('TabContent').innerHTML = _temp.replace("{$i}", i).replace("{$BusLineID}", _BusNumberData[i].LineInfo.VehicleID).replace("{$BusLineName}", _BusNumberData[i].LineInfo.VehicleName).replaceAll("{$StartStation}", _BusNumberData[i].LineInfo.StartStation).replaceAll("{$EndStation}", _BusNumberData[i].LineInfo.EndStation).replace("{$LineInfoID}", "LineInfo" + i);
            $('LineInfo' + i).style.display = 'block';
            fnResize();
        }
