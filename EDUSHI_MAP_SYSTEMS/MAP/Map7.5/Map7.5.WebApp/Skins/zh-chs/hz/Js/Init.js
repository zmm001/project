...
window.onresize = function () {
    ...
    if (_MapFullScreenState) {
        ...
        jQuery("#Wrapper").css("height", (h - 96) + 'px');
...
function fnOpenMergeSearch(key, lst_id, domain, cid, vid, tid, card, vip, type, tags, adtype) {
    var tab = new TabControl.Tab(document, 'MergeSearch', key, '/Fundation/MergeSearch.aspx?key=' + escape(key), true, true, 80);
    fnAddTab(tab);
    return tab;
}
    ...
    if (id > 0) {
        if (!_EntityPopControl) {
            ENetwork.DownloadScript('/Controls/EntityPopControl.js', function() {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _EntityPopControl.ID = vM.AppendEntity(_EntityPopControl.Body, _PopLayer, false, 0, 0, 332, 315, 62, 302, false);
                } else {
                    _EntityPopControl.ID = vM.AppendEntity(_EntityPopControl.Body, _PopLayer, false, x, y, 332, 315, 62, 302, false);
                }
    ...
    if (id > 0) {
        if (!_CompanyPopControl) {
            ENetwork.DownloadScript('/Controls/CompanyPopControl.js', function() {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _CompanyPopControl.ID = vM.AppendEntity(_CompanyPopControl.Body, _PopLayer, false, 0, 0, 328, 310, 62, 302, false);
                }
                else {
                    _CompanyPopControl.ID = vM.AppendEntity(_CompanyPopControl.Body, _PopLayer, false, x, y, 328, 310, 62, 302, false);
                }
    ...
    if (id > 0) {
        if (!_EShopPopControl) {
            ENetwork.DownloadScript('/Controls/EShopPopControl.js', function() {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _EShopPopControl.ID = vM.AppendEntity(_EShopPopControl.Body, _PopLayer, false, 0, 0, 328, 330, 62, 322, false);
                }
                else {
                    _EShopPopControl.ID = vM.AppendEntity(_EShopPopControl.Body, _PopLayer, false, x, y, 328, 330, 62, 322, false);
                }
...
function fnShowBusStation(id, name, x, y) {
    if (!_BusStationControl) {
        ENetwork.DownloadScript('/Controls/BusStationControl.js', function() {
            ...
            _BusStationControl.ID = vM.AppendEntity(_BusStationControl.Body, _PopLayer, false, x, y, 330, 300, 64, 288, false);
            _BusStationControl.Width = 330;
            _BusStationControl.Height = 300;
    ...
    if (id > 0) {
        if (!_HotelPopControl) {
            ENetwork.DownloadScript('/Controls/HotelPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _HotelPopControl.ID = vM.AppendEntity(_HotelPopControl.Body, _PopLayer, false, 0, 0, 328, 330, 62, 322, false);
                }
                else {
                    _HotelPopControl.ID = vM.AppendEntity(_HotelPopControl.Body, _PopLayer, false, x, y, 328, 330, 62, 322, false);
                }
