...
function fnOnload() {
    ...
    _cookieobj.key1 = unescape(fnRequest('s'));
    _cookieobj.key2 = unescape(fnRequest('e'));
    _cookieobj.type = 10;
    var j1 = JSON.stringify(_cookieobj);
    if (cookieNickNameCookie != null) {
        var _SearchList = _cookie.getCookie(cookieNickNameCookie + this.Config.CityCode);
        if (_SearchList != null) {
            ...
            for (var i = 0; i < len; i++) {
                if (_local[i].key1 == unescape(fnRequest('s')) && _local[i].key2 == unescape(fnRequest('e')) && parseInt(_local[i].type) == 10) {
                    f = false;
                    break;
                }
            }
            if (f) {
                _cookie.setCookie(cookieNickNameCookie, j1 + "," + _SearchList, expiration, "/", "edushi.com");
            }
        }
        else {
            _cookie.setCookie(cookieNickNameCookie + this.Config.CityCode, j1, expiration, "/", "edushi.com");
        }
    }
    ...
    var url;
    url = window.Config.EDataCenterUrl + 'CommMap5.0/Bus.aspx?domain=' + window.Config.Domain + '&req=9&l=' + window.Config.Language + '&Start=' + fnRequest('s') + '&End=' + fnRequest('e');
    LoadHTML = '';
...
function fnTransfer(sindex, eindex) {
    ...
    var url = window.Config.EDataCenterUrl + 'CommMap5.0/Transfer.aspx?domain=' + window.Config.Domain + '&l=' + window.Config.Language + '&StartId=' + sItem.ID + '&StartName=' + escape(sItem.Value) + '&StartType=' + stempType + '&StartX=' + sItem.X + '&StartY=' + sItem.Y + '&EndId=' + eItem.ID + '&EndName=' + escape(eItem.Value) + '&EndType=' + etempType + '&EndX=' + eItem.X + '&EndY=' + eItem.Y;
