...
function fnShowData(o){
    ...
    reLoadPageSize();
    var t='<div class="PjIncomeNav" id="TabContent" style=\"padding-top:10px;\">本建筑共收录<strong>{$count}</strong>家企业<a href="javascript:parent.fnEShopAndCompanyJoin('+fnRequest('oid') + ',\'' + unescape(fnRequest('ocname')) + '\')"  title="企业入驻"><img src="/Images/ArchitBtn.gif" alt="企业入驻" class="PjIncomeImg" /></a><ul class="ShowList">{$Record}</ul><div class="Paginate">{$PageStr}</div></div>';
    var strRecord = '<li>{$No}.<a href="{$Url}" style="{$Style}" target="_blank" title="{$CompanyName}">{$CompanyName}</a>{$VipIcon}</li>';
    var strRecordTemp='';
    if(_Search.length > 0){
        _Count=_Search.length;
        ...
        for(i=intBegin;i<intEnd;i++){
            var sUrl, sVipIcon;
            if (_Search[i].Vip * 1 > 0) {
                //sUrl = 'http://' + _Search[i].Eaddress + '.' + window.Config.Domain;
                sVipIcon = '<img src="/Images/VipMember.gif" />';
            }
            else {
                sVipIcon = '';
            }
            if (_Search[i].LST_ID == 1 && _Search[i].Vip * 1 > 0) {
                sUrl = this.Config.DianUrl + 'vipstore/3/Index.aspx?StoreID=' + _Search[i].CompanyID;
            } else {
                sUrl = parent.parent.fnCreateUrl(_Search[i].CompanyID, '1');
            }
            
            strRecordTemp += strRecord.replaceAll('{$CompanyName}', _Search[i].OCName).replace('{$CompanyID}', _Search[i].CompanyID).replace('{$No}', (i + 1)).replace('{$Url}', sUrl).replace('{$VipIcon}', '');
        }
