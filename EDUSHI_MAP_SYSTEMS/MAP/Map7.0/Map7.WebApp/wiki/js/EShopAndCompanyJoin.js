...
function LoadEShopBigType(oSelect, type) {
    var sBigTypeUrl = ecedatacenterurl + 'ShopType/ShopTypeList.aspx?req=' + (type == 1 ? 'trade' : 'zone') + '&domain=' + ecdomain + '&l=' + eclanguage + '&parentid=0';
...
function LoadEShopSmallType(oSelect, type, parentId) {
    ...
    var sSmallTypeUrl = ecedatacenterurl + 'ShopType/ShopTypeList.aspx?req=' + (type == 1 ? 'trade' : 'zone') + '&domain=' + ecdomain + '&l=' + eclanguage + '&parentid=' + parentId;
...
function fnCheckUserLoginInit(edatacenterurl,cityCode) {
    ...
    if ($('liLoginInfo').innerHTML == '') {
        ...
        if (cookieNickNameCookie != null) {
            $('liLoginInfo').innerHTML = '欢迎您！<span style="color:#ff6400">' + cookieNickNameCookie + '</span><span>[<a href="' + ecedatacenterurl + 'Index.aspx?city=' + eccityCode + '" target="_blank">我的E都市</a>]</span>&nbsp;<span>[<a href="javascript:fnLoginOut();">退出</a>]</span>';
        }
        else {
...
function fnLoginSuccess(memberInfo) {
    $('liLoginInfo').innerHTML = '欢迎您！<span style="color:#ff6400">' + memberInfo.LM_NickName + '</span><span>[<a href="' + ecuserCenterUrl + 'Index.aspx?city=' + eccityCode + '" target="_blank">我的E都市</a>]</span>&nbsp;<span>[<a href="javascript:fnLoginOut();">退出</a>]</span>';
