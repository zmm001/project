...
function fnBusStationShow(){
   if(typeof _Station != 'undefined'){
         ...
         var sBusStation = '<div class="busstation" id="TabContent">共有<strong>{$No}</strong>项符合站名<strong>{$Key}</strong>的查询结果{$Blist}</div>';
         document.body.innerHTML=sBusStation.replace('{$Blist}',s).replace('{$No}',l).replace('{$Key}',unescape(fnRequest('key')));
         $('TabContent').style.height = fnGetWindowHeight()-10+'px';  //初始化高度
     }
     else{
           document.body.innerHTML='<div class="NoInfoClew">没有相关数据！</div>';
           return false;
    }        
}
function fnShow(i){
