...
function fnBusStationShow(){
    var sBusNo = '<div class="Bus" id="TabContent">共有<strong>{$No}</strong>项符合<strong>{$Key}路</strong>的查询结果{$NoList}</div>';
    var r='';
    ...
    document.body.innerHTML=sBusNo.replace('{$No}',_Bus.length).replace('{$Key}',unescape(fnRequest('key'))).replace('{$NoList}',r);
...
function fnShowInfo(i){
    ...
    if(fnRequest('action')==1){
        document.body.innerHTML='<div class="Bus" id="TabContent"><div id="NoList0" class="EachRow"><div class="BusLineActive"><strong>'+_BusNumberData[i].LineInfo[0].VehicleName+'</strong> &nbsp;&nbsp;'+_BusNumberData[i].LineInfo[0].StartStation+'-'+_BusNumberData[i].LineInfo[0].EndStation+'</div><div class="DottedLine"></div></div><div id="NoInfo0" style="display:block"></div></div>';
        $('TabContent').style.height = fnGetWindowHeight()-10+'px';  //初始化高度
    }
    ...
    if(k==1){
        ...
        for(i=0;i<_BusNumberData[no].BusUp.length;i++){
            ...
            else
            {
                if(_CurrentBusStation == oBusUp.StationName)
                { 
                    info+= ' <li style="background-color:#f6f6f6; color:#FF6400">'+(i+1)+'.<span style="color:#FF6400" onclick="parent.fnGotoBusStation('+oBusUp.StationID+',\''+oBusUp.StationName+'\','+oBusUp.PositionX+','+oBusUp.PositionY+');">'+oBusUp.StationName+'&nbsp;<img src="'+window.Config.SkinPath+'Images/GoToIco.gif" alt="定位公交站点" style="cursor:pointer;" onclick="parent.fnGotoBusStation('+oBusUp.StationID+',\''+oBusUp.StationName+'\','+oBusUp.PositionX+','+oBusUp.PositionY+');" /></span></li>';
                }
                else
                {
                    info+= ' <li>'+(i+1)+'.<span onclick="parent.fnGotoBusStation('+oBusUp.StationID+',\''+oBusUp.StationName+'\','+oBusUp.PositionX+','+oBusUp.PositionY+');">'+oBusUp.StationName+'&nbsp;<img src="'+window.Config.SkinPath+'Images/GoToIco.gif" alt="定位公交站点" style="cursor:pointer;" onclick="parent.fnGotoBusStation('+oBusUp.StationID+',\''+oBusUp.StationName+'\','+oBusUp.PositionX+','+oBusUp.PositionY+');" /></span></li>';
                }
            }
        ...
        for(i=0;i<_BusNumberData[no].BusDown.length;i++){
            ...
            else
            {
                if(_CurrentBusStation == oBusDown.StationName)
                {
                    info+= ' <li style="background-color:#f6f6f6;color:#FF6400">'+(i+1)+'.<span style="color:#FF6400" onclick="parent.fnGotoBusStation('+oBusDown.StationID+',\''+oBusDown.StationName+'\','+oBusDown.PositionX+','+oBusDown.PositionY+');">'+oBusDown.StationName+'&nbsp;<img src="'+window.Config.SkinPath+'Images/GoToIco.gif" alt="定位公交站点" style="cursor:pointer;" onclick="parent.fnGotoBusStation('+oBusDown.StationID+',\''+oBusDown.StationName+'\','+oBusDown.PositionX+','+oBusDown.PositionY+');" /></span></li>';
                }
                else
                {
