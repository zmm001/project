...
function fnOutPut(a)
{
        ...
        ENetwork.DownloadScript(url,function(){
            ...
            {
                $('ADImage').src = window.Config.PicUrl + 'cn/'+window.Config.CityCode+'/'+window.Config.Language+'/themeImages/AD/' + _ThemeAD.ThemeAD[0].BCT_ADImage;
                $('ADImage').style.display = 'block';
                $('ADImage').parentNode.href = _ThemeAD.ThemeAD[0].BCT_ADUrl;
            }
            else
            {
                if(typeof parent._StaticAd!='undefined' && parent._StaticAd.SearchListAd.length > 0)
                {
                    if (this.$('ADImage'))
                    {
                        this.$('ADImage').src = window.Config.PicUrl + parent._StaticAd.SearchListAd[0].MAI_ImagePath + parent._StaticAd.SearchListAd[0].MAI_ImageName;
                        this.$('ADImage').style.display = 'block';
                        this.$('ADImage').parentNode.href = 'http://' + parent._StaticAd.SearchListAd[0].MAI_ImageLink.replace('http://','');
                    }
                }
            }
        });
...
function fnShow(){
    ...
    var sLocalSearch = '<div class="LocalResult" id="TabContent"> '
                            ...
                            +'<div class="Photo"><a href="{$ADLink}" target="_blank"><img id="ADImage" src="{$ADImagePath}" width="276px" height="45px" style="border:0px;{$DisPlay}"></a></div></div>';
