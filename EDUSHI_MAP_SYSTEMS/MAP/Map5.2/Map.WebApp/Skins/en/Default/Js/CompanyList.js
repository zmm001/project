...
function fnShowData(o){
    ...
    if(_Search.SearchTable.length > 0){
        ...
        for(i=intBegin;i<intEnd;i++){
            //杭州添加E店链接及图标
            var sUrl,sVipIcon;
            if (_Search.SearchTable[i].Vip*1 > 0)
            {
                sUrl = 'http://' + _Search.SearchTable[i].Eaddress + '.' + window.Config.Domain;
                sVipIcon = '<img src="' + window.Config.SkinPath + 'Images/VipMember.gif" style="margin:0 0 -7px 3px; border:none" />';
            }
            else
            {
                sUrl = 'http://'+window.Config.Domain+'/yp/CompanyDetail.aspx?ID='+_Search.SearchTable[i].CompanyID;
                sVipIcon = '';
            }            
