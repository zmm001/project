    ...
    LoadPopData: function() {
        if (typeof _EntityInfo != 'undefined' && _EntityInfo.OwnerInfoDetail[0]) {
            ...
            if (window.__IsSourceAD) {
                ...
                _script.src = this.Config.EDataCenterUrl + 'Ad/ImageAd.aspx?domain=' + this.Config.Domain + '&l=' + this.Config.Language + '&req=SOURCEPOP&domid=AD_a';
                this.$('AD_a').appendChild(_script);
            }
            else {
                if (!this.HasLoadAd) {
                    _script = this.$C('script');
                    _script.charset = 'utf-8';
                    _script.src = this.Config.EDataCenterUrl + 'Ad/ImageAd.aspx?domain=' + this.Config.Domain + '&l=' + this.Config.Language + '&req=POP&domid=AD_a';
                    this.$('AD_a').appendChild(_script);
                }
                this.HasLoadAd = true;
            }
