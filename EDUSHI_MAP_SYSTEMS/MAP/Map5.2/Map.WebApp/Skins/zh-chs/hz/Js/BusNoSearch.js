...
function fnBusStationShow(){
    ...
    { 
        ...
        for(i=0;i<len;i++)
        {
            r+='<div id="NoList'+i+'" class="EachRow" onclick="fnLoadData('+i+','+_Bus[i].VehicleID+');"><div id="BusLineName'+i+'" class="BusLineNormal" title="点击展开线路站点"><strong>'+_Bus[i].VehicleName+'</strong> &nbsp;&nbsp;'+_Bus[i].StartStation+'-'+_Bus[i].EndStation+'</div><div class="DottedLine"></div></div><div style="display:none" id="NoInfo'+i+'"></div>';
        }
    }else{
...
function fnReverse(no, k) {
    if (parent.__IsEntityPopOn || parent.__IsCompanyPopOn || parent.__IsEShopPopOn || parent.__IsThemePopOn) {
