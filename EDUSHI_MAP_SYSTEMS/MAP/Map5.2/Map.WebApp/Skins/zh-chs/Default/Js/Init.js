...
/********************end:公交搜索回调**********************/
function fnOpenMergeSearch(key, domain, vip, type, tags, cityname, adtype) {
    fnOpenTab();
    if (!adtype) {
        adtype = 0;
    }
    var tab = new TabControl.Tab(document, 'MergeSearch', key, GlobalConfig.SkinPath + 'Fundation/MergeSearch.aspx?key=' + escape(key) + '&domain=' + domain + '&vip=' + vip + '&type=' + type + '&tags=' + tags + '&cityname=' + escape(cityname) + '&adtype=' + adtype, true, true, 80);
    fnAddTab(tab);
    return tab;
}
...
function fnUrlSearch() {
    __EdushiSuggest.Hide();
    var btype = fnRequest('btype');
    var stype = fnRequest('stype');
    var key1 = fnRequest('key1');
    var key2 = fnRequest('key2');
    var iMapSearchType;
    //地图搜索
    if (btype == 'map') {
        fnSearchChange($('liMapSearch'));
        switch (stype) {
            case 'site':
                fnGoogleStat('模糊搜索');
                iMapSearchType = 0;
                fnMapSearchChange('0');
                $('radioSearchType0').checked = true;
                break;
            case 'name':
                fnGoogleStat('名称搜索');
                iMapSearchType = 1;
                fnMapSearchChange('1');
                $('radioSearchType1').checked = true;
                break;
            case 'ads':
                fnGoogleStat('地址搜索');
                iMapSearchType = 2;
                fnMapSearchChange('2');
                $('radioSearchType2').checked = true;
                break;
            case 'cir':
                fnGoogleStat('周边搜索');
                iMapSearchType = 3;
        ...
        switch (stype) {
            ...
            case 'path':
                ...
                $("txtSearchKey").innerText = unescape(key1);
                tab = new TabControl.Tab(document, 'search', unescape(key1), '/Fundation/LocalSearch.aspx?cityname=' + escape(GlobalConfig.CityName) + '&type=' + iMapSearchType + '&keyword1=' + key1, true, true, 80);
                break;
            case 'cir':
                fnGoogleStat('周边搜索');
                if (key1.length < 1) {
                ...
                $('txtSearchPlace').innerText = unescape(key1);
                $('txtSearchSomething').innerText = unescape(key2);
                tab = new TabControl.Tab(document, 'search', unescape(key1) + unescape(key2), '/Fundation/LocalSearch.aspx?cityname=' + escape(GlobalConfig.CityName) + '&type=' + iMapSearchType + '&keyword1=' + key1 + '&keyword2=' + key2, true, true, 80);
                break;
        }
        fnAddTab(tab);
    }
    //公交、地铁搜索
        ...
        switch (stype) {
            case 'deuce':   //公交两点搜索
                ...
                $('txtBusStart').innerText = unescape(key1);
                $('txtBusEnd').innerText = unescape(key2);
                fnAddBusTransferSearchTab(unescape(key1), unescape(key2), '0');
                break;
            case 'line':   //公交线路搜索
                fnGoogleStat('公交线路搜索');
                fnBusSearchChange('1');
                ...
                $('txtBusLine').innerText = unescape(key1);
                fnAddBusLineSearchTab(unescape(key1));
                break;
            case 'station':   //公交站点搜索
                fnGoogleStat('公交站点搜索');
                fnBusSearchChange('2');
                ...
                $('txtBusStation').innerText = unescape(key1);
                fnAddBusStationSearchTab(unescape(key1));
                break;
            case 'zbBus': //周边公交
                fnGoogleStat('周边公交');
                if (key1.length < 1) {
                ...
                var klist = unescape(key1).split(',');
                fnShowPeripheralBus(klist[0], klist[1], unescape(key2));
                break;
        }
    }
}