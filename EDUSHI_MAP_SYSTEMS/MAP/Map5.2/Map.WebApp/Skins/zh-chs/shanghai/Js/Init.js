...
function fnLoadInit() {
    ...
    {
        $('liLoginInfo').innerHTML = '欢迎您！<span>[<a href="http://my.edushi.com/Register.aspx?city='+GlobalConfig.CityCode+'" target="_blank" title="注册">注册</a>]</span><span>[<a href="javascript:;" onclick="fnShowLogin();pageTracker._trackEvent(\'Map5.2\', \'Mater_UI\', \'登录\'); ">登录</a>]</span>';
    }
    //加载对象
...
//--------tmm----------------浮云点击
    ...
    {     
         HotelTab = new TabControl.Tab(document, 'hotellist', '酒店预订', GlobalConfig.SkinPath + 'Fundation/HotelList.htm', true, true, 80);
            fnAddTab(HotelTab);
    ...
    function fnAddADHotelTab() {
        HotelTab = new TabControl.Tab(document, 'hotellist', '酒店预订', GlobalConfig.SkinPath + 'Fundation/HotelList.htm', true, false, 80);
        fnAddTab(HotelTab);
    }
    ///----------
...
window.onresize = function() {
    ...
    if (_MapFullScreenState) {
        ...
        if (vM.MapState() != 0) {
            $('BdtScrollBoxBg').style.width = w - 370 + 'px';
            $('divBdtList').style.width = w - 370 + 'px';
        }
        else {
            $('BdtScrollBoxBg').style.width = w - 270 + 'px';
            $('divBdtList').style.width = w - 270 + 'px';        
        }
    }
    else {
        ...
        if (_MapExpandingState) {
            ...
            vM.MapWidth(w - 342);
            if (vM.MapState() != 0) {
                $('BdtScrollBoxBg').style.width = parseInt($('DetailMap').style.width) - 370 + 'px';
                $('divBdtList').style.width = w - 370 + 'px';
            }
            else {
                $('BdtScrollBoxBg').style.width = parseInt($('DetailMap').style.width) - 270 + 'px';
                $('divBdtList').style.width = w - 270 + 'px';
            }
        }
        else {
...
{
    $('liLoginInfo').innerHTML = '欢迎您！&nbsp;<span>[<a href="'+GlobalConfig.UserCenterUrl+'Register.aspx?city='+GlobalConfig.CityCode+'" target="_blank">注册</a>]</span>&nbsp;<span>[<a href="javascript:;" onclick="fnShowLogin()">登录</a>]</span>';
}
