...
Object.extend(EntityPopControl.prototype,{
    ...
    LoadPopData : function(){
        ...
        if(typeof _EntityInfo != 'undefined' && _EntityInfo.OwnerInfoDetail[0])
        {
           ...
           var reg = new RegExp("酒店|宾馆");
           if (reg.test( this.EntityInfo.OCName))
             {               
                    var strurl = "http://"+this.Config.Domain+"/ExistHotel.aspx?HotelName="+escape( this.EntityInfo.OCName)+"&HotelAddre="+escape(this.EntityInfo.Address)+"&CityCode="+this.Config.CityCode;
       ...
       LoadHotelXML:function(){
                if ( ExistHotel != 'false') {                  
                  this.$('HotelXML').innerHTML="<font color='#000000'>查看详情并预订</font>";
                  this.$('view_a').href='http://'+this.Config.Domain+'/yp/HotelOwner.aspx?id='+this.EntityInfo.OwnerID+'&SiteID='+ExistHotel.HotelID+'&SiteCode='+ExistHotel.SiteCode+'&CityCode='+this.Config.CityCode+'';
                }
                else 
                {
                 this.$('HotelXML').innerHTML="查看详情>>";
