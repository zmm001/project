        ...
        if(_ThemeInfo.ThemeInfoDeatil[0])
        {
            ...
            //广告
                if(window.__IsSourceAD)
                {
                        ...
                        this.$("AD_script").src=this.Config.EDataCenterUrl+'Ad/ImageAd.aspx?domain='+this.Config.Domain+'&l='+this.Config.Language+'&req=SOURCEPOP&domid=AD_a';
                }
                else
                {
                    if(!this.HasLoadAd)
                    {
                        this.$("AD_script").src=this.Config.EDataCenterUrl+'Ad/ImageAd.aspx?domain='+this.Config.Domain+'&l='+this.Config.Language+'&req=POP&domid=AD_a';
                    }
                    this.HasLoadAd=true;
                }
         }
