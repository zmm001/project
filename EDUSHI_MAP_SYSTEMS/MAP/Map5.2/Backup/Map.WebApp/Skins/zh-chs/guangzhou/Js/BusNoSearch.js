...
function fnBusStationShow(){
    ...
    { 
        ...
        $('Content').innerHTML = '没有相关数据';
       return false;
    }
    $('Content').innerHTML = sBusNo.replace('{$No}', _Bus.length).replace('{$Key}', unescape(fnRequest('key'))).replace('{$NoList}', r);
...
function fnShowInfo(i){
    ...
    if(fnRequest('action')==1){
        $('Content').innerHTML = '<div id="NoList0" class="EachRow"><div class="BusLineActive"><strong>' + _BusNumberData[i].LineInfo[0].VehicleName + '</strong> &nbsp;&nbsp;' + _BusNumberData[i].LineInfo[0].StartStation + '-' + _BusNumberData[i].LineInfo[0].EndStation + '</div><div class="DottedLine"></div></div><div id="NoInfo0" style="display:block"></div>';
        $('Content').style.height = fnGetWindowHeight()-10+'px';  //初始化高度
    }
