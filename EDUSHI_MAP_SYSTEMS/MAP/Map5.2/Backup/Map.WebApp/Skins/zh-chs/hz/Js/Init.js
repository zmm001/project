...
function fnLoadInit() {
    ...
    {
        $('liLoginInfo').innerHTML = '欢迎您！<span>[<a href="' + GlobalConfig.UserCenterUrl + 'Register.aspx" target="_blank" title="注册">注册</a>]</span><span>[<a href="javascript:;" onclick="fnShowLogin();pageTracker._trackEvent(\'Map5.2\', \'Mater_UI\', \'登录\'); ">登录</a>]</span>';
    }
    ...
    if (parseInt(navigator.appVersion.split("MSIE")[1]) == 6) {
        vM.Body.Body.style.cursor = "http://hz.edushi.com/Skins/zh-chs/Default/Images/n.cur";
        vM.Body.CurNormal = "url('http://hz.edushi.com/Skins/zh-chs/Default/Images/n.cur')";
        vM.Body.CurDrag = "url('http://hz.edushi.com/Skins/zh-chs/Default/Images/d.cur')";
    } else {
...
function fnLoadBdtList() {
    var sUrl = GlobalConfig.EDataCenterUrl + 'Bdt/NewBdtList.aspx?citycode=' + GlobalConfig.CityCode + '&l=' + GlobalConfig.Language;
    ENetwork.DownloadScript(sUrl, function() {
    var arrayHtml = [];
    arrayHtml.push('<span><a href="' + GlobalConfig.UserCenterUrl + 'User/Default.aspx?UserID=41" target="_blank" class="Us">angle</a>正在打听：<a href="' + GlobalConfig.WebRootPath + 'bdt/ListedQu.aspx?ID=66245" target="_blank">十一朋友聚会在城西这边，有什么好玩的地方推荐下！</a></span>');
            ...
            vM.onContextMenuClick = function(key, wx, wy, mx, my, spot) {
                switch (key) {
                    ...
                    case 'postbdt':
                        ...
                        if (spot) {
                            var sTitle = escape('关于“' + spot.Title + '”，我想打听。。。。。');
...
{
    var tab = new TabControl.Tab(document, 'FromHereSearch', sKeyword, GlobalConfig.SkinPath + 'Fundation/LocalSearch.aspx?cityname=' + GlobalConfig.CityName + '&type=4&keyword1=' + escape(sKeyword) + '&x=' + x + '&y=' + y + '&len=' + nArea, true, true, 80);
    fnAddTab(tab); 
}
...
function fnAddADTab() {
    var tab = new TabControl.Tab(document,'ads','赞助商',GlobalConfig.SkinPath+'Fundation/News.html',true,true,80);
    fnAddTab(tab);
}
//下载常用位置
...
window.onresize = function() {
    ...
    if (_MapFullScreenState) {
        ...
        if (vM.MapState() != 0) {
            $('BdtScrollBoxBg').style.width = w - 410 + 'px';
            $('divBdtList').style.width = w - 410 + 'px';
        }
        else {
            $('BdtScrollBoxBg').style.width = w - 310 + 'px';
            $('divBdtList').style.width = w - 310 + 'px';
        }
    }
    else {
        ...
        if (_MapExpandingState) {
            ...
            vM.MapWidth(w - 342);
            if (vM.MapState() != 0) {
                $('BdtScrollBoxBg').style.width = parseInt($('DetailMap').style.width) - 410 + 'px';
                $('divBdtList').style.width = w - 410 + 'px';
            }
            else {
                $('BdtScrollBoxBg').style.width = parseInt($('DetailMap').style.width) - 310 + 'px';
                $('divBdtList').style.width = w - 310 + 'px';
            }
        }
        else {
...
{ 
    ...
    {         
        ...
        case 'liEShopSearch': 
            ...
            $('liBusSearch').className = '';
            $('liEAddress').className = '';
            var tab = new TabControl.Tab(document,'EShopSearch','找店助手',GlobalConfig.SkinPath+'Fundation/FindEstore.html',true,true,80);
            fnAddTab(tab);
            break;
        case 'liEAddress':
            $('liEShopSearch').className = '';
...
function fnMapSearchByHotkey(sHotkey,isStat)
{
    ...
    tab = new TabControl.Tab(document, 'search', sHotkey, GlobalConfig.SkinPath + 'Fundation/LocalSearch.aspx?cityname=' + GlobalConfig.CityName + '&type=0&keyword1=' + escape(sHotkey), true, true, 80);
    fnAddTab(tab);
}
...
{    
    ...
    {        
        ...
        case '2':   //地址搜索
            ...
            tab = new TabControl.Tab(document, 'search', sKeyword1, GlobalConfig.SkinPath + 'Fundation/LocalSearch.aspx?cityname=' + GlobalConfig.CityName + '&type=' + iMapSearchType + '&keyword1=' + escape(sKeyword1), true, true, 80);
            break;
        case '3':   //周边搜索
            ...
            tab = new TabControl.Tab(document, 'search', sKeyword1 + sKeyword2, GlobalConfig.SkinPath + 'Fundation/LocalSearch.aspx?cityname=' + GlobalConfig.CityName + '&type=' + iMapSearchType + '&keyword1=' + escape(sKeyword1) + '&keyword2=' + escape(sKeyword2), true, true, 80);
            break;
    }
    fnAddTab(tab);
    switch (iMapSearchType) {
...
function fnEShopSearch(keyword,isStat)
{
    ...
    var tab = new TabControl.Tab(document,'EStoreSearch',sAddress+' '+sThing,GlobalConfig.SkinPath+'Fundation/FindEstoreResult.html?address='+ escape(sAddress) + '&name='+ escape(sThing) ,true,true,80);  
    fnAddTab(tab);
}
//黄页搜索
...
function ZoomBarClass()
{
    ...
    this.Switch3D = function(obj)
    {
        if(vM.MapState()!=0 && !vM.MapLoading())
        {
            ...
            var w = parseInt($('DetailMap').style.width);
            $('BdtScrollBoxBg').style.width = w - 310 + 'px';
            $('divBdtList').style.width = w - 310 + 'px';
        }
    };     
...
function fnShowBusStation(id, name, x, y){
    if (!_BusStationControl){
        ...
        _BusStationControl.ID = vM.appendEntity(_BusStationControl.Body, _PopLayer, false, x, y, 303, 195, 0, 38, false);
        _BusStationControl.Width = 303;
        _BusStationControl.Height = 195;
...
function fnShowBusStationIco(arrBusData, type){
    ...
    {                  
        var PopHtml='<span style="cursor:pointer; height:26px; float:left; font-size:12px; color:#000;white-space:nowrap;{$topwidth}"><span style="cursor:pointer; width:24px; height:24px; float:left; background:url(' + GlobalConfig.SkinPath + 'Images/gongjiao.gif) no-repeat; text-align:center; padding-top:2px; color:white;">{$No}</span><span style="cursor:pointer; height:19px;line-height:19px; background:#fff; padding-right:4px; float:left;{$width};" onclick="parent.fnShowBusStation({$StationID},\'{$BusStationName}\',{$X},{$Y})">{$BusStationName}</span></span>';
        if(_BusLineType==1){
            ...
            {
                vM.DrawPolyLine(_BusTransferLineLayer, vM.ChangeCoords(_BusLineData.UpCoord, vM.Property.ZoomPer[vM.Zoom()]), 4, '#ff3300', 0.7);
            }
        }else{
            ...
            {
                vM.DrawPolyLine(_BusTransferLineLayer, vM.ChangeCoords(_BusLineData.DownCoord, vM.Property.ZoomPer[vM.Zoom()]), 4, '#ff3300', 0.7);
            }          
        }
    }
}
...
function fnDrawingBusLine(busLineCoordList)
{
    ...
    for (var i=0; i<_BusLineCoordList.length; i++)
    {
        ...
        {
            PopHtml='<span style="cursor:pointer; height:26px; float:left; font-size:12px; color:#000;white-space:nowrap;{$topwidth}"><span style="cursor:pointer; width:24px; height:24px; float:left; background:url(' + GlobalConfig.SkinPath + 'Images/gongjiao2.gif) no-repeat; text-align:center; padding-top:2px; color:white;">{$No}</span><span style="cursor:pointer; height:19px;line-height:19px; background:#fff; padding-right:4px; float:left;{$width};" onclick="parent.fnShowBusStation({$StationID},\'{$BusStationName}\',{$X},{$Y})">{$BusStationName}</span></span>';            
        }
...
/********************end:公交搜索回调**********************/
function fnOpenMergeSearch(key, lst_id, domain, cid, vid, tid, card, vip, type, tags, cityname, adtype) {
    fnOpenTab();
    if (!adtype) {
        adtype = 0;
    }
    var tab = new TabControl.Tab(document, 'MergeSearch', key, GlobalConfig.SkinPath + 'Fundation/MergeSearchT.aspx?key=' + escape(key) + '&lst_id=' + lst_id + '&domain=' + domain + '&cid=' + cid + '&vid=' + vid + '&tid=' + tid + '&card=' + card + '&vip=' + vip + '&type=' + type + '&tags=' + tags + '&cityname=' + escape(cityname) + '&adtype='+adtype, true, true, 80);
    fnAddTab(tab);
    return tab;
}
function fnMarkEAddressRegister(x,y,title,content)
{
    ...
    fnOpenTab();
    var tab = new TabControl.Tab(document,'EAddressRegister','申请E地址',GlobalConfig.SkinPath+'Fundation/EAddressHandle.aspx?x='+x+'&y='+y+'&title='+escape(title)+'&content='+escape(content),true,true,80);
...
function fnEAddressRegister(name)
{
    ...
    var tab = new TabControl.Tab(document,'EAddressRegister','申请E地址',GlobalConfig.SkinPath+'Fundation/EAddressHandle.aspx?eaddress='+name,true,true,80);
    if(!_TabControl.ExistTab(tab))
    {
        _EAddressRegTab = tab;
        fnAddTab(_EAddressRegTab);
...
function fnOpenEAddressWin(obj)
{
    ...
    window.open(GlobalConfig.EdizhiUrl + obj.value);
}
