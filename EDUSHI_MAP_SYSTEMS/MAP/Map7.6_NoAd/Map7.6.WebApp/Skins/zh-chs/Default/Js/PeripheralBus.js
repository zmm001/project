...
function fnOnload()
{
    ...
    var url = window.Config.DataCetnerSearchDataUrl + 'SearchBusByRangeStation/' + window.Config.CityCode + '/' + window.Config.Language + '/Format/Json/Search?x=' + unescape(fnRequest('x')) + '&y=' + unescape(fnRequest('y'));
    ENetwork.DownloadScript(url,function(){fnShowBustList();});

}
function fnShowBustList()
{       
    var sHtml = '<div id="TabContent" class="periheralBus" style=\"padding-top:10px;\">这里是<strong>'+unescape(fnRequest('oname'))+'</strong>的周边公交：<br />点击您需要的公交站查看吧！点击"<img src="/Images/GoToIco.gif" alt="定位公交站点" />"可在地图上定位';
        ...
        sHtml +=
            '<div class="Station">' +
                '<div class="NormalInfo" onclick="fnDisplayBus(this);">' +
                    '<a>' + _Station[i].StationName + '</a>&nbsp;<img src="/Images/GoToIco.gif" alt="定位公交站点" style="cursor:pointer;" onclick="parent.fnGotoBusStation(' + _Station[i].StationID + ',\'' + _Station[i].StationName + '\',' + _Station[i].PositionX + ',' + _Station[i].PositionY + ');event.cancelBubble=true;" />' +
                '</div>' +
                '<ul class="BusList">';
