...
window.onresize = function () {
    ...
    if (_MapFullScreenState) {
        ...
        jQuery("#Content").css("height", (h - 126) + "px");
        if ((w - 650) < 500) {
        ...
        jQuery("#BodyContainer").css("height", (h - 135) + 'px');
        jQuery("#Navigation").css("height", (h - 100) + 'px');
        jQuery("#Wrapper").css("height", (h - 100) + 'px');
...
function fnCheckUserLoginInit() {
    if ($('liLoginInfo').innerHTML == '') {
        ...
        if (cookieNickNameCookie != null) {
            $('liLoginInfo').innerHTML = '<a href="' + GlobalConfig.UserCenterUrl + 'Index.aspx?city=' + GlobalConfig.CityCode + '" target="_blank"><span style="color:#ff6400;margin-right:5px;">欢迎您！ ' + unescape(cookieNickNameCookie) + '</span></a><a href="javascript:fnLoginOut();">[退出]</a>';
        }
        else {
    ...
    if (id > 0) {
        if (!_EntityPopControl) {
            ENetwork.DownloadScript('/Controls/EntityPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _EntityPopControl.ID = vM.AppendEntity(_EntityPopControl.Body, _PopLayer, false, 0, 0, 485, 310, 92, 273, false);
                } else {
                    _EntityPopControl.ID = vM.AppendEntity(_EntityPopControl.Body, _PopLayer, false, x, y, 485, 310, 92, 273, false);
                }
    ...
    if (id > 0) {
        if (!_CompanyPopControl) {
            ENetwork.DownloadScript('/Controls/CompanyPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _CompanyPopControl.ID = vM.AppendEntity(_CompanyPopControl.Body, _PopLayer, false, 0, 0, 485, 310, 92, 213, false);
                }
                else {
                    _CompanyPopControl.ID = vM.AppendEntity(_CompanyPopControl.Body, _PopLayer, false, x, y, 485, 310, 92, 213, false);
                }
...
function fnShowBusLinePop(index, x, y, type) {
    if (!_BusLinePopControl) {
        ENetwork.DownloadScript('/Controls/BusLinePopControl.js', function () {
            ...
            _BusLinePopControl.ID = vM.AppendEntity(_BusLinePopControl.Body, _PopLayer, false, x, y, 308, 300, 0, 300, false);
            _BusLinePopControl.Width = 308;
...
function fnWaterBusSearch() {
    ...
    fnGoogleStat('水上巴士搜索');
    var sStartStation = $('WaterBus1').value.trim();
    if (sStartStation.length < 1) {
        $('WaterBus1').focus();
        return;
    }
    var sEndStation = $('WaterBus2').value.trim();
    if (sEndStation.length < 1) {
        $('WaterBus2').focus();
        return;
    }
...
function fnShowWaterBusStation(id, name, x, y, type, bid) {
    if (!_WaterBusStationControl) {
        ENetwork.DownloadScript('/Controls/WaterBusStationControl.js', function () {
            ...
            _WaterBusStationControl.ID = vM.AppendEntity(_WaterBusStationControl.Body, _PopLayer, false, x, y, 330, 300, 64, 288, false);
            _WaterBusStationControl.Width = 330;
            _WaterBusStationControl.Height = 300;
...
function fnShowSubWayPop(index) {
    ...
    if (!_SubWayControl) {
        ENetwork.DownloadScript('/Controls/SubWayControl.js', function () {
            ...
            _SubWayControl.ID = vM.AppendEntity(_SubWayControl.Body, _PopLayer, false, x, y, 320, 220, 0, 0, false);
            _SubWayControl.Width = 320;
...
function fnShowFTPop(index) {
    if (!_FTPopControl) {
        ENetwork.DownloadScript('/Controls/FTPopControl.js', function () {
            ...
            _FTPopControl.ID = vM.AppendEntity(_FTPopControl.Body, _PopLayer, false, FTHouseData[index].Cx, FTHouseData[index].Cy, 380, 206, 98, 206, false);
