...
function fnShow() {
    ...
    for (i = Begin; i < End; i++) {
        ...
        if (oTheme.BCC_CompanyID * 1 == 0) {
            ...
            if (flgMapTemp) {
                if (parseInt(btid) == 431) {
                    s += LiHtml.replace('{$No}', i + 1).replace('{$Title}', oTheme.BCC_CompanyName).replace('{$Title}', t).replace('{$Address}', oTheme.BCC_Address).replace('{$ThemeImage}', sThemeImage).replace('{$yuding}', '<span style="display:block;float:right;background:#f90;color:#fff;padding:0 2px;"><a onclick="javascript:;" href="http://dacai.edushi.com/account/register" style="color:#fff;">在线购彩</a></span>').replace('{$width}', '185px').replace("{$click}", tempclick).replace("{$url}", 'javascript:;');
                }
                else {
            ...
            if (flgMapTemp) {
                if (parseInt(btid) == 431) {
                    s += LiHtml.replace('{$No}', i + 1).replace('{$Title}', oTheme.BCC_CompanyName).replace('{$Title}', t).replace('{$Address}', oTheme.BCC_Address).replace('{$ThemeImage}', sThemeImage).replace('{$yuding}', '<span style="display:block;float:right;background:#f90;color:#fff;padding:0 2px;"><a onclick="javascript:;" target="_blank" href="http://dacai.edushi.com/account/register" style="color:#fff;">在线购彩</a></span>').replace('{$width}', '185px').replace("{$click}", tempclick).replace("{$url}", 'javascript:;');
                }
                else {
                    s += LiHtml.replace('{$No}', i + 1).replace('{$Title}', oTheme.BCC_CompanyName).replace('{$Title}', t).replace('{$Address}', oTheme.BCC_Address).replace('{$ThemeImage}', sThemeImage).replace('{$yuding}', '<span style="display:block;float:right;background:#f90;color:#fff;padding:0 2px;"><a onclick="parent.parent.PiaoList();" href="#" style="color:#fff;">我要订票</a></span>').replace('{$width}', '185px').replace("{$click}", tempclick).replace("{$url}", 'javascript:;');
                }
