                ...
                for (var k = 0; k < _BusLineList.length; k++) {
                    ...
                    if (typeof type != 'undefined' && typeof bid != 'undefined') {
                        if (parseInt(type) == parseInt(_BusLineList[k].Direction) && parseInt(bid) == parseInt(_BusLineList[k].VehicleID)) {
                            sHtml = string.Format('<li><dl><dt><a href="javascript:;" onclick="window.WaterBusStationControl.onBusClick({1},\'{2}\',\'{3}\')">{0}</a><span>（{5}班次信息）</span></dt><dd><a href="javascript:;">{4}</a></dd></dl></li>', _BusLineList[k].VehicleName, _BusLineList[k].VehicleID, _BusLineList[k].VehicleName, sStationName, _BusLineList[k].VehicleFrequency, _BusLineList[k].Direction == 1 ? '上行' : '下行');
                            break;
                        }
                    } else {
                        sHtml += string.Format('<li><dl><dt><a href="javascript:;" onclick="window.WaterBusStationControl.onBusClick({1},\'{2}\',\'{3}\')">{0}</a><span>（{5}班次信息）</span></dt><dd><a href="javascript:;">{4}</a></dd></dl></li>', _BusLineList[k].VehicleName, _BusLineList[k].VehicleID, _BusLineList[k].VehicleName, sStationName, _BusLineList[k].VehicleFrequency, _BusLineList[k].Direction == 1 ? '上行' : '下行');
