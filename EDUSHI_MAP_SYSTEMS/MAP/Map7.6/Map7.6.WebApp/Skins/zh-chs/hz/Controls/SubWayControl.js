...
Object.extend(SubWayControl.prototype, {
    ...
    ShowBusStation: function(data) {
        ...
        for (var i = 0; i < data.List.length; i++) {
            t += string.Format("<a style=\"background:{0};cursor:pointer;\" onclick=\"window.SubWayControl.onBusClick({2},'{3}','{4}');\">{1}</a>", data.List[i].Color.toString() == '' ? '#cc0000' : data.List[i].Color, data.List[i].Name, data.List[i].ID, data.List[i].Name, data.List[i].Name);
        }
