    ...
    if (id > 0) {
        if (!_EntityPopControl) {
            ENetwork.DownloadScript('/Controls/EntityPopControl.js', function () {
                ...
                _EntityPopControl.onMergeSearch = function (oid, ocname, x, y) {
                    if (_EntityPopControl.Tab != null) {
                        _EntityPopControl.Tab.destroy();
                    }
                    var tab = new TabControl.Tab(document, 'companyList', ocname, '/Fundation/CompanyList.html?x=' + x + '&y=' + y + '&ocname=' + escape(ocname) + '&oid=' + oid, true, true, 80);
                    fnAddTab(tab);
                    _EntityPopControl.Tab = tab;
    ...
    if (id > 0) {
        if (!_HotelPopControl) {
            ENetwork.DownloadScript('/Controls/HotelPopControl.js', function () {
                ...
                //兼容地图定位
                if (x == 0 && y == 0) {
                    _HotelPopControl.ID = vM.AppendEntity(_HotelPopControl.Body, _PopLayer, false, 0, 0, 328, 340, 62, 332, false);
                }
                else {
                    _HotelPopControl.ID = vM.AppendEntity(_HotelPopControl.Body, _PopLayer, false, x, y, 328, 340, 62, 332, false);
                }
