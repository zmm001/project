...
LocalSearch.prototype.Show = function()
{
    if(typeof searchData!='undefined' &&searchData!=null)
    {
        ...
        var _LocalSearch = '<div class="LocalResult">共有<span>{$Count}</span>项符合<span>{$Key}</span>的查询结果'
                        ...
                        +'<ul class="LocalList"><!--_Li--><li style="height:auto;"><div style="height:20px; width:283px; overflow:hidden;">{$No}.<a title="{$Title}" onclick="parent.ResultControlForm._entityClick({$oid},{$cid},{$X},{$Y});">{$Title}</a>{$IsVip}</div>'
        ...
        for(i=Begin;i<End;i++){
            ...
            if (this.Data.LST_ID[i]*1 > 0){
                sTmpHtml = sTmpHtml.replace('{$IsVip}', '<a href="' + window.Config._BendiUrl + 'VipStore/' + this.Data.LST_ID[i] + '/Index.aspx?City=' + window.Config._CityCode + '&StoreID=' + this.Data.CompanyID[i] + '" target="_blank"><img src="' + window.Config._ImgPath + 'Edian.gif" alt="E店" style="margin:0 0 -7px 3px; border:none" /></a>');
            }
            else{
