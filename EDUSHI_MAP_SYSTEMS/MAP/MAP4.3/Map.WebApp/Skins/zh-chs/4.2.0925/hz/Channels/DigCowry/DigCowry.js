...
Object.extend(DigCowryControl.prototype, MapContrlBase.prototype);
...
DigCowryControl.prototype.ShowSuccess = function(){
    ...
    this.Body.src = GlobalConfig._WebRootPath + 'DigCowry/DigSuccess.aspx?rd='+ Math.round(Math.random()*10000);
    this.Show();
    fnMakeCell();
};
//挖宝失败泡泡
DigCowryControl.prototype.ShowFailture = function(){
    this.Body.src = GlobalConfig._WebRootPath + 'DigCowry/DigFailture.aspx?DcGlobalADImg=' + escape(_DcGlobalADImg) + '&DcGlobalADImgLink=' + escape(_DcGlobalADImgLink);
    this.Show();
};
...
{
    _DcPop.Hide();
    //window.open(GlobalConfig._WebRootPath + 'DigCowry/TrophyVerdict.aspx?TrophyID=' + cid, '_blank');
};
...
Object.extend(DigCowry.prototype,{
    ...
    Init:           function(){
        ...
        //初始化栅格大小,以及冠名商图名广告
        ENetwork.DownloadScript(GlobalConfig._DataCenterUrl + 'CommMap/Dig.aspx?node='+GlobalConfig._Node+'&req=1&l=' + GlobalConfig._L + '&v=1.0',function(){
            if (typeof GConfig != 'undefined' && GConfig)
            {
                _DcCellSize.w = GConfig.GridWidth * 1;
                _DcCellSize.h = GConfig.GridHeight * 1;
                _DcGlobalADImg = GConfig.PADPic2;
                _DcGlobalADImgLink = GConfig.PADUrl2;
            }
        });
        
        $('MapToolBar').parentNode.style.width = '230px';
        $('MapToolBar').innerHTML = '<li id="liDigCowry" style="background-image:url({$ChannelPath}images/digicon.gif);background-repeat:no-repeat; margin-top:-1px;" onclick="fnBeginDig();"><a title="挖宝" ></a></li>' +  $('MapToolBar').innerHTML;
        ...
        tip.style.top = getTop($('liDigCowry'))*1+23 + 'px';
        ...
        vM.onMapClick = function(e){
            if (_DcIsBeginDigCowry)
            {
                ...
                _DcTimer = setTimeout(function(){
                    //判断点击区域是否在随机栅格中
                    if (x > _DcCurrentCell.x && x < _DcCurrentCell.x + vM.GetMapPos(_DcCellSize.w)
...
//产生随机栅格
function fnMakeCell(){
    if (vM.MapWidth() * vM.MapHeight() < 400000)
    {
        _DcCurrentCell.x = 0;
        _DcCurrentCell.y = 0;
        _DCLayer2.innerHTML = '';
...
function fnShowDigPop(flag, x, y){
   if (!_DcPop){
        _DcPop = new DigCowryControl({Parent:vM});
