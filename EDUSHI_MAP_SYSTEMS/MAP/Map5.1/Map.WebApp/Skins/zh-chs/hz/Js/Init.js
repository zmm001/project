...
function fnInitPicAD(){
    var sADUrl = GlobalConfig.EDataCenterUrl + 'CommMap5.0/ad.aspx?domain='+GlobalConfig.Domain+'&l='+GlobalConfig.Language+'&req=5';
    ...
    if(!_EntityPopControl)
    {
        ...
        _EntityPopControl.onLoadComplete  = function(){
            ...
            vM.onSignClick = function(sign){
                switch (sign.SortKey){
                    case 'bus':
                        fnShowBusStation(sign.ID,sign.Title,sign.X,sign.Y);
                        break;
                    case 'vir':
...
{
    $('liLoginInfo').innerHTML = '欢迎您！<span style="color:#ff6400">'+memberInfo.LM_NickName+'</span><span>[<a href="http://bendi.edushi.com/Member/Index.aspx?City=hz" target="_blank">用户中心</a>]</span>&nbsp;<span>[<a href="javascript:fnLoginOut();">退出</a>]</span>';
    ...
    if(memberInfo.LM_EAddress.length > 0){
        ...
        $('lnkMyEAddress').href = 'http://bendi.edushi.com/Member/MyEdushi.aspx?City=hz';
...
{
    $('liLoginInfo').innerHTML = '欢迎您！E粉丝&nbsp;<span>[<a href="http://bendi.edushi.com/hz/Register.shtml" target="_blank">注册</a>]</span>&nbsp;<span>[<a href="javascript:;" onclick="fnShowLogin()">登录</a>]</span>';
    $('lnkMyEAddress').setAttribute('onmouseover', '');
    $('lnkMyEAddress').setAttribute('onmouseout','');
    $('lnkMyEAddress').target = '_blank';
    $('lnkMyEAddress').href = 'http://bendi.edushi.com/Member/MyEdushi.aspx?City=hz';
...
function fnEShopSearch(keyword)
{
    ...
    window.open(GlobalConfig.BendiUrl + GlobalConfig.CityCode + '/LocalSearch/?' + escape(sKeyword), '_blank', '');
}
//黄页搜索
...
function ZoomBarClass()
{
    ...
    this.Switch23D = function(obj)
    {
        if(vM.MapState()!=0)
        {
            vM.MapState(0);
            obj.title = '2D';
            obj.className = 'Change2D'
        }
        else
        {
            vM.MapState(1);
            obj.title = '3D';
            obj.className = 'Change3D'            
        }
    } 
}
//地图移动和鹰眼联动
...
function fnUrlParse()
{
    ...
    if (q != '')
    {
        fnMapSearchByHotkey(q);
    }
    var nq = fnRequest('nq');
    if (nq != '')
    {
        fnNearbySearch(nq,x,y,1000);
    }
