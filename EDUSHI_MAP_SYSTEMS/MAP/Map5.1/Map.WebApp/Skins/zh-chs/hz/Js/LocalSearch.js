...
{
    _PageSize = Math.floor((fnGetWindowHeight()-95-20*iVouchAdLength)/46);
    if(_PageSize < 5)
    {
        _PageSize = 5;
    }
...
function fnShowData()
{
    ...
    _SearchData = _Search;
    var sLocalSearchHtml = '<div class="LocalResult" id="TabContent">共有<strong>{$Count}项</strong>符合<strong>{$Key}</strong>的查询结果'
                    ...
                    +'<ul class="LocalList"><!--_Li--><li style="height:auto;"><div class="Number"><span>{$No}</span></div><div class="DetailCon"><div class="TitleNav"><span class="HeadLine"><a href="javascript:;" title="{$Title}" onclick="if(_PrevElement != null){_PrevElement.className=\'Number\';};this.parentNode.parentNode.parentNode.previousSibling.className=\'Number Currently\';_PrevElement=this.parentNode.parentNode.parentNode.previousSibling;parent.fnShowSearchPop({$oid},{$cid},{$lstid},{$X},{$Y})">{$Title}</a></span>{$IsVip}</div><div class="Address">地址：{$Address}</div></div>'
                    +'<div class="DottedLine"></div></li><!--/_Li--></ul><div class="Paginate">{$Page}</div>'
                    +'<!--{$RandomTopicList}--></div><div class="Photo"><a href="{$ADLink}" target="_blank"><img src="{$ADImagePath}" width="276px" height="45px" style="border:0px;{$DisPlay}"></a></div>';
    ...
    for(i=Begin;i<End;i++){
        ...
        if (_Search.SearchTable[i].LST_ID >0 && _Search.SearchTable[i].Vip > 0)
        {
            sTmpHtml = sTmpHtml.replace('{$IsVip}', '<span class="IsVipIco"><a href="http://' + _Search.SearchTable[i].Eaddress + '.' + window.Config.Domain + '" target="_blank"><img src="' + window.Config.SkinPath + 'Images/Edian.gif" border="0" /></a></span>');
        }
        else
        {
    ...
    if(typeof parent._StaticAd != 'undefined' && parent._StaticAd.SearchListAd.length>0 )
    {
        var arrSearchAdPic = parent._StaticAd.SearchListAd[0];
        r = r.replaceAll('{$DisPlay}','display:block').replace('{$ADLink}','http://'+arrSearchAdPic.MAI_ImageLink.replace('http://','')).replace('{$ADImagePath}',window.Config.PicUrl + arrSearchAdPic.MAI_ImagePath+'/'+arrSearchAdPic.MAI_ImageName);
    }
    else
    {
    ...
    $('TabContent').style.height = fnGetWindowHeight()-60+'px';  //初始化高度
