            ...
            if(arrBusData!=null)
            {
                ...
                if(type==1){
                    ...
                    for(i=0;i<l;i++){
                        ...
                        var nd = vM.$C('div');
                        nd.style.filter = ' progid:DXImageTransform.Microsoft.BasicImage(grayscale=1)';
                        nd.id='B_pop'+i;
...
function fnAppendIcon(x, y, text, sImgPath, sImgPath2, sFn, w, h, ew, eh, sEyeImg){
    ...
    var p = vM.$C('div');
    p.style.filter = ' progid:DXImageTransform.Microsoft.BasicImage(grayscale=1)';
    ...
    var pe = vMe.$C('div');
    pe.style.filter = ' progid:DXImageTransform.Microsoft.BasicImage(grayscale=1)';
...
function fnLoadPro(){
    ...
    ENetwork.DownloadScript(url,function(){
        ...
        var tid = fnRequest('tid');       //主题URL参数定位
...
function fnSaveHistory(type, spot){
    ...
    if($('NoHistoryLi'))
    {
        $('JDhistroy').innerHTML='<li onmouseover="OverBackground(this)" onmouseout="OutBackground(this)" onclick="' + sFnName + '(\''+spot.ID+'\', ' + spot.CenterX + ', ' + spot.CenterY + ');">' + _HistoryArray.length + '. ' + spot.OwnerName+'</li>';
    }
    else
    {
         $('JDhistroy').innerHTML='<li onmouseover="OverBackground(this)" onmouseout="OutBackground(this)" onclick="' + sFnName + '(\''+spot.ID+'\', ' + spot.CenterX + ', ' + spot.CenterY + ');">' + _HistoryArray.length + '. ' + spot.OwnerName+'</li>'+$('JDhistroy').innerHTML;
    }
}
