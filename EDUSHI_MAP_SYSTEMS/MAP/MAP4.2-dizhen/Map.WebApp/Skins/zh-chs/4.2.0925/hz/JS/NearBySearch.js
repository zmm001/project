...
function fnOnload()
{
    ...
    var _EntrPopASearch = '<div class="textLine" style="padding:2px; margin-top:10px;">查找范围：<input name="rdArea" id="minArea" value="500" onclick="ChangeArea(this)" checked="checked" type="radio" />500米&nbsp;<input name="rdArea" id="maxArea" onclick="ChangeArea(this)" value="1000" type="radio" />1000米 </div>'
                        +'<div style="border-top:dotted 1px #999; margin:10px 10px 10px 0"></div><div class="textLine" style="padding:2px 5px 0 5px; ">'
                        +'<div style="width:220px; float:left;">关键字：<input id="txtKeyword" value="输入你要查找的关键字" style="width:155px;color:#333; border:solid 1px #333; margin-right:3px; " onfocus="this.value=\'\';" type="text"  /></div><div id="btnSearch" class="btn" style="background-image:url(../' + window.Config._ImgPath + '14.jpg); " onclick="DoSearch($(\'txtKeyword\').value,{$x},{$y});"></div></div>'
                        +'<div style="border-top:solid 1px #999; margin:10px 10px 3px 0;clear:both;"></div><div class="textLine" style="padding:0 5px">推荐关键字：{$seachHotKeyWords}</div>';
    var sHotKeyList = '';
