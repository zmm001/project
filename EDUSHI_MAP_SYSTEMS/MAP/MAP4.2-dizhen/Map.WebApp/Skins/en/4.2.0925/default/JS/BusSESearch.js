...
function fnShow(){
    ...
    if(typeof sStartAndEndData!='undefind'&&sStartAndEndData!=null){        
        ...
        if (sl == 0)
        {
            s = '<option value="">no results</option>';
        }
        if (el == 0)
        {
            e = '<option value="">no results</option>';
        }
        var _BusSE = '<div class="se"><div class="title">the keywords you input includes more than one optimatic stops, find out the most o</div><div>From<span>{$start}</span></div>'
                    +'<div><select id="start">{$ostart}</select></div><div>To<span>{$end}</span></div>'
        ...
        document.body.innerHTML="No Datas";
    }
}
function fnLoadInfo(){
...
function fnShowInfo(){
    ...
    if(sChangeRouteData==null){
        document.body.innerHTML='can not find any possible transpotation，<a style="cursor:pointer;" onclick="fnShow();">Go Back</a>';
    }else{
        var l=sChangeRouteData.FirstBusName_Js.length;
        if(sChangeRouteData.Tag==1){
            for(i=0;i<l;i++){
                s+='<div class="xianlu"><div class="left" onclick="parent.ResultControlForm.BusIDSearch('+sChangeRouteData.FirstBusId_Js[i]+',\''+sChangeRouteData.FirstBusName_Js[i]+'\')">'+sChangeRouteData.FirstBusName_Js[i]+'</div><div class="right">Direct Route</div></div>';
            }
        }else{
            for(i=0;i<l;i++){
                s+='<div class="xianlu"><div class="left">Choices'+(i+1)+':</div><div class="right">take<span>'+sChangeRouteData.FirstBusName_Js[i]+'</span>first and get off at <span>'+sChangeRouteData.Stationname_Js[i]+'</span>, than take<span>'+sChangeRouteData.SecondBusName_Js[i]+'</span></div></div>'
            }
        }
        var _BusSEInfo = '<div class="jie"><div class="title">From<span>{$start}</span>To<span>{$end}</span></div><div class="fangan">Possible transportation choices</div>'
                        +'<div class="shuomimg"><div class="left">Routes</div><div class="right">Choices</div></div>{$info}<a style="cursor:pointer;margin-left:120px;" onclick="fnShow();">Go Back</a></div>'
