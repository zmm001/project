...
LocalSearch.prototype.Show = function()
{
    if(typeof searchData!='undefined' &&searchData!=null)
    {
        ...
        //以上修正无法分页问题
        var _LocalSearch = '<div class="LocalResult">There have <span>{$Count}</span> results searched of keyword "<span>{$Key}</span>"'
                        ...
                        +'<ul class="LocalList"><!--_Li--><li><div style="height:20px; width:283px; overflow:hidden;">{$No}.<a title="{$Title}" onclick="parent.ResultControlForm._entityClick({$oid},{$cid},{$X},{$Y});">{$Title}</a></div>'
                        +'<span style="margin-left:15px; color:#656565;">Address：{$Address}</span><br /></li><!--/_Li--></ul><h2>{$Page}</h2>'
        ...
        if(iCount>0)
        {
            for(var i=0;i<iCount;i++)
            {
                ...
                {
                    c+=TrHtml.replace("{$Acontent}","<a href=\"javascript:parent.ResultControlForm._vouchClick('"+escape(vouchadList.Title[i])+"',vouchadList.Content[" + i + "],"+vouchadList.X[i]+","+vouchadList.Y[i]+")\" style=\"text-decoration:underline;font-weight:bold;color:red;font-size:12px;\">"+vouchadList.Title[i]+"</a>");
                }
                else
                {
        ...
        for(i=Begin;i<End;i++){
            ...
            s+=LiHtml.replace('{$No}',i+1).replace('{$Title}',this.Data.OCName[i]).replace('{$Title}',t).replace('{$Address}',this.Data.Address[i]).replace('{$EID}',this.Data.Eaddress[i]).replace('{$AppDomain}',window.Config._AppDomain).replace('{$X}',this.Data.x[i]).replace('{$Y}',this.Data.y[i]).replace('{$oid}',this.Data.OwnerID[i]).replace('{$cid}',this.Data.CompanyID[i]);
        }
        var strPage=fnPageShow(this.Page,this.PageCount,this.PageSize,'pageStr',this.FnName, ' total of {$PageCount}');
