            ...
            function() {
                if (typeof _TransferResult == undefined || _TransferResult == null) {
                    ...
                    if (_TransferResult.length < 1) {
                        ...
                        $('TabContent').className = 'ulResultList';
...
function fnLoadStationList(url)
{
    ENetwork.DownloadScript(url,
        function(){
            ...
            $('TabContent').innerHTML = sBusSE.replace('{$ostart}', s).replace('{$oend}', e);
...
function fnShowSearchResult(url,start,end) {
    parent.fnOpenTab();
    var tab = new parent.TabControl.Tab(parent.document, 'BusSearch', start+'→'+end, '/Fundation/BusTransferSearch.html?action=newTab&url='+encodeURIComponent(url), true, true, 80);
    parent.fnAddTab(tab);
}
function fnTransfer()
{
