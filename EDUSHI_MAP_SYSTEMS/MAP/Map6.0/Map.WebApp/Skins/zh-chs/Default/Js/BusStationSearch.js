...
function fnBusStationShow() {
    if (typeof _Station != 'undefined' && _Station.length > 0) {
        ...
        var s = '';
        for (i = 0; i < l; i++) {
            s = s + '<div class="action" onclick="fnShow(' + i + ');"><div class="h" id="h' + i + '"><div class="imgs" id="img' + i + '"></div><a>' + _Station[i].StationName.replace(unescape(fnRequest('key')), '<span style="color:#ff8601">' + unescape(fnRequest('key')) + '</span>') + '</a>&nbsp;<img src="/Images/GoToIco.gif" alt="定位公交站点" style="cursor:pointer;" onclick="parent.fnGotoBusStation(' + _Station[i].StationID + ',\'' + _Station[i].StationName + '\',' + _Station[i].PositionX + ',' + _Station[i].PositionY + ');event.cancelBubble=true;" /></div></div><div id="LinInfo' + i + '" class="info" style="display:none;">{$list}</div>';
        ...
        $('TabContent').innerHTML = sBusStation.replace('{$Blist}', s).replace('{$No}', l).replace('{$Key}', unescape(fnRequest('key')));

    }
    else {
        $('TabContent').innerHTML = '<div class="NoInfoClew">没有相关数据！</div>';
    }
    fnResize();
}
function fnShow(i){
