function GetHotels(currentid) {
    ...
    var url = "Channel/Hotels.aspx?mid=" + currentid  + "&rnd=" + Math.random();
...
function fnGetHotelInfo() {
    ...
    if (typeof SearchTable != 'undefined') {
        ...
        if (htLength > 0) {
            $('jd').innerHTML = '<div class="title">预订...</div>';
            ...
            for (var i = 0; i < htLength; i++) {
                ...
                hotelList += ' <li><span class="span5 y5">高级客房</span> <span class="span6 y6"><a href="{$HotelUrl}" target="_blank">{$sityName} </a> </span><span class="span7 y5">{$price} </span><span class="span8"><a href="{$HotelUrl}" target="_blank">预订</a> </span></li>';
