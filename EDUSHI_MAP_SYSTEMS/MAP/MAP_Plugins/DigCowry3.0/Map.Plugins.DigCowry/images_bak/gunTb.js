...
return array;},applyConfig:function(obj,config){if(obj&&config&&typeof config=='object'){for(var p in config){if(!POP.lang.hasOwnProperty(obj,p))
obj[p]=config[p];}}
return obj;}};

TB.widget={};TB.widget.SimpleScroll={};TB.widget.SimpleMarquee={};

TB.widget.SimpleTab=new function(){var Y=POP.util;var defConfig={eventType:'click',currentClass:'Current',tabClass:'',autoSwitchToFirst:true,stopEvent:true,delay:0.3};var getImmediateDescendants=function(p){var ret=[];if(!p)return ret;for(var i=0,c=p.childNodes;i<c.length;i++){if(c[i].nodeType==1)
ret[ret.length]=c[i];}
...
(function(){var Y=POP.util;TB.widget.Slide=function(container,config){this.init(container,config);}
...
this.delayTimeId=null;this.autoPlayTimeId=null;this.curSlide=-1;this.sliding=false;this.pause=false;this.onSlide=new Y.CustomEvent("onSlide",this,false,Y.CustomEvent.FLAT);if(POP.lang.isFunction(this.config.onSlide)){this.onSlide.subscribe(this.config.onSlide,this,true);}
