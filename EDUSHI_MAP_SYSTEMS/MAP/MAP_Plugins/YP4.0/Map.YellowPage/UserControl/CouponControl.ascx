...
<div class="list">
    <div class="title"><a href="<%=DianUrl%>cuxiao/Voucher.aspx" target="_blank">优惠...</a></div >
      <ul>
        ...
        <ItemTemplate>
        <li><a class="jiequ_w_130" target="_blank" href='<%#getLink(DianUrl, "?id=" + ObjToStr(Eval("LV_ID")) + "&StoreID=" + ObjToStr(Eval("MCI_ID")), "LV_ShowTitle", Eval("LST_ID"), Eval("LCE_Level"))%>' title='<%#Eval("LV_Title") %>'><%#Eval("LV_Title")  %> </a></li>       
     </ItemTemplate>
   </asp:Repeater>
     </ul>
      </div>
