...
function getinfo(edataurl, x, y, kw, domain, mapUrl, id, state) {
    var rimBusHTML = '<h1><span class=\"tyellow\">周边</span>公交信息</h1>';
        ...
        for (var i = 0; i < s.length; i++) {
            if (typeof s[i].Bus != 'undefined' && s[i].Bus.length > 0) {
                ...
                if (bus != '' && stationName != '') {
                    ...
                    //rimBusHTML += '<li>【<a href="http://' + domain + '?sid=' + stationID + '&sname=' + escape(stationName) + '&x=' + x + '&y=' + y + '">' + stationName + '</a>】' + bus + '' + '</li> ';
                    rimBusHTML += '<li>【' + stationName + '】' + bus + '' + '</li> ';
                }    
            }
        }
    }
...
function GetLivInfo(mapUrl, id, state) {
    var liviHtml = '<h1><span class="tyellow">周边</span>生活服务信息</h1>';
    var cs = ' <li><h3>便利店/超市</h3>'; //超市/便利店
    var yh = ' <li><h3>银行/ATM机</h3>'; //周边银行
    var yy = '  <li> <h3>医院</h3>'; //周边医院
    var xx = '<li style="padding:8px 0 0 0"><h3>学校</h3>'; //学校
    var shbmjf = ''; // 生活便民缴费
    ...
    liviHtml += cs + '</li>' + yh + '</li>' + yy + '</li>' + xx + '</li>';
...
function fnJobInfoHTML(curr, index, obj, state) {
    ...
    for (var n = (CurrentPage - 1) * pageSize; n < CurrentPage * pageSize; n++) {
        ...
        if (n == (CurrentPage - 1) * pageSize) {
            zhaopinHtml += '<div id="subDivZhaopin{$index}" class="Elsebox"><h2>招聘信息({$sityName})</span></h2><div class="list"><table width="100%" border="0" cellspacing="0" cellpadding="0"> <thead><tr><td width="24%">职位名称</td> <td>公司名称</td><td width="12%">工作地点</td><td width="20%">发布日期</td> <td width="12%">详细信息</td> </tr></thead><tbody>{$JobsHTML}</tbody></table></div></div>';
