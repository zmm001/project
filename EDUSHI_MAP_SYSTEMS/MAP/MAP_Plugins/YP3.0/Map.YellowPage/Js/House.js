...
function flagHouse() {
    if (typeof HouseTable != 'undefined') {
        ...
        if (SearchHouse.length > 0) {
            this.$('div_house').innerHTML = '';
            for (var i = 0; i < HouseTable.length; i++) {
                sumArray.push(HouseTable[i].length);
                if (HouseTable[i].length > 0) {
...
function houseHtml(curr, index, obj, state) {
    ...
    for (var n = (CurrentPage - 1) * pageSizeHouse; n < CurrentPage * pageSizeHouse; n++) {
        ...
        if (n == (CurrentPage - 1) * pageSizeHouse) {
            houseHtml += '<div id="subDivHouse{$index}"><h2 class="hh02"><span class="l">房产信息({$SiteName})</span></h2> <div class="list"><table width="100%" border="0" cellspacing="0" cellpadding="0"><thead><tr><td width="30%">房产名称</td><td width="70%">单元/m<sup>2</sup></td></tr></thead><tbody>{$houselist}</tbody></table>';
            houseHtml = houseHtml.replace('{$SiteName}', SearchHouse[index][n].SiteName).replace('{$index}', index);
        }
        houselist += '<tr><td><a href="{$Url}" target="_blank"><span class="sp01">{$Name}</a></span></td><td><a href="{$Url}" target="_blank">{$UnitPrice}&nbsp;元/㎡</a>&nbsp;&nbsp;&nbsp;&nbsp;(本房源信息来自于{$SiteName})</td></tr>';
