...
var action = {
    init: function () {
        var self = this
        var gender = getQueryString("category1")
        var category = getQueryString("category2")
        if (gender != null && category != null) {
            $('.options').attr("class", gender == "1" ? 'options male' : 'options female');
            var documents = $(gender == "1" ? '#male a' : '#female a')//$('.item ' + gender == "1" ? '.male' : '.female')
            $.each(documents, function (i, n) {
                if ($(n).data('id') == category) {
                    $(n).addClass("current").siblings().removeClass("current");
                }
            })
            $('.options .selected').css("display", "block")
            states.options.gender = gender == "1" ? 'male' : 'female'
            states.options.category = category
            $('.options .selected').html(foreOptions)
            setting.data({
                category1Id: null,
                category2Id: parseInt(category),
                isAll: true,
                NovelRecommendType: 1,
                BookStatus: 0,
                WordNumType: 0,
                UpdateType: 0,
                pageIndex: 0,
